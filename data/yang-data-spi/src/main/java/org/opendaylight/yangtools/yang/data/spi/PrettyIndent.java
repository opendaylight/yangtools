 ...
 */
package org.opendaylight.yangtools.yang.data.spi;
 ...
 */
public final class PrettyIndent {
    ...
    static {
        ...
        if (indent < 1) {
            LOG.warn("Invalid pretty-indent %s, using %s instead", indent, DEFAULT_INDENT);
            indent = DEFAULT_INDENT;
        } else if (indent != DEFAULT_INDENT) {
            LOG.info("Using pretty-indent %s", indent);
        }
    ...
    public static StringBuilder indent(final StringBuilder sb, final int depth) {
        int remaining = depth;
