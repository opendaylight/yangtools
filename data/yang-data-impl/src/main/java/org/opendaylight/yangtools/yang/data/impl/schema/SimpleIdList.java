...
/**
 * A naive implementation if {@link IdList}. This implementation seeks to be correct, but is not terribly efficient
 * nor thread-safe. Attempts at concurrent modification will result in a wrecked, unusable state.
 */
@Beta
public final class SimpleIdList<I, E extends Identifiable<I>> extends AbstractList<E>
        implements IdList<I, E> {
    ...
    // Remove by element is no longer optional, as the default implementation of IdList.removeElement calls it.
    public boolean remove(final E element) {
        final var ret = elements.remove(element);
        if (ret) {
            verifyNotNull(map.remove(element.getIdentifier(), element));
        }
        return ret;
    }
