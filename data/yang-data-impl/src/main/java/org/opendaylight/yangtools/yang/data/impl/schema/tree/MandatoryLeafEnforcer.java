...
final class MandatoryLeafEnforcer implements Immutable {
    ...
    private static void findMandatoryNodes(final Builder<MandatoryDescendant> builder,
        ...
        Collection<? extends AugmentationSchemaNode> schemaAugmentations =
            schema instanceof AugmentationTarget ? ((AugmentationTarget) schema).getAvailableAugmentations() :
                Collections.emptyList();
        for (final DataSchemaNode child : schema.getChildNodes()) {
            if (SchemaAwareApplyOperation.belongsToTree(type, child)) {
                boolean schemaIsAugmentationRoot = schemaAugmentations.stream().anyMatch(augmentationSchemaNode ->
                    augmentationSchemaNode.findDataChildByName(child.getQName()).isPresent());
                if (child instanceof ContainerSchemaNode) {
                    final ContainerSchemaNode container = (ContainerSchemaNode) child;
                    if (!container.isPresenceContainer()) {
                        if (schemaIsAugmentationRoot) {
                            // The container is augmentation content of augmentation that is augmenting the data tree
                            // in the 'schema' point. The container (and it's subtree) has to be based
                            // on that augmentation in the mandatory node reference.
                            final AugmentationSchemaNode aug = getAugIdentifierOfChild(schema, child);
                            findMandatoryNodes(builder,
                                id.node(DataSchemaContextNode
                                    .augmentationIdentifierFrom(aug)).node(NodeIdentifier.create(container.getQName())),
                                container, type);
                            continue;
                        }
                        // the schema-to-child(container) is not going to new augmentation, so just appending the NodeID
                        // to the ongoing ID and continue the search.
