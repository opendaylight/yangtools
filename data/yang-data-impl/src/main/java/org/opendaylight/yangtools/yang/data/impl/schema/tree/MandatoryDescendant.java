    ...
    static @NonNull MandatoryDescendant create(final YangInstanceIdentifier parentId,
        ...
        if (childSchema.isAugmenting()) {
            if (!inAugmentedSubtree) {
                verify(parentSchema instanceof AugmentationTarget, "Unexpected augmenting "
                    + "child %s in non-target %s at %s", childSchema, parentSchema, parentId);
            ...
            final YangInstanceIdentifier legacyPath =
                YangInstanceIdentifier.create(augSubtreePath.subList(1, augSubtreePath.size()));
