...
public class BitsStringCodecTest {

    @ParameterizedTest(name = "Deserialize \"{1}\" -> {2}")
    @MethodSource("bitsSerializationArgs")
    void deserialization(int bitsTotal, final String input, final Bits output) {
    ...
    @MethodSource("bitsSerializationArgs")
    void serialization(int bitsTotal, final String output, final Bits input) {
        final BitsCodec<String> codec = getCodec(buildBitsTypeDefinition(bitsTotal), BitsCodec.class);
    ...
    private static String buildBitsValue(int... selectedIndexes) {
        return String.join(" ", IntStream.of(selectedIndexes).mapToObj(i -> "Bit" + i).toList());
    }

    private static final Bits buildBitsObj(final int numOfBits, final String bytesAsHex) {
