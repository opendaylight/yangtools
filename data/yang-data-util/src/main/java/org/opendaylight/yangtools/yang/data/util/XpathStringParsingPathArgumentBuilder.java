...
final class XpathStringParsingPathArgumentBuilder implements Mutable {
    ...
    private String nextDoubleQuotedValue() {
        ...
        while (true) {
            ...
            // Decode next escape as per https://www.rfc-editor.org/rfc/rfc7950#section-6.1.3
            final char escape = data.charAt(nextBackslash + 1);
            sb.append(switch (escape) {
                ...
                default -> {
                    // adjust offset before referencing it
                    offset = nextBackslash;
                    throw iae("Unrecognized escape '" + escape + "'");
                }
            });
