...
/**
 * A {@link List} where each element has a unique identifier. In addition to normal List operations, which interface
 * specifies a number of lookup and modification methods which operate on elements' identifiers.
 */
@Beta
public interface IndexedList<I, E extends Identifiable<I>> extends List<E> {
    /**
     ...
     * @throws IllegalArgumentException if there is already an element with the same identifier as {@code element}
     */
    int insertAfter(final I id, E element);
     ...
     * @throws IllegalArgumentException if there is already an element with the same identifier as {@code element}
     */
    int insertBefore(final I id, E element);
}
