...
/**
 * Node which has been normalized according to the YANG schema and is identifiable by a
 * {@link org.opendaylight.yangtools.yang.data.api.YangInstanceIdentifier}.
 * These nodes form the interior of a {@link NormalizedTree}.
 *
 * <p>
 * See subinterfaces of this interface for concretization of node.
 */
/*
 * FIXME: 11.0.0: NormalizedNode represents the perfectly-compliant view of the data, as evaluated by an implementation,
 ...
 *                "UnverifiedData".
 *
 * FIXME: 11.0.0: Once we have UnverifiedData, we should really rename this to "NormalizedData" or similar to unload
 *                some "Node" ambiguity. "Node" should be a generic term reserved for a particular domain -- hence
 *                'node' can be used to refer to either a 'schema node' in context of yang.model.api, or to
 *                a 'normalized data node' in context of yang.data.api.
 *
 * FIXME: 11.0.0: Well, not quite. The structure of unverified data is really codec specific -- and JSON and XML
 ...
 * FIXME: 11.0.0: Seal this interface
 */
public non-sealed interface NormalizedNode extends NormalizedData, Identifiable<PathArgument> {
    @Override
