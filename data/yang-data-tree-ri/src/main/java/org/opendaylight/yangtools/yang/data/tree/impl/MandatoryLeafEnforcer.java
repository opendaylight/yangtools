...
final class MandatoryLeafEnforcer implements Immutable {
    ...
    //
    //        We then can short-circuit to using NormalizedNodes.getDirectChild() for each step and fail if we do not
    //        find a step. Obviously in that case we need to reconstruct the full path -- but that is an error path and
    //        we can expend some time to get at that. A simple Deque<PathArgument> seems like a reasonable compromise
    //        to track what we went through...
