        ...
        for (final DataSchemaNode child : schema.getChildNodes()) {
            if (SchemaAwareApplyOperation.belongsToTree(type, child)) {
                if (child instanceof ContainerSchemaNode container) {
                    ...
                    boolean needEnforce = child instanceof MandatoryAware mandatory && mandatory.isMandatory();
                    if (!needEnforce && child instanceof ElementCountConstraintAware constraintAware) {
                        needEnforce = constraintAware.getElementCountConstraint()
                            .map(constraint -> {
