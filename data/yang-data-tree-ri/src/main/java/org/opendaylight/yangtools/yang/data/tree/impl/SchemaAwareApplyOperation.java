...
abstract sealed class SchemaAwareApplyOperation<T extends DataSchemaNode> extends ModificationApplyOperation
    ...
    Optional<? extends TreeNode> apply(final ModifiedNode modification, final TreeNode storeMeta,
        ...
        return switch (modification.getOperation()) {
            case DELETE -> {
                ...
                modification.resolveModificationType(storeMeta != null ? ModificationType.DELETE
            ...
            case TOUCH -> {
                ...
                yield modification.setSnapshot(Optional.of(applyTouch(modification, storeMeta, version)));
            }
            case MERGE -> {
                ...
                if (storeMeta == null) {
                    ...
                    result = applyWrite(modification, modification.getWrittenValue(), storeMeta, version);
                    fullVerifyStructure(result.getData());
                } else {
                    result = applyMerge(modification, storeMeta, version);
                }
            ...
            case WRITE -> {
                ...
                yield modification.setSnapshot(Optional.of(applyWrite(modification,
                    verifyNotNull(modification.getWrittenValue()), storeMeta, version)));
            }
            case NONE -> {
                modification.resolveModificationType(ModificationType.UNMODIFIED);
                yield Optional.ofNullable(storeMeta);
            }
        };
    }
     ...
     * @param storeMeta Current state of data tree
    ...
    protected abstract void checkTouchApplicable(ModificationPath path, NodeModification modification,
        @Nullable TreeNode storeMeta, Version version) throws DataValidationFailedException;
