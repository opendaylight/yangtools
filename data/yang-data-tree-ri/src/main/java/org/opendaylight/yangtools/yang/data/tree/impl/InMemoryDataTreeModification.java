...
final class InMemoryDataTreeModification extends AbstractCursorAware implements CursorAwareDataTreeModification {
     ...
     */
    private abstract sealed static class SingletonState implements State {
        @Override
    ...
    private Entry<YangInstanceIdentifier, ModifiedNode> resolveTerminal(final YangInstanceIdentifier path) {
        ...
        return switch (local) {
            ...
            case Prepared prepared -> lockedResolveTerminal(prepared, prepared.root, path);
    ...
    public void applyToCursor(final DataTreeModificationCursor cursor) {
        ...
        switch (local) {
            ...
            case Prepared prepared -> lockedApplyToCursor(prepared, cursor, prepared.root);
    ...
    // FIXME: These two may be racing with validate()/prepare()/commit(). What we mean to say is that we want to
