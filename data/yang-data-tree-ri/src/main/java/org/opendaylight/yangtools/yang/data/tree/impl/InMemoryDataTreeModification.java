...
final class InMemoryDataTreeModification extends AbstractCursorAware implements CursorAwareDataTreeModification {
    ...
    private Optional<NormalizedNode> readNode(final YangInstanceIdentifier path,
            final @Nullable ModifiedNode rootNode) {
        if (rootNode == null) {
            return snapshot.readNode(path);
        }

        final var terminal = resolveTerminal(rootNode, path);
    ...
    private Optional<VersionInfo> readVersionInfo(final YangInstanceIdentifier path,
            final @Nullable ModifiedNode rootNode) {
        if (rootNode == null) {
            return snapshot.readVersionInfo(path);
        }
    ...
    private <T> T accessData(final BiFunction<YangInstanceIdentifier, @Nullable ModifiedNode, T> function,
        ...
        return switch (local) {
            case Open open -> accessData(open, function, path, open.root);
            case Noop noop -> accessData(noop, function, path, null);
            case Ready ready -> accessData(ready, function, path, ready.root);
            case AppliedToSnapshot ready -> accessData(ready, function, path, ready.root);
            case Prepared prepared -> accessData(prepared, function, path, prepared.root);
            default -> throw illegalState(local, "access data of");
        };
    }

    private static <T> T accessData(final State observed,
    ...
    public void applyToCursor(final DataTreeModificationCursor cursor) {
        ...
        switch (local) {
            ...
            case Prepared prepared -> applyToCursor(prepared, cursor, prepared.root);
    ...
    void validate(final YangInstanceIdentifier path, final TreeNode current) throws DataValidationFailedException {
        final var local = acquireState();
        switch (local) {
            case Noop noop -> skipValidate(noop);
