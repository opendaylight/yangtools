...
import org.opendaylight.yangtools.yang.parser.spi.meta.NamespaceStorage.Global;
...
final class SchemaTreeNamespaceBehaviour<D extends DeclaredStatement<QName>, E extends SchemaTreeEffectiveStatement<D>>
     ...
     */
    @Override
    public StmtContext<QName, D, E> getFrom(final Global global, final NamespaceStorage storage,
            final QName key) {
    ...
    @Override
    public Map<QName, StmtContext<QName, D, E>> getAllFrom(final Global global, final NamespaceStorage storage) {
        // FIXME: 7.0.0: this method needs to be well-defined
        return null;
    }

    @Override
    public void addTo(final Global global, final NamespaceStorage storage, final QName key,
    ...
    private static NamespaceStorage globalOrStatementSpecific(final NamespaceStorage storage) {
        var current = storage;
        while (true) {
            if (current.level() == Level.STATEMENT) {
                return current;
            }
