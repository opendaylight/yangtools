...
public final class SubstatementValidator {
    ...
    private @Nullable SourceException evaluate(final StmtContext<?, ?, ?> ctx, final StatementCardinality cardinality,
        ...
        if (count == null) {
            ...
            return new MissingSubstatementException(ctx,
                "%s is missing %s. Minimal count is %s. Error in module %s (%s)", statementNameOf(currentStatement),
                statementNameOf(cardinality.def), cardinality.minRequired, root.rawArgument(),
    ...
    private @Nullable SourceException evaluate(final StmtContext<?, ?, ?> ctx, final StatementDefinition def,
        ...
        if (cardinality == null) {
            if (ctx.namespaceItem(ParserNamespaces.EXTENSION, def.statementName()) == null) {
                final var root = ctx.getRoot();
                return new InvalidSubstatementException(ctx, "%s is not valid for %s. Error in module %s (%s)",
                    statementNameOf(def), statementNameOf(currentStatement), root.rawArgument(),
                    ctx.namespaceItem(ParserNamespaces.MODULECTX_TO_QNAME, root));
            }
            return null;
        }
    ...
    private @Nullable InvalidSubstatementException evaluateMinMax(final StmtContext<?, ?, ?> ctx,
        ...
        if (count < cardinality.minRequired()) {
            ...
            return new InvalidSubstatementException(ctx,
                "Minimal count of %s for %s is %s, detected %s. Error in module %s (%s)", statementNameOf(def),
                statementNameOf(currentStatement), cardinality.minRequired(), count, root.rawArgument(),
                ctx.namespaceItem(ParserNamespaces.MODULECTX_TO_QNAME, root));
        }
        return evaluateMax(ctx, def, cardinality, count);
    }
    ...
    private @Nullable InvalidSubstatementException evaluateMax(final StmtContext<?, ?, ?> ctx,
        ...
        if (count > cardinality.maxAllowed()) {
            final var root = ctx.getRoot();
            return new InvalidSubstatementException(ctx,
                "Maximal count of %s for %s is %s, detected %s. Error in module %s (%s)", statementNameOf(def),
                statementNameOf(currentStatement), cardinality.maxAllowed(), count, root.rawArgument(),
                ctx.namespaceItem(ParserNamespaces.MODULECTX_TO_QNAME, root));
        }
        return null;
    }
    ...
    @NonNullByDefault
    private static String statementNameOf(final StatementDefinition def) {
        final var qname = def.statementName();
        return YangConstants.RFC6020_YIN_MODULE.equals(qname.getModule()) ? qname.getLocalName() : qname.toString();
    }
