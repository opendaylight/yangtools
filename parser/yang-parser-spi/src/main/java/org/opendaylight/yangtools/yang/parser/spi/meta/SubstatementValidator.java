...
public final class SubstatementValidator {
    ...
    @NonNullByDefault
    private @Nullable SourceException evaluate(final NamespaceStmtCtx ctx, final StatementCardinality cardinality,
            final @Nullable Integer count) {
        if (count != null) {
            return evaluateMinMax(ctx, cardinality, count);
        }
        return new MissingSubstatementException(ctx, "statement %s requires at least %s: none present",
            currentStatement.humanName(), substatementsStr(cardinality.def, cardinality.minRequired));
    }
    ...
    private @Nullable SourceException evaluate(final @NonNull NamespaceStmtCtx ctx,
        ...
        // The parent statement has not defined cardinality for this statement, which means it is not allowed.
        // If the offending statement is an extension, though, we allow it, relying on the extension to enforce its
        // valid parents.
        if (def.isExtension()) {
            return null;
        }
        return new InvalidSubstatementException(ctx, "statement %s does not allow %s substatements: %s present",
            currentStatement.humanName(), def.humanName(), count);
    }

    @NonNullByDefault
    private @Nullable InvalidSubstatementException evaluateMinMax(final CommonStmtCtx ctx,
            final StatementCardinality cardinality, final int count) {
        final var def = cardinality.def;
        if (count < cardinality.minRequired()) {
            return new InvalidSubstatementException(ctx, "statement %s requires at least %s: %s present",
                currentStatement.humanName(), substatementsStr(def, cardinality.minRequired()), count);
        }
        return evaluateMax(ctx, def, cardinality, count);
    }
