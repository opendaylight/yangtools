...
public record ResolvedSourceInfo(
    ...
    public Map<String, QNameModule> getImportsPrefixToQNameIncludingSelf() {
        final Map<String, QNameModule> allPrefixed = getImportsPrefixToQName();
        if (prefix != null) {
    ...
    public Map<String, QNameModule> getImportsPrefixToQName() {
        final Map<String, QNameModule> importPrefixed = new HashMap<>();
        imports.forEach((imp) -> importPrefixed.put(imp.prefix, imp.importedModule.qnameModule));
        return importPrefixed;
    }
