...
public final class StmtContextUtils {
    ...
    private static void validateIfFeatureOnLeaf(final StmtContext<QName, ListStatement, ?> listCtx,
        ...
        final var it = leafCtx.allSubstatementsStream()
            .filter(stmt -> stmt.producesAnyOf(IfFeatureStatement.DEF, WhenStatement.DEF))
            .toList()
            .iterator();
    ...
    public static @Nullable QNameModule getModuleQNameByPrefix(final @NonNull RootStmtContext<?, ?, ?> ctx,
        ...
        final var imported = resolvedInfo.mapAccessibleModulesToPrefixes().get(prefix);
        if (imported != null) {
            return imported;
        }
        ...
        if (ctx.produces(SubmoduleStatement.DEF)) {
            verify(resolvedInfo.belongsTo().prefix().equals(prefix),
                "The specified prefix %s does not match an imported or belongs-to prefix of module %s", prefix,
                resolvedInfo.sourceId());
