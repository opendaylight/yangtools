...
final class Quirk20130929 implements InferenceAction {
    ...
    private final @NonNull ArrayList<Prerequisite<ModuleEffectiveStatement>> reqNacmModules;
    ...
    private Quirk20130929(final Prerequisite<? extends Mutable<?, ?, ?>> reqIetfNetconf,
            final List<Prerequisite<ModuleEffectiveStatement>> reqNacmModules) {
    ...
    static void applyTo(final @NonNull Mutable<?, ?, GetFilterElementAttributesEffectiveStatement> gfeaStmt) {
        ...
        final var inference = gfeaStmt.newInferenceAction(ModelProcessingPhase.EFFECTIVE_MODEL);
        inference.apply(new Quirk20130929(inference.mutatesEffectiveCtx(rootModule),
            nacmModules.stream().map(nacmModule -> inference.requiresEffective(nacmModule)).toList()));
    }
    ...
    public void apply(final InferenceContext ctx) {
        ...
        for (var reqNacmModule : reqNacmModules) {
            ...
            for (var extension : nacmModule.extensions()) {
                final var qname = extension.argument();
                if (!DEFAULT_DENY_ALL.equals(qname.getLocalName())) {
                    LOG.debug("Skipping extension {}", qname);
                    continue;
                }
                ...
                for (var rpc : rpcs) {
                    ...
                    LOG.debug("Quirk added {} to rpc {}", qname, rpc.getArgument().getLocalName());
                }
                return;
            }
        }
