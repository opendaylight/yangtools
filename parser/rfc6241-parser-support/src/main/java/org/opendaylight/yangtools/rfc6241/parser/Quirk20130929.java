...
final class Quirk20130929 implements InferenceAction {
    ...
    static void applyTo(final @NonNull Mutable<?, ?, GetFilterElementAttributesEffectiveStatement> gfeaStmt) {
        ...
        if (rootModule == null) {
            LOG.info("Ignoring quirk in non-module {}", root);
            return;
        }
        final var module = root.currentModule();
        if (!module.equals(NetconfConstants.RFC6241_MODULE)) {
            LOG.info("Ignoring quirk in non-RFC6241 module {}", module);
            return;
        }
        ...
        final var nacmModules = verifyNotNull(root.namespace(ParserNamespaces.NAMESPACE_TO_MODULE)).entrySet().stream()
            .filter(entry -> {
                ...
                if (!NACMConstants.MODULE_NAME.equals(stmt.argument())) {
                    LOG.info("Skipping mis-named {}", stmt);
                    return false;
                }
                if (!NACMConstants.MODULE_NAMESPACE.equals(entry.getKey().namespace())) {
                    LOG.info("Skipping mis-namespaced {}", stmt);
                    return false;
                }
                LOG.info("Accepting {}", stmt);
                return true;
            })
        ...
        if (nacmModules.isEmpty()) {
            LOG.info("Ignoring quirk as there are no ietf-netconf-acm modules present");
            return;
        }

        LOG.info("Considering {}", nacmModules);
    ...
    public void apply(final InferenceContext ctx) {
        ...
        if (rpcs.isEmpty()) {
            LOG.info("Ignoring quirk as there are no target RPCs");
            return;
        }
        ...
        for (var reqNacmModule : reqNacmModules) {
            ...
            for (var stmt : nacmModule.declaredSubstatements()) {
                ...
                if (extension == null) {
                    LOG.info("Skipping non-extension {}", stmt);
                    continue;
                }
                final var qname = extension.getArgument();
                if (!DEFAULT_DENY_ALL.equals(qname.getLocalName())) {
                    LOG.info("Skipping extension {}", qname);
                    continue;
                }
                if (!extension.isSupportedToBuildEffective()) {
                    LOG.info("Skipping unsupported extension {}", qname);
                    continue;
                }

                final var support = ietfNetconf.namespaceItem(StatementSupport.NAMESPACE, qname);
                if (support == null) {
                    LOG.info("Skipping effective extension {} without support, weird", qname);
                    continue;
                }
                if (!(support instanceof UndeclaredStatementFactory)) {
                    LOG.info("Skipping effective extension {} without support for undeclared statements", qname);
                    continue;
                }
                ...
                for (var rpc : rpcs) {
                    ...
                    LOG.info("Quirk added {} to rpc {}", qname, rpc.getArgument().getLocalName());
                }
                return;
            }
        }
        LOG.info("Skipping quirk as there is no supported nacm:default-deny-all implementation");
    }
