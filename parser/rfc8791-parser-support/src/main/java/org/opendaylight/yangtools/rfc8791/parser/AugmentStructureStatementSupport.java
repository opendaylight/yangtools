...
public final class AugmentStructureStatementSupport
    ...
    public AugmentStructureArgument parseArgumentValue(final StmtContext<?, ?, ?> ctx, final String value) {
        if (!(ArgumentUtils.nodeIdentifierFromPath(ctx, value) instanceof Absolute abs)) {
            throw new SourceException(ctx, "Argument '%s' is not an absolute schema node identifier", value);
        }
        final var steps = abs.getNodeIdentifiers();
    ...
    @Override
    public void onStatementAdded(final StmtContext.Mutable<AugmentStructureArgument, AugmentStructureStatement,
