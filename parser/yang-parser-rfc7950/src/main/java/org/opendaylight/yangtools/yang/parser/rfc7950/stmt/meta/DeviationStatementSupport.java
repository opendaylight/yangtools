...
public final class DeviationStatementSupport
    ...
    private static QNameModule getBelongsToModule(final @NonNull StmtContext<?, ?, ?> root) {
        // Submodule must have exactly one belongs-to statement - enforced by SubmoduleStatementSupport
        return Iterables.getOnlyElement(
            root.namespace(ParserNamespaces.BELONGSTO_PREFIX_TO_QNAME_MODULE).values()
        );
    }

    private static QNameModule getModuleQName(final @NonNull StmtContext<?, ?, ?> ctx,
            final @NonNull StmtContext<?, ?, ?> root) {
