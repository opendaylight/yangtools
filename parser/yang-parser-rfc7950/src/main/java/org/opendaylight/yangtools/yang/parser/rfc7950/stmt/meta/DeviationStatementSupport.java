...
public final class DeviationStatementSupport
    ...
    private static final SubstatementValidator SUBSTATEMENT_VALIDATOR =
        SubstatementValidator.builder(DeviationStatement.DEF)
            .addOptional(DescriptionStatement.DEF)
            .addAny(DeviateStatement.DEF)
            .addOptional(ReferenceStatement.DEF)
            .build();

    public DeviationStatementSupport(final YangParserConfiguration config) {
        super(DeviationStatement.DEF, StatementPolicy.reject(), config, SUBSTATEMENT_VALIDATOR);
    }

    @Override
    public Absolute parseArgumentValue(final StmtContext<?, ?, ?> ctx, final String value) {
        return ctx.identifierBinding().parseAbsoluteSchemaNodeidAs("deviation-arg", ctx, value);
    }
    ...
    public void onFullDefinitionDeclared(final Mutable<Absolute, DeviationStatement, DeviationEffectiveStatement> ctx) {
        ...
        final var currentModule = root.produces(SubmoduleStatement.DEF)
    ...
    private static @Nullable QNameModule getBelongsToModule(final StmtContext<?, ?, ?> root) {
    ...
    private static @NonNull QNameModule getModuleQName(final StmtContext<?, ?, ?> root) {
        return verifyNotNull(root.namespaceItem(ParserNamespaces.RESOLVED_INFO, Empty.value()),
