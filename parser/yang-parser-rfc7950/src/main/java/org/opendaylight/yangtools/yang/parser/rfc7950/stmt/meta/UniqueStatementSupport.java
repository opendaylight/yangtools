...
public final class UniqueStatementSupport
    ...
    public Set<Descendant> adaptArgumentValue(
        ...
        if (allMatchModule(origArg.stream().flatMap(desc -> desc.getNodeIdentifiers().stream()), targetModule)) {
            return origArg;
        }
        ...
        return origArg.stream()
            .map(descendant -> {
                ...
                return allMatchModule(nodeIds.stream(), targetModule) ? descendant
    ...
    private static boolean allMatchModule(final Stream<QName> qnames, final QNameModule module) {
