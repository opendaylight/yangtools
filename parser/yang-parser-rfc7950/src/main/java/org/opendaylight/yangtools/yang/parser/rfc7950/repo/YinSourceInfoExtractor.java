...
public abstract sealed class YinSourceInfoExtractor implements SourceInfo.Extractor {
    private static final class ForModule extends YinSourceInfoExtractor {
        ...
        private @NonNull XMLNamespace extractNamespace() throws ExtractorException {
            return getFirstElementArgument(root, NAMESPACE, NAMESPACE_ARG, (element, arg) -> {
                try {
    ...
    @NonNullByDefault
    @FunctionalInterface
    private interface ArgumentFunction<T> {

        T apply(Element parent, String argument) throws ExtractorException;
    }
    ...
    public static SourceInfo.Extractor of(final Node root, final SourceIdentifier sourceId) throws ExtractorException {
        ...
        if (!RFC6020_YIN_NAMESPACE_STRING.equals(element.getNamespaceURI())) {
            throw new ExtractorException("Root element does not have YIN namespace", refOf(element));
        }
        return switch (element.getLocalName()) {
            ...
            default -> throw new ExtractorException("Root element needs to be a module or submodule", refOf(element));
        };
    }
    ...
    final Unqualified extractPrefix(final Element parent) throws ExtractorException {
        return getFirstElementArgument(parent, PREFIX, PREFIX_ARG, (element, arg) -> {
            try {
    ...
    @NonNullByDefault
    final <T> T getFirstElementArgument(final Element parent, final String keyword, final String argument,
            ArgumentFunction<T> function) throws ExtractorException {
        ...
        for (int i = 0, length = childNodes.getLength(); i < length; ++i) {
            if (childNodes.item(i) instanceof Element element && isYinElement(element, keyword)) {
                ...
                if (arg != null && arg.getSpecified()) {
                    return function.apply(element, arg.getValue());
                }
    ...
    @NonNullByDefault
    private static ExtractorException newInvalidArgument(final Element stmt, final Exception cause) {
        return new ExtractorException(
            "Invalid argument to " + stmt.getLocalName() + ": " + cause.getMessage(), cause, refOf(stmt));
    }

    @NonNullByDefault
    private static ExtractorException newMissingSubstatement(final Element parent, final String keyword) {
