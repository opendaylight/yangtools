...
public abstract sealed class YinSourceInfoExtractor implements SourceInfo.Extractor {
    private static final class ForModule extends YinSourceInfoExtractor {
        ...
        private @NonNull XMLNamespace extractNamespace() throws ExtractorException {
            ...
            for (int i = 0, length = childNodes.getLength(); i < length; ++i) {
                if (childNodes.item(i) instanceof Element element && isYinElement(element, NAMESPACE)) {
                    final var arg = element.getAttributeNodeNS(null, NAMESPACE_ARG);
                    if (arg == null || !arg.getSpecified()) {
                        throw new ExtractorMissingArgumentException(null, NAMESPACE);
                    }
                    try {
                        return XMLNamespace.of(arg.getValue());
    ...
    private static final class ForSubmodule extends YinSourceInfoExtractor {
        ...
        @Override
        public SourceInfo.@NonNull Submodule extractSourceInfo() throws ExtractorException {
