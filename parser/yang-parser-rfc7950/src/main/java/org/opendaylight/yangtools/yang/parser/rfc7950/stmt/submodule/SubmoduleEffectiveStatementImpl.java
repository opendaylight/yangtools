...
final class SubmoduleEffectiveStatementImpl
    ...
    SubmoduleEffectiveStatementImpl(final Current<Unqualified, SubmoduleStatement> stmt,
        ...
        final var belongsTo = verifyNotNull(resolvedInfo.belongsTo());
        verify(findBelongsTo(stmt, substatements).prefix().argument().equals(belongsTo.prefix()), "Belongs-to "
            + "prefix of the submodule’s effective statement does not match the prefix in its resolved info");
        final var belongsToModuleQName = belongsTo.parentModuleQname();
         ...
         */
        final var includedSubmodules = resolvedInfo.includes();
        if (!includedSubmodules.isEmpty()) {
            ...
            for (final var submoduleCtx : includedSubmodules) {
                submoduleContextsInit.add(
                    (StmtContext<?, SubmoduleStatement, SubmoduleEffectiveStatement>)submoduleCtx.root());
            }
