...
enum AugmentStrategy {
    ...
    private void copyStatement(final List<Mutable<?, ?, ?>> buffer, final Mutable<?, ?, ?> target,
        ...
        if (dataDef != null) {
            final var qname = dataDef.getArgument();
            final var existing = target.namespaceItem(ParserNamespaces.schemaTree(), qname);
            if (existing != null) {
                throw new InferenceException(dataDef,
                    "An augment cannot add node '%s' because this name is already used in target by a %s statement",
                    qname, existing.publicDefinition().humanName());
            }
        }
