        ...
        if (stmt.producesDeclared(DataDefinitionStatement.class)) {
            ...
            if (existing != null) {
                throw new InferenceException(stmt, """
                    ...
                    same name (originating from %s)""", stmt.publicDefinition().getStatementName(), arg.getLocalName(),
                    existing.publicDefinition().getStatementName(), existing.sourceReference());
            }
        }
