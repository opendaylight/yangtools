...
class YT1825Test {
    ...
    void testResolution() throws Exception {
        ...
        final var context = reactor.newBuild()
            .addSource(YangStatementStreamSource.create(new URLYangTextSource(YT1825Test.class.getResource(
                "/ietf-netconf-acm@2018-02-14.yang"))))
            .addSource(YangStatementStreamSource.create(new URLYangTextSource(YT1825Test.class.getResource(
                "/ietf-yang-types@2013-07-15.yang"))))
            .buildEffective();

        final var module = context.findModule(NACMConstants.RFC8341_MODULE).orElseThrow();
        final var nacm = assertInstanceOf(ContainerSchemaNode.class,
            module.dataChildByName(QName.create(NACMConstants.RFC8341_MODULE, "nacm"))).asEffectiveStatement();
