 ...
 * Identification requires {@link ModelProcessingPhase#FULL_DECLARATION} to ascertain the module revision, which we want
 * to match exactly.
 */
final class IetfRestconfModuleInference implements InferenceAction {
    ...
    private IetfRestconfModuleInference(final Prerequisite<? extends Mutable<?, ?, ?>> prereq) {
    ...
    static void applyTo(@NonNull final Mutable<?, ?, ?> module) {
        ...
        if (module.producesDeclared(ModuleStatement.class) && IETF_RESTCONF.equals(module.rawArgument())) {
            ...
            action.apply(new IetfRestconfModuleInference(prereq));
        }
    }
                    ...
                    if (grpSub.producesDeclared(ContainerStatement.class) && "restconf".equals(grpSub.rawArgument())) {
                        for (var contSub : grpSub.mutableDeclaredSubstatements()) {
                            if (contSub.producesDeclared(ContainerStatement.class)
                                ...
                                IetfRestconfOperationsInference.applyTo(moduleCtx, contSub);
                                return;
                            }
                        }
                    }
