...
public final class YangDataStatementSupport
    ...
    public void onStatementDefinitionDeclared(
            final Mutable<String, YangDataStatement, YangDataEffectiveStatement> ctx) {
        ...
        if (ctx.isSupportedToBuildEffective() && "yang-api".equals(ctx.argument())) {
            ...
            if (stmts.size() == 1) {
                ...
                if (stmt.producesEffective(UsesEffectiveStatement.class) && "restconf".equals(stmt.rawArgument())) {
                    ...
                    // done later in inference processing
                    IetfRestconfModuleInference.applyTo(ctx.coerceParentContext());
                }
            }
        }
    }
