...
public final class SourceLinkageResolver {
    ...
    private abstract static sealed class Src<
        ...
        final ReactorSource<?> source;
        final I info;
        final R ref;

        private boolean required;

        Src(final ReactorSource<?> source, final I info, final R ref) {
    ...
    private static final class ModuleSrc extends Src<SourceInfo.Module, SourceLinkage.ModuleRef, ModuleLinkage> {
        ...
        ModuleSrc(final ReactorSource<?> source, final Module info) {
            super(source, info, new SourceLinkage.ModuleRef(info.sourceId()));
        }
    ...
    private static final class SubmoduleSrc
        ...
        SubmoduleSrc(final ReactorSource<?> source, final Submodule info) {
            super(source, info, new SourceLinkage.SubmoduleRef(info.sourceId()));
        }
    }
    ...
    @NonNullByDefault
    public SourceLinkage.Ref addSource(final ReactorSource<?> source) {
    ...
    @NonNullByDefault
    public SourceLinkage.Ref addLibSource(final ReactorSource<?> source) {
        return insertSource(source).ref;
    }

    @NonNullByDefault
    private Src<?, ?, ?> insertSource(final ReactorSource<?> source) {
        switch (source.sourceInfo()) {
            case SourceInfo.Module info -> {
                final var module = new ModuleSrc(source, info);
                ...
                modules.put(module.ref, module);
                return module;
            }
            case SourceInfo.Submodule info -> {
                final var submodule = new SubmoduleSrc(source, info);
