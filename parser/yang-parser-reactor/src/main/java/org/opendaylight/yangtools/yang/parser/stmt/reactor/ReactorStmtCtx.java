...
abstract sealed class ReactorStmtCtx<A, D extends DeclaredStatement<A>, E extends EffectiveStatement<A, D>>
    ...
    private static final int TRANSIENT_INSTANCE  = 0x01;
    private static final int IGNORE_IF_FEATURE   = 0x02;
    private static final int NORESOLVE_CONFIG    = 0x04;
    ...
    private void initFlags(final StatementDefinitionContext<?, ?, ?> def) {
        final var support = def.support();
        if (support.isIgnoringConfig()) {
            flags |= TRANSIENT_INSTANCE;
        }
        if (support.isIgnoringIfFeatures()) {
            flags |= IGNORE_IF_FEATURE | SET_SUPPORTED_BY_FEATURES;
        }
    }

    final void initFlags(final ReactorStmtCtx<?, ?, ?> parent) {
        if (parent.isIgnoringIfFeature()) {
            flags |= IGNORE_IF_FEATURE | SET_SUPPORTED_BY_FEATURES;
        }
        if (parent.isTransientInstance()) {
            flags |= TRANSIENT_INSTANCE;
        }
        if (parent.notResolvingConfig()) {
            flags |= NORESOLVE_CONFIG;
        }
    }
    ...
    private @NonNull EffectiveConfig loadEffectiveConfig(final ReactorStmtCtx<?, ?, ?> parent) {
        ...
        if (notResolvingConfig()) {
            return EffectiveConfig.UNDETERMINED;
        }

        final Boolean myConfig;
        final var parentConfig = parent.nullableConfig();
        final var optConfig = findSubstatementArgument(ConfigEffectiveStatement.class);
        if (optConfig.isPresent()) {
            myConfig = optConfig.orElseThrow();
            // Validity check: if parent is config=false this cannot be a config=true
            if (myConfig && parentConfig != null && !parentConfig) {
    ...
    private @Nullable Boolean nullableConfig() {
        final var config = effectiveConfig();
        return switch (config) {
            case IGNORED -> throw new VerifyException(this + " is ignoring config: this should never happen");
            default -> config.asNullable();
        };
    }
    ...
    @Override
    public final boolean isIgnoringIfFeature() {
        return (flags & IGNORE_IF_FEATURE) != 0;
    }

    @Override
    public final boolean isTransientInstance() {
        return (flags & TRANSIENT_INSTANCE) != 0;
    }

    private boolean notResolvingConfig() {
        return (flags & NORESOLVE_CONFIG) != 0;
    }
