...
final class ResolvedSourceBuilder {
    ...
    public ResolvedSourceInfo build(final Map<SourceSpecificContext, ResolvedSourceInfo> allResolved) {
        ...
        buildFinished = new ResolvedSourceInfo(sourceInfo.sourceId(), resolveQnameModule(),
            prefix, context.getRoot(), resolveImports(allResolved), resolveIncludes(), resolvedBelongsTo);
        return buildFinished;
    }

    private List<ResolvedSourceInfo.ResolvedImport> resolveImports(
        ...
        return imports.build().entrySet().stream()
            .map(prefixedImport -> {
                ...
                return new ResolvedSourceInfo.ResolvedImport(prefixedImport.getKey(), allResolved.get(impContext));
            })
            .toList();
    }
