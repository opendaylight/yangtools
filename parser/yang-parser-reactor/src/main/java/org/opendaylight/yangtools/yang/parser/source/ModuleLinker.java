...
final class ModuleLinker extends SourceLinker {
    ...
    /**
     * {@return a stream of the names of submodules required by this module and lack an exact revision specification}
     */
    Stream<Unqualified> inexactSubmodules() {
    ...
    void narrowInexact(final Unqualified submodule, final RevisionUnion revision) {
        ...
        switch (spec) {
            ...
            default -> throw new VerifyException("Attempted to narrow " + submodule.getLocalName() + " from " + spec);
        }
    }
    ...
    @Nullable SubmoduleLinker lookupSubmodule(final Unqualified submodule) {
        return submoduleSpecs.get(requireNonNull(submodule)) instanceof ResolvedRevision resolved ? resolved.submodule
            : null;
    }
    ...
    private void requireInclude(final SourceLinker source, final Include dependency) throws ReactorException {
        ...
        if (revision == null) {
            ...
            if (!submoduleSpecs.containsKey(name)) {
                ...
                submoduleSpecs.put(name, new AnyRevision(depRef != null ? depRef : source.sourceId().toReference()));
            }
            return;
        }
        ...
        // care about the two HashMap lookup operations.
