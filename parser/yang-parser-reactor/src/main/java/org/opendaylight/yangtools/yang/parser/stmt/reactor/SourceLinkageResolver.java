    ...
    private void tryResolveBelongsTo() {
        for (final Map.Entry<SourceIdentifier, SourceIdentifier> mapping : submoduleToParentMap.entrySet()) {
            ...
            if (resolvedSubmodule == null) {
                // FIXME: better exception
                throw new IllegalStateException("Submodule %s was not resolved".formatted(submoduleId));
            }
            ...
            if (resolvedParent == null) {
                ...
                throw new IllegalStateException(
                    "Parent module %s of submodule %s was not resolved".formatted(parentId, submoduleId));
            }
        ...
        while (iterator.hasNext()) {
            ...
            for (final SourceIdentifier sibling : siblings) {
                ...
                if (resolvedSibling == null) {
                    // FIXME: better exception
                    throw new IllegalStateException("Included module %s of module %s was not resolved".formatted(
                        sibling, resolvedSource.context().getSourceInfo().sourceId()));
                }
