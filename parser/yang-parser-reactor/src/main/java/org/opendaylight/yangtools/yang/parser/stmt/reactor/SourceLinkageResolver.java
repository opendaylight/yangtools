...
public final class SourceLinkageResolver {
        ...
        for (var source : allSources.entrySet()) {
            if (source.getValue() instanceof SourceInfo.Submodule submoduleInfo) {
                ...
                throw new SomeModifiersUnresolvedException(ModelProcessingPhase.INIT, submoduleId,
                    tryInferenceException(belongsTo.sourceRef(), submoduleId,
                        "Module [" + parentName.getLocalName() +  "] from belongs-to was not found"));
            }
        }
    }
                ...
                if (match == null) {
                    ...
                    if (dependency instanceof Import) {
                        throw new SomeModifiersUnresolvedException(ModelProcessingPhase.INIT, current,
                            tryInferenceException(dependency.sourceRef(), current,
                                "Imported module [" + dependencyName.getLocalName() + "] was not found"));
                    }

                    // FIXME: also handling of BelongsTo?
                    throw new SomeModifiersUnresolvedException(ModelProcessingPhase.INIT, current,
                        tryInferenceException(dependency.sourceRef(), current,
                            "Included submodule [" + dependencyName.getLocalName() + "] was not found"));
                }
                    ...
                    if (dep instanceof Import importedDep) {
                        ...
                        if (importedDep.revision() != null && currentVersion == YangVersion.VERSION_1) {
                            if (dependencyVersion != YangVersion.VERSION_1) {
                                throw new SomeModifiersUnresolvedException(ModelProcessingPhase.INIT, current,
                                    tryVersionLinkageException(importedDep.sourceRef(), current,
                                        "Cannot import by revision version " + dependencyVersion + " module "
                        ...
                        if (currentVersion != dependencyVersion) {
                            throw new SomeModifiersUnresolvedException(ModelProcessingPhase.INIT, current,
                                tryVersionLinkageException(dep.sourceRef(), current,
    ...
    @NonNullByDefault
    private static Exception tryInferenceException(final @Nullable StatementSourceReference sourceRef,
            final SourceIdentifier sourceId, final String message) {
        if (sourceRef != null) {
            return new InferenceException(message, sourceRef);
        }
        return new IllegalStateException(message + " [guessed at " + sourceId.toYangFilename() + "]");
    }


    @NonNullByDefault
    private static Exception tryVersionLinkageException(final @Nullable StatementSourceReference sourceRef,
            final SourceIdentifier sourceId, final String message) {
        if (sourceRef != null) {
            return new YangVersionLinkageException(sourceRef, "%s", message);
        }
        return new IllegalStateException(message + " [guessed at " + sourceId.toYangFilename() + "]");
    }
}
