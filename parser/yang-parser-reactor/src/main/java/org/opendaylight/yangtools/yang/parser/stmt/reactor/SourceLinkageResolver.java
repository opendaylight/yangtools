...
public final class SourceLinkageResolver {
    ...
    public static void fillNamespaces(final @NonNull ResolvedSourceInfo resolvedSource) {
    ...
    private static void populateRootNamespaces(final @NonNull RootStatementContext<?, ?, ?> root,
            final @NonNull ResolvedSourceInfo resolvedSource) {
    ...
    private static void fillCommonNamespaces(final @NonNull RootStatementContext<?, ?, ?> root,
            final @NonNull ResolvedSourceInfo resolvedSource) {
    ...
    private static void fillIncludedNamespaces(final @NonNull RootStatementContext<?, ?, ?> root,
            final @NonNull ResolvedSourceInfo resolvedSource) {
    ...
    private static void fillImportedNamespaces(final @NonNull RootStatementContext<?, ?, ?> root,
            final @NonNull ResolvedSourceInfo resolvedSource) {
        for (var anImport : resolvedSource.imports()) {
            final String prefix = anImport.prefix().getLocalName();
        ...
        //TODO: consider adding the self-referencing IMPORT_PREFIX_TO_QNAME_MODULE and IMPORT_PREFIX_TO_MODULECTX
        // here so that Submodules can have it as well. This requires change in the ResolvedSourceInfo, where
        // prefix of a Submodule is taken from its parent module.
    }

    private static void fillSubmoduleNamespaces(final @NonNull RootStatementContext<?, ?, ?> root,
            final @NonNull ResolvedSourceInfo resolvedSource) {
        ...
        root.addToNamespace(ParserNamespaces.BELONGSTO_PREFIX_TO_QNAME_MODULE, belongsTo.prefix().getLocalName(),
            belongsTo.parentModuleQname());
        root.addToNs(ParserNamespaces.BELONGSTO_PREFIX_TO_MODULECTX, belongsTo.prefix().getLocalName(),
            belongsTo.parentRoot());
        root.addToNs(ParserNamespaces.BELONGSTO_PREFIX_TO_MODULE_NAME, belongsTo.prefix().getLocalName(),
            belongsTo.sourceId().name());
    }

    private static void fillModuleNamespaces(final @NonNull RootStatementContext<?, ?, ?> root,
            final @NonNull ResolvedSourceInfo resolvedSource) {
        ...
        // include module itself among imported namespaces - substatements of a module need to be able to reference the
        // module by its prefix as it was any other imported module.
        root.addToNs(ParserNamespaces.IMPORT_PREFIX_TO_QNAME_MODULE, resolvedSource.prefix().getLocalName(),
            resolvedSource.qnameModule());
        root.addToNs(ParserNamespaces.IMPORT_PREFIX_TO_MODULECTX, resolvedSource.prefix().getLocalName(), root);
    }
}
