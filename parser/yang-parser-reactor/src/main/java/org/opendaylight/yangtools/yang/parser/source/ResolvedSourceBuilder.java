...
abstract sealed class ResolvedSourceBuilder<R extends SourceInfoRef> implements Mutable {
    ...
    final void resolveImport(final ForModule parentModule, final Import dependency, final ForModule target)
        ...
        final var path = findImportPath(requireNonNull(target), requireNonNull(parentModule));
        if (path != null) {
            final var sourceId = sourceId();
            final var depRef = dependency.sourceRef();
            throw new SomeModifiersUnresolvedException(ModelProcessingPhase.SOURCE_LINKAGE, sourceId,
     ...
     *         is not imported
     */
    private static @Nullable ArrayList<ResolvedSourceBuilder<?>> findImportPath(
        ...
        while (impIt.hasNext()) {
            ...
            final var path = findImportPath(target, module);
            if (path != null) {
                path.add(source);
                return path;
            }
        ...
        while (incIt.hasNext()) {
            final var path = findImportPath(incIt.next(), module);
            if (path != null) {
                path.add(source);
                return path;
            }
