...
abstract sealed class ResolvedSourceBuilder<R extends SourceInfoRef> implements Mutable {
    ...
    final ResolvedSourceInfo build() {
        ...
        final var result = buildProduct(
            imports.buildResolved((requirement, infoRef) ->
                new ResolvedImport(requirement, infoRef.ref(), infoRef.info().moduleName().getModule())),
            includes.buildResolved((requirement, infoRef) -> new ResolvedInclude(requirement, infoRef.ref())));
        product = result;
