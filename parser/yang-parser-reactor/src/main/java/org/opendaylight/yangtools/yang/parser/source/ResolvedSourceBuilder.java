...
abstract sealed class ResolvedSourceBuilder<R extends SourceInfoRef> implements Mutable {
    ...
    static final class ForModule extends ResolvedSourceBuilder<SourceInfoRef.OfModule> {
        ...
        /**
         * Check that specified module is not imported, directly or indirectly, by this module.
         *
         * @param module the module
         * @return the reverse sequence of sources through which the specified module is imported, or {@code null} when
         *         it is not imported
         */
        private @Nullable List<ResolvedSourceBuilder<?>> importPathOf(final ForModule module) {
            // detect attempts to include self and defer to recursion
            return equals(module) ? List.of(this) : recursiveImportPathOf(this, module);
        }
    ...
    final void resolveImport(final ForModule parentModule, final Import dependency, final ForModule target)
        ...
        final var path = target.importPathOf(requireNonNull(parentModule));
        if (path != null) {
            final var sourceId = sourceId();
            final var depRef = dependency.sourceRef();
            throw new SomeModifiersUnresolvedException(ModelProcessingPhase.SOURCE_LINKAGE, sourceId,
     ...
     *         is not imported
     */
    private static @Nullable ArrayList<@NonNull ResolvedSourceBuilder<?>> recursiveImportPathOf(
        ...
        while (impIt.hasNext()) {
            ...
            final var path = recursiveImportPathOf(target, module);
            if (path != null) {
                path.add(source);
                return path;
            }
        ...
        while (incIt.hasNext()) {
            final var path = recursiveImportPathOf(incIt.next(), module);
            if (path != null) {
                path.add(source);
                return path;
            }
