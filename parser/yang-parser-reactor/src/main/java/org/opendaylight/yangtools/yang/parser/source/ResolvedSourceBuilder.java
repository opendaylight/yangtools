...
abstract sealed class ResolvedSourceBuilder<R extends SourceInfoRef> implements Mutable {
    ...
    private @Nullable StatementSourceReference includeRefOf(final @NonNull HashSet<ResolvedSourceBuilder<?>> visited,
            final @NonNull Unqualified name) {
        // search all dependencies
        for (var dependency : sourceInfo().includes()) {
            if (dependency.revision() == null && name.equals(dependency.name())) {
                final var depRef = dependency.sourceRef();
                return depRef != null ? depRef : sourceId().toReference();
            }
        }

        // recursively search through all included sources
        final var it = includes.present();
        while (it.hasNext()) {
            final ResolvedSourceBuilder<?> source = it.next();
            if (visited.add(source)) {
                final var ret = source.includeRefOf(visited, name);
                if (ret != null) {
                    return ret;
                }
            }
