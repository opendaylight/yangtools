...
abstract sealed class ResolvedSourceBuilder<R extends SourceInfoRef> implements Mutable {
    ...
    private abstract static sealed class Dependencies<D extends SourceDependency, B extends ResolvedSourceBuilder<?>> {
         ...
         */
        static final <D extends SourceDependency, B extends ResolvedSourceBuilder<?>>
                @NonNull Dependencies<D, B> of(final @NonNull Set<@NonNull D> dependencies) {
         ...
         */
        @NonNullByDefault
        abstract <R> List<@NonNull R> buildResolved(BiFunction<D, B, R> function);
    ...
    private static final class NoDependencies<D extends SourceDependency, B extends ResolvedSourceBuilder<?>>
        ...
        private static final @NonNull NoDependencies<?, ?> INSTANCE = new NoDependencies<>();
        ...
        static <D extends SourceDependency, B extends ResolvedSourceBuilder<?>> @NonNull NoDependencies<D, B> of() {
            return (NoDependencies<D, B>) INSTANCE;
        }

        @Override
        Set<@NonNull D> missing() {
            return Set.of();
        }
        ...
        @Override
        <R> List<@NonNull R> buildResolved(final BiFunction<D, B, R> function) {
            // null check to keep behaviour parity with SomeDependencies: there are no unresolved dependencies, so
            // function would be dereferenced at least once
    ...
    private static final class SomeDependencies<D extends SourceDependency, B extends ResolvedSourceBuilder<?>>
         ...
         * providing combined a combined capability to detect
         * <ol>
         ...
         * </ol>
         */
        private final @NonNull LinkedHashMap<@NonNull D, @Nullable B> map;
        /**
         * An unmodifiable view on what dependencies have not been satisfied. This is a materialized view
         * of {@code map.keySet()} filtering our any entries which have a {@code null} value.
         *
         ...
         */
        private final @NonNull Set<@NonNull D> missing;

        /**
         * Default constructor. Should only be called from {@link #Dependenciesof(Set)}.
         *
         * @param dependencies the set of dependencies
         */
        SomeDependencies(final @NonNull Set<@NonNull D> dependencies) {
            map = LinkedHashMap.newLinkedHashMap(dependencies.size());
            for (var dependency : dependencies) {
                map.put(requireNonNull(dependency), null);
            }
            missing = Collections.unmodifiableSet(Maps.filterValues(map, Objects::nonNull).keySet());
        }
        ...
        @Override
        void doResolveMissing(final D dependency, final B resolved) {
            if (!map.replace(dependency, null, resolved)) {
                ...
                throw prev == null
                    ...
                    : new VerifyException(
                        "Attempted to override import " + dependency + " from " + prev + " to " + resolved);
            }
        }
    ...
    /**
     * A {@link Dependencies} implementation indicated resolution has been completed and consumed. The only
    ...
    private static final class TerminalDependencies<D extends SourceDependency, B extends ResolvedSourceBuilder<?>>
            extends Dependencies<D, B> {
        private static final @NonNull TerminalDependencies<?, ?> INSTANCE = new TerminalDependencies<>();
        ...
        @SuppressWarnings("unchecked")
        static <D extends SourceDependency, B extends ResolvedSourceBuilder<?>>
                @NonNull TerminalDependencies<D, B> of() {
        ...
        private static @NonNull UnsupportedOperationException uoe() {
    ...
    private @NonNull Dependencies<Import, ResolvedSourceBuilder.ForModule> imports;
    private @NonNull Dependencies<Include, ResolvedSourceBuilder.ForSubmodule> includes;
    ...
    private ResolvedSourceBuilder(final R infoRef) {
        ...
        imports = SomeDependencies.of(info.imports());
        includes = SomeDependencies.of(info.includes());
    }
     ...
     */
    final @NonNullByDefault Set<Include> missingIncludes() {
