...
abstract sealed class ResolvedSourceBuilder<R extends SourceInfoRef> implements Mutable {
     ...
     * @param builder {@link ResolvedSourceBuilder} of the imported module.
     */
    @Deprecated(forRemoval = true)
    @NonNullByDefault
    final void resolveImport(final Import dependency, final ResolvedSourceBuilder<?> builder) {
        if (!(builder instanceof ForModule module)) {
            throw new VerifyException("Attempted to resolve import " + dependency + " with non-module " + builder);
        }
        resolveImport(dependency, module);
    }
     ...
     *
     * @param dependency the {@link Import} being satisfied
     * @param builder {@link ForModule} of the imported module.
     */
    @NonNullByDefault
    final void resolveImport(final Import dependency, final ForModule builder) {
        ensureBuilderOpened();
        imports.resolveMissing(dependency, builder.infoRef());
    }
