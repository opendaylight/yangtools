...
final class InferredStatementContext<A, D extends DeclaredStatement<A>, E extends EffectiveStatement<A, D>>
    ...
    private @NonNull E tryToReusePrototype(final @NonNull StatementFactory<A, D, E> factory) {
        ...
        final var declared = adoptSubstatements(declCopy.stream());
        final var effective = adoptSubstatements(effCopy.stream());
    ...
    private ImmutableList<ReactorStmtCtx<?, ?, ?>> adoptSubstatements(final Stream<EffectiveCopy> stream) {
        return stream.map(copy -> copy.toChildContext(this)).collect(ImmutableList.toImmutableList());
    }
