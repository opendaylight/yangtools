...
abstract class StatementContextBase<A, D extends DeclaredStatement<A>, E extends EffectiveStatement<A, D>>
    ...
    final <K, V> void onNamespaceItemAddedAction(final ParserNamespace<K, V> namespace,
        ...
        if (entry != null) {
            ...
            waitForPhase(entry.getValue(), namespace, access, phase, criterion, listener);
            return;
        }
        ...
        access.addListener((key, value) -> {
            if (criterion.match(key)) {
                ...
                waitForPhase(value, namespace, access, phase, criterion, listener);
                return true;
            }
            return false;
        });
    ...
    private <K, V> void waitForPhase(final Object value, final ParserNamespace<K, V> namespace,
            final NamespaceAccess<K, V> access, final ModelProcessingPhase phase,
