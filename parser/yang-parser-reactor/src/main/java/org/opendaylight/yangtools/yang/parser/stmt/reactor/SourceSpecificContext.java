...
final class SourceSpecificContext implements NamespaceStorage, Mutable {
    ...
    void loadRootStatement() {
        ...
        this.source.writeRoot(rootWriter, stmtDef(), getRootVersion());
        if (!root.yangVersion().equals(getSourceInfo().yangVersion())) {
            root.setRootVersionImpl(getSourceInfo().yangVersion());
        }
    }
