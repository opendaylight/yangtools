...
final class SourceSpecificContext implements NamespaceStorage, Mutable {
    ...
    /**
     * Creates solely the RootStatementContext since it's needed for source linkage resolution. The Writer is stored as
     * well, so it can be used to create the substatements as well at a later stage.
     */
    void loadRootStatement() {
        final YangVersion rootVersion = getRootVersion();
        rootWriter = new StatementContextWriter(this, inProgressPhase);
        source.writeRoot(rootWriter, stmtDef(), rootVersion);
    ...
    void loadStatements() {
        ...
        switch (inProgressPhase) {
            case STATEMENT_DEFINITION:
                ...
                // no need to keep the Writer anymore
                rootWriter = null;
                break;
