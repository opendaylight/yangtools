...
final class SourceSpecificContext implements NamespaceStorage, Mutable {
    ...
    Optional<StatementSourceException> failModifiers(final ModelProcessingPhase phase) {
        final var exceptions = new ArrayList<StatementSourceException>();
        for (var modifier : modifiers.get(phase)) {
            try {
        ...
        return switch (exceptions.size()) {
            ...
            default -> {
                final var ex = new InferenceException("Yang model processing phase " + phase + " failed", root,
                    exceptions.get(0));
