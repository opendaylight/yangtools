...
record YangToSourcesState(
        ...
        @NonNull ImmutableMap<String, FileState> configurations,
    ...
    static @NonNull YangToSourcesState readFrom(final DataInput in) throws IOException {
        final ImmutableMap<String, FileState> inputFiles = readStates(in, WITH_PREFIX);
        final ImmutableMap<String, FileState> configurations = readStates(in, WITHOUT_PREFIX);
        final ImmutableMap<String, FileState> outputFiles = readStates(in, WITH_PREFIX);
    ...
    private static void writeStates(final DataOutput out, final Collection<FileState> items,
    ...
    private static ImmutableMap<String, FileState> readStates(final DataInput in, final boolean withPrefix)
            throws IOException {
        ...
        for (int i = 0; i < size; ++i) {
            outputFiles.add(new FileState(prefix + in.readUTF(), WritableObjects.readLong(in), in.readInt()));
        }
        return Maps.uniqueIndex(outputFiles, FileState::path);
    }
