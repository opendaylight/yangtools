...
final class BuildInfo implements Immutable, WritableObject {
    ...
    static BuildInfo of(final Collection<File> inputFiles, final Set<File> outputFiles) {
        ...
        }), InputFile::fileName), outputFiles);
    }
    ...
    static BuildInfo readFrom(final DataInput in) throws IOException {
        ...
        if (!InputFile.HASH_FUNCTION_NAME.equals(hashName)) {
        ...
        for (int i = 0; i < inputSize; ++i) {
            final InputFile file = InputFile.readFrom(in);
            inputBuilder.put(file.fileName(), file);
        }
    ...
    public void writeTo(final DataOutput out) throws IOException {
        out.writeUTF(HEADER);
        out.writeUTF(InputFile.HASH_FUNCTION_NAME);
