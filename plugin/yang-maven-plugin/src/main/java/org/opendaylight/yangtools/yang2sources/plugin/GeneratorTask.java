...
abstract class GeneratorTask<T extends GeneratorTaskFactory> extends ParserConfigAware {
    ...
    final List<FileState> execute(final BuildContext buildContext) throws FileGeneratorException, IOException {
        return execute(factory, context, buildContext);
    }

    abstract List<FileState> execute(T factory, ContextHolder modelContext, BuildContext buildContext)
