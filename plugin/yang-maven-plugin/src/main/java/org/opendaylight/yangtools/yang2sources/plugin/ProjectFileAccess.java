...
final class ProjectFileAccess {
    ...
    @NonNull Path persistentPath(final GeneratedFileType fileType) throws FileGeneratorException {
        if (GeneratedFileType.SOURCE.equals(fileType)) {
            final var existing = sourceDir;
            if (existing != null) {
                return existing;
            }

            final var created = Path.of(project.getBuild().getSourceDirectory());
            sourceDir = created;
            return created;
        }
        if (GeneratedFileType.RESOURCE.equals(fileType)) {
            final var existing = resourceDir;
            if (existing != null) {
                return existing;
            }

            final var created = Path.of(project.getBuild().getSourceDirectory()).resolveSibling("resources");
            resourceDir = created;
            return created;
        }
    ...
    @NonNull Path transientPath(final GeneratedFileType fileType) throws FileGeneratorException {
        if (GeneratedFileType.SOURCE.equals(fileType)) {
            final var existing = buildSourceDir;
            if (existing != null) {
                return existing;
            }

            final var created = buildDirectoryFor("generated-sources");
            buildSourceDir = created;
            return created;
        }
        if (GeneratedFileType.RESOURCE.equals(fileType)) {
            final var existing = buildResourceDir;
            if (existing != null) {
                return existing;
            }

            final var created =  buildDirectoryFor("generated-resources");
            buildResourceDir = created;
            return created;
        }
