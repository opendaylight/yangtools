...
final class FileGeneratorTask extends GeneratorTask<FileGeneratorTaskFactory> {
    ...
    private File persistentDirectory(final GeneratedFileType fileType) throws FileGeneratorException {
        ...
        } else if (GeneratedFileType.TEST_SOURCE.equals(fileType)) {
            ret = new File(project.getBuild().getTestSourceDirectory());
        ...
        } else if (GeneratedFileType.TEST_RESOURCE.equals(fileType)) {
            ret = new File(new File(project.getBuild().getTestSourceDirectory()).getParentFile(), "resources");
        } else {
    ...
    private File transientDirectory(final GeneratedFileType fileType) throws FileGeneratorException {
        ...
        if (GeneratedFileType.SOURCE.equals(fileType)) {
            ...
            ret = transientDirectory("generated-test-sources");
            project.addTestCompileSourceRoot(ret.toString());
        } else if (GeneratedFileType.TEST_RESOURCE.equals(fileType)) {
            ret = transientDirectory("generated-test-resources");
            project.addTestResource(createResouce(ret));
        } else {
            throw new FileGeneratorException("Unknown generated file type " + fileType);
        }
        return ret;
