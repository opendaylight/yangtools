...
class YT786Test {
    ...
    void parseNotification(final String testDesc, final Absolute path, final String xml, final NormalizedNode expected)
            throws Exception {

        final var reader = UntrustedXML.createXMLStreamReader(new StringReader(xml));
        final var result = new NormalizationResultHolder();
        final var streamWriter = ImmutableNormalizedNodeStreamWriter.from(result);
        final var xmlParser = XmlParserStream.create(streamWriter, inferenceOf(path));
    ...
    void writeNotification(final String testDesc, final Absolute path, final String expected,
        ...
        final var nodeStreamWriter = XMLStreamNormalizedNodeStreamWriter
            .create(xmlStreamWriter, inferenceOf(path));
    ...
    private static Stream<Arguments> testArgs() {
        // test descriptor, path, xml, normalized node
        return Stream.of(
            Arguments.of(
                "root level",
