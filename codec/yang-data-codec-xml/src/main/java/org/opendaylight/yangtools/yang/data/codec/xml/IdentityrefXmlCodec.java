...
final class IdentityrefXmlCodec implements XmlCodec<QName> {
    ...
    IdentityrefXmlCodec(final EffectiveModelContext context, final ModelContextPrefixes prefixes,
    ...
    public void writeValue(final XMLStreamWriter ctx, final QName value) throws XMLStreamException {
        final var assigner = new RandomPrefix(prefixes, ctx.getNamespaceContext());
        final var str = QNameCodecUtil.encodeQName(value, uri -> assigner.encodePrefix(uri.getNamespace()));

        for (var e : assigner.emittedPrefixes()) {
