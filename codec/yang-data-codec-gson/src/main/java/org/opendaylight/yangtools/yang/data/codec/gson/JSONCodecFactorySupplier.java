...
public enum JSONCodecFactorySupplier {
        ...
        private static int codecsForChildren(final JSONCodecFactory lazy, final SchemaInferenceStack stack,
            ...
            for (var child : parent.dataTreeNodes()) {
                switch (child) {
                    ...
                    case TypedDataSchemaNode typed -> {
     ...
     * still cached, but only for 30 seconds after their last use, and through soft references, so they are also
     * released when the JVM runs short of memory.
     *
     * @param modelContext EffectiveModelContext instance
     * @return A non-sharable {@link JSONCodecFactory}
