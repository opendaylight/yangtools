...
class YT1899Test {
    ...
    void multiEntryListWithChoiceChild() throws Exception {
        ...
        // everything.
        assertEquals(expected, parse(inputJson));
        assertEquals(expected, parse(inputJson));
    }
    ...
    private static NormalizedNode parse(final String inputJson) throws IOException {
        final var holder = new NormalizationResultHolder();
        try (var writer = ImmutableNormalizedNodeStreamWriter.from(holder)) {
            try (var parser = JsonParserStream.create(writer,
