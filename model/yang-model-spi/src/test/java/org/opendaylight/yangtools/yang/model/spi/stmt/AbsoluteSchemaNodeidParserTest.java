...
class AbsoluteSchemaNodeidParserTest extends AbstractNamespaceBindingTest<Absolute> {
    ...
    void emptyParseArgumentThrows() {
        ...
        assertEquals("absolute-schema-nodeid cannot be empty", ex.getMessage());
    ...
    void notSlashParseArgumentThrows() {
        ...
        assertEquals("'a' is not '/' as required by absolute-schema-nodeid", ex.getMessage());
    ...
    @Test
    void toStringReportsNamespaceBinding() {
        assertEquals("AbsoluteSchemaNodeidParser{namespaceBinding=namespaceBinding}", parser.toString());
    }
}
