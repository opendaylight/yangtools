...
public final class SchemaNodeIdentifierParser {
    ...
    private static QName parseIdentifier(final QNameModule module, final String str, final int errorOffset)
            throws ParseException {
        final var identifier = UnresolvedQName.tryLocalName(str);
        if (identifier == null) {
            throw new ParseException("'" + str + "' is not a valid identifier", errorOffset);
        }
     ...
     * @param str the string
     * @return a {@link QName}
     * @throws ParseException if the string cannot be parsed or its prefix cannot be bound
     */
    public QName parseNodeIdentifier(final String str) throws ParseException {
        return parseNodeIdentifier(str, 0);
    }

    private QName parseNodeIdentifier(final String str, final int errorOffset) throws ParseException {
        final int colon = str.indexOf(':');
        if (colon == -1) {
            return parseIdentifier(resolver.currentModule(), str, errorOffset);
        }

        final var prefixStr = str.substring(0, colon);
        final var prefix = UnresolvedQName.tryLocalName(prefixStr);
        if (prefix == null) {
            throw new ParseException("'" + prefixStr + "' is not a valid prefix", errorOffset);
        }

        final var module = resolver.lookupModule(prefix);
        if (module == null) {
            throw new ParseException("Prefix '" + prefixStr + "' cannot be resolved", errorOffset);
        }

        final var colonPlusOne = colon + 1;
        return parseIdentifier(module, str.substring(colonPlusOne), errorOffset + colonPlusOne);
    }
     ...
     * @throws ParseException if the string cannot be parsed or any of its prefixes cannot be bound
     */
    public Absolute parseAbsoluteSchemaNodeid(final String str) throws ParseException {
     ...
     * @throws ParseException if the string cannot be parsed or any of its prefixes cannot be bound
     */
    public Descendant parseDescendantSchemaNodeid(final String str) throws ParseException {
        return Descendant.of(parseSchemaNodeId(str, 0));
    }

    private ImmutableList<QName> parseSchemaNodeId(final String str, final int beginIndex) throws ParseException {
