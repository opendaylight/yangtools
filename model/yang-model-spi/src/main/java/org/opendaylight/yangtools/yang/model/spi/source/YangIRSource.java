...
import org.opendaylight.yangtools.yang.model.spi.meta.TextStatementReference;

/**
 * A {@link YangSourceRepresentation} based on {@link IRStatement}.
 */
@NonNullByDefault
public record YangIRSource(SourceIdentifier sourceId, IRStatement rootStatement, @Nullable String symbolicName)
        implements YangSourceRepresentation {
    public YangIRSource(final SourceIdentifier sourceId, final IRStatement rootStatement,
    ...
    @Beta
    public static StatementSourceReference refOf(final SourceIdentifier source, final IRStatement stmt) {
        return TextStatementReference.atPosition(source.name().getLocalName(), stmt.startLine(),
            stmt.startColumn() + 1);
    }
}
