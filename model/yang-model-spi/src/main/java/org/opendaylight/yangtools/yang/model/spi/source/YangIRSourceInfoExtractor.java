...
abstract sealed class YangIRSourceInfoExtractor implements SourceInfo.Extractor {
            ...
            for (var stmt : root.statements()) {
                if (isStatement(stmt, NAMESPACE)) {
                    ...
                    try {
                        ...
                        throw new ExtractorInvalidArgumentException(refOf(stmt), NAMESPACE, e);
                    }
                }
            }
        ...
        for (var stmt : root.statements()) {
            if (isStatement(stmt, YANG_VERSION)) {
                ...
                try {
                    ...
                    throw new ExtractorInvalidArgumentException(refOf(stmt), YANG_VERSION, e);
                }
            }
        }
    ...
    final @NonNull Revision revisionArgument(final IRStatement stmt) throws ExtractorException {
        final var arg = stringArgument(stmt);
        try {
            return Revision.of(arg);
        } catch (IllegalArgumentException e) {
            throw new ExtractorInvalidArgumentException(refOf(stmt), stmt.keyword().asStringDeclaration(), e);
        }
    }
