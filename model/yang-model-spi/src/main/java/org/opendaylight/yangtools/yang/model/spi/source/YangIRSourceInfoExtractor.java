...
abstract sealed class YangIRSourceInfoExtractor implements SourceInfo.Extractor {
            ...
            for (var stmt : root.statements()) {
                if (isStatement(stmt, NAMESPACE)) {
                    ...
                    try {
                        ...
                        throw new ExtractorMalformedArgumentException(refOf(stmt), NAMESPACE, e);
                    }
                }
            }
            throw new ExtractorMissingStatementException(refOf(root), NAMESPACE);
        }
    }
    ...
    static final class ForSubmodule extends YangIRSourceInfoExtractor {
        ...
        private @NonNull BelongsTo extractBelongsTo() throws ExtractorException {
            ...
            throw new ExtractorMissingStatementException(refOf(root), BELONGS_TO);
        }
    }
    ...
    final void fillBuilder(final SourceInfo.Builder<?, ?> builder) throws ExtractorException {
        ...
        for (var stmt : root.statements()) {
            if (isStatement(stmt, IMPORT)) {
                builder.addImport(new Import(Unqualified.of(stringArgument(stmt)),
                    extractPrefix(stmt), extractRevisionDate(stmt)));
            } else if (isStatement(stmt, INCLUDE)) {
                builder.addInclude(new Include(Unqualified.of(stringArgument(stmt)),
                    extractRevisionDate(stmt)));
            } else if (isStatement(stmt, REVISION)) {
                builder.addRevision(Revision.of(stringArgument(stmt)));
            }
        }
    }
        ...
        for (var stmt : root.statements()) {
            if (isStatement(stmt, YANG_VERSION)) {
                ...
                try {
                    ...
                    throw new ExtractorMalformedArgumentException(refOf(stmt), YANG_VERSION, e);
                }
            }
        }
    ...
    final @NonNull Unqualified extractPrefix(final IRStatement stmt) throws ExtractorException {
        for (var subStmt : stmt.statements()) {
            if (isStatement(subStmt, PREFIX)) {
                return unqualifiedArgument(stmt);
            }
        }
        throw new ExtractorMissingStatementException(refOf(stmt), PREFIX);
    }
    ...
    private @Nullable Revision extractRevisionDate(final IRStatement stmt) throws ExtractorException {
        for (var subStmt : stmt.statements()) {
            if (isStatement(subStmt, REVISION_DATE)) {
                return Revision.of(stringArgument(subStmt));
            }
        }
        return null;
    }
    ...
    final @NonNull String stringArgument(final IRStatement stmt) throws ExtractorException {
    ...
    final @NonNull Unqualified unqualifiedArgument(final IRStatement stmt) throws ExtractorException {
        final var arg = stringArgument(stmt);
        try {
