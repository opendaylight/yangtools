...
abstract sealed class YangIRSourceInfoExtractor implements SourceInfo.Extractor {
    ...
    @NonNullByDefault
    private @Nullable Revision extractRevisionDate(final IRStatement parent) throws ExtractorException {
        final var stmt = firstStatement(parent, REVISION_DATE);
        return stmt == null ? null : revisionArgument(stmt);
    }

    @NonNullByDefault
    private static @Nullable IRStatement firstStatement(final IRStatement parent, final String keyword) {
