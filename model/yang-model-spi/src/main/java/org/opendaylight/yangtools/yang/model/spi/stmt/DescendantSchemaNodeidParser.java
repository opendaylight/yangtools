...
public final class DescendantSchemaNodeidParser extends AbstractArgumentParser<Descendant> {
    ...
    private Descendant parseDescendantSchemaNodeid(final String str)
        ...
        return Descendant.of(parseNodeIdentifiers(str));
    }

    // Note: assumes str is non-empty, str is being cons
    private ImmutableList<QName> parseNodeIdentifiers(final String str)
            throws ArgumentSyntaxException, ArgumentBindingException {
        ...
        while (true) {
            final var slash = str.indexOf('/', beginIndex);
            if (slash == -1) {
                break;
            }
            ...
            builder.add(nodeIdentifierParser.parseNodeIdentifier(str, beginIndex, slash));
            beginIndex = slash + 1;
        }

        // final node-identifier
        builder.add(nodeIdentifierParser.parseNodeIdentifier(str, beginIndex, str.length()));
        return builder.build();
    }

    @Override
    ToStringHelper addToStringAttributes(final ToStringHelper helper) {
        return helper.add("namespaceBinding", nodeIdentifierParser.identifierParser.namespaceBinding);
    }
}
