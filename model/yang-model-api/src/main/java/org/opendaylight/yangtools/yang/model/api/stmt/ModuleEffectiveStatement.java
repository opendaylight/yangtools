...
public non-sealed interface ModuleEffectiveStatement
     ...
     * </pre>
     */
    default @NonNull Map<QName, ExtensionEffectiveStatement> extensionNamespace() {
        return streamEffectiveSubstatements(ExtensionEffectiveStatement.class)
            .collect(Collectors.toUnmodifiableMap(ExtensionEffectiveStatement::argument, Function.identity()));
    }
    ...
    // FIXME: qname is implied to implied to have the same namespace as localQNameModule(), hence this should be driven
    //        through getLocalName()
    default @NonNull Map<QName, FeatureEffectiveStatement> featureNamespace() {
        return indexEffectiveSubstatements(FeatureEffectiveStatement.class);
    }
    ...
    // FIXME: qname is implied to implied to have the same namespace as localQNameModule(), hence this should be driven
    //        through getLocalName()
    default @NonNull Map<QName, IdentityEffectiveStatement> identityNamespace() {
        return indexEffectiveSubstatements(IdentityEffectiveStatement.class);
    }
