...
public non-sealed interface EffectiveStatement<A, D extends DeclaredStatement<A>> extends ModelStatement<A> {
    ...
    default <X, Z extends EffectiveStatement<X, ?>>
            @NonNull Map<X, Z> indexEffectiveSubstatements(final @NonNull Class<Z> stmt) {
        return streamEffectiveSubstatements(stmt)
            .collect(Collectors.toUnmodifiableMap(EffectiveStatement::argument, Function.identity()));
    }
}
