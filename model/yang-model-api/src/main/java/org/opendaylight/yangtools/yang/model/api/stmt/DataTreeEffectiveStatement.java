...
public sealed interface DataTreeEffectiveStatement<D extends DeclaredStatement<QName>>
    ...
    /**
     * Interface implemented by all {@link SchemaTreeAwareEffectiveStatement}s which can contain a {@code data tree}
     *  child.
     *
    ...
    interface IndexedIn<A, D extends DeclaredStatement<A>> extends SchemaTreeEffectiveStatement.IndexedIn<A, D> {
         ...
         * {@link SchemaTreeAwareEffectiveStatement#schemaTreeNodes()} in that all data nodes are also schema nodes.
         * The structure of the tree is different, though, as {@code choice} and {@code case} statements are glossed
         * over and they do not contribute to the tree hierarchy -- only their children do.
         *
