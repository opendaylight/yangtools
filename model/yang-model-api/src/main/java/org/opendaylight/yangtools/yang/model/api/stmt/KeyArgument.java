...
public sealed interface KeyArgument extends SizedIterable<QName> permits KeyArgument.OfOne, KeyArgument.OfMore {
     ...
     */
    sealed interface OfOne extends KeyArgument permits KeyArgument1 {
        /**
     ...
     */
    sealed interface OfMore extends KeyArgument permits KeyArgument2, KeyArgumentN {
        // just a marker
    }
    ...
    static KeyArgument of(final List<QName> nodeIdentifiers) {
        return switch (nodeIdentifiers.size()) {
            ...
            default -> of(ImmutableSet.copyOf(nodeIdentifiers));
        };
    }

    private static KeyArgument of(final ImmutableSet<QName> nodeIdentifiers) {
        return switch (nodeIdentifiers.size()) {
            case 1 -> of(nodeIdentifiers.iterator().next());
            case 2 -> new KeyArgument2(List.copyOf(nodeIdentifiers));
            default -> new KeyArgumentN(nodeIdentifiers);
        };
    }
