...
public final class DecimalTypeBuilder extends RangeRestrictedTypeBuilder<DecimalTypeDefinition, Decimal64> {
    ...
    private BaseDecimalType ensureResolvedScale(final BaseDecimalType decimalType, final int scale) {
        ...
        final var constraint = decimalType.getRangeConstraint().get();

        // Check if we need to resolve anything at all
        for (var range : constraint.getAllowedRanges().asRanges()) {
    ...
    private BaseDecimalType resolveScale(final BaseDecimalType unresolved, final int scale) {
        final var unresolvedRanges = unresolved.getRangeConstraint().get().getAllowedRanges().asRanges();
        final ImmutableRangeSet.Builder<Decimal64> builder = ImmutableRangeSet.builder();
        for (final var range : unresolvedRanges) {
        ...
        final var resolved = new ResolvedRangeConstraint<>(unresolved.getRangeConstraint().get(), scaledRangeSet);
