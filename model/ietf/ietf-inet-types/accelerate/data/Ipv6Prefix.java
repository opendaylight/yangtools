...
public class Ipv6Prefix
 implements Serializable {
    ...
    /**
     * Creates a Ipv6Prefix object from a byte sequence, the argument is copied before
     * Ipv6Prefix instantiation 
     * Callers which do not need to worry about the _binary_form safety should use 
     * Ipv6Prefix(_binary_form, netmask, false) instead
     * The prefix is canonicalized by default (this is cheap enough to be done
     * every time).
     *
     * @param _binary_form Ipv6Prefix represented as byte array - as seen on the network
     * @param _netmask netmask
     */

    public Ipv6Prefix(byte [] _binary_form, int _netmask) {
        this(_binary_form, _netmask, true);
    }

    /**
     * Creates an Ipv6Prefix from binary (routing protocol) representation
     *
     * @param _binary_form ipv6 prefix as byte array
     * @param _netmask netmask
     * @param clone_me if true - create clone of the byte array at instantiation
     */

    public Ipv6Prefix(byte [] _binary_form, int _netmask, boolean clone_me) {
        ...
        if (clone_me) {
            this._binary_form = Arrays.copyOf(_binary_form, INADDR6SZ);
    ...
    private InetAddress getInetForm() throws IllegalStateException {
        try {
            return InetAddress.getByAddress(this._binary_form);
        } catch (final Exception e) {
            throw new IllegalStateException(
                "Impossible return code from getByAddress: \n" + e.getCause()
            );
        }
    }
