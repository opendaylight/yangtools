...
public class Ipv4Prefix
 implements Serializable {
    ...
    static public void canonicalizeIpv4Prefix(byte [] _prefix, int mask) {
        ...
        int i;
        long ffff = 0xffffffff; 
        byte [] _mask = Ints.toByteArray((int) (ffff << (32 - mask)));
    ...
    /**
     * Creates a Ipv4Prefix object from a byte sequence, the argument is copied before
     * Ipv4Prefix instantiation 
     * Callers which do not need to worry about the _binary_form safety should use 
     * Ipv4Prefix(_binary_form, netmask, false) instead
     * The prefix is canonicalized by default (this is cheap enough to be done
     * every time).
     *
     * @param _binary_form Ipv4Prefix represented as byte array - as seen on the network
     ...
     *
     * @param _binary_form ipv4 prefix as byte array
     * @param _netmask netmask
     * @param clone_me if true - create clone of the byte array at instantiation
     */

    public Ipv4Prefix(byte [] _binary_form, int _netmask, boolean clone_me) {
        ...
        if (clone_me) {
            this._binary_form = Arrays.copyOf(_binary_form, INADDR4SZ);
