...
public class YangOrganizationTreeIterator extends AbstractIterator<@NonNull String> {
    ...
    private String composeLine(DeclaredStatement<?> stmt, final String currentIndent, final int longestSibling) {
    ...
    private void pushStatement(Iterator<? extends DeclaredStatement<?>> stmtIterator,
            @Nullable DeclaredStatement<?> stmt, IndentType indentType, @Nullable Integer longestChildName) {
        ...
        if (longestChildName == null) {
            if (stmt != null) {
                longestChildName = stmt.declaredSubstatements().stream()
                        .filter(decStmt -> decStmt instanceof DataDefinitionStatement)
