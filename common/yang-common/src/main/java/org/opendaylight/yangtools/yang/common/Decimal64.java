...
public class Decimal64 extends Number implements CanonicalValue<Decimal64> {
    ...
    public final String toCanonicalString() {
        ...
        int periodPosition = sb.length() - scale;

        // Calculating correct period position for numbers between -0.01 and 0.01.
        int positionOffset = 0;
        if (periodPosition < 0 && sb.charAt(0) == '-') {
            periodPosition--;
            positionOffset++;
        }

        // Adding '0' after period if value is between -1 and 1. For example .3 will become .03.
        while (periodPosition < 0) {
            sb.insert(positionOffset, '0');
            periodPosition++;
        }
        periodPosition += positionOffset;
        sb.insert(periodPosition, ".");

        // Adding '0' before period if value is between -1 and 1. For example .34 will become 0.34.
        if (periodPosition == 0 || sb.charAt(periodPosition - 1) == '-') {
            sb.insert(periodPosition, '0');
        }

        // Trim excess trailing zeroes
        int trimZeroes = sb.length();
        while (sb.charAt(trimZeroes - 1) == '0' && sb.charAt(trimZeroes - 2) == '0') {
            trimZeroes--;
        }
        sb.setLength(trimZeroes);
