...
/**
 * A {@link ListField.Readable} using {@link VarHandle#get(Object...)}.
 */
@NonNullByDefault
final record PlainGetListField<R, V>(Class<R> recv, Class<V> val, VarHandle vh) implements ListField.Readable<R, V> {
    PlainGetListField {
        requireNonNull(recv);
        requireNonNull(val);
        requireNonNull(vh);
