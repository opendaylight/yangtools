 ...
 * </ol>
 *
 * <p>An example of a suitable document is
 * {@snippet lang="xml" :
 * <foo>
 *   <!-- This is okay, even with this comment, which is markup -->
 *   <bar>baz</bar>
 *   <bar><xyzzy/></bar>
 * </foo>
 * }
 *
 * <p>An example of an unsuitable document is
 * {@snippet lang="xml" :
 * <foo>
 *   <!-- This is not okay: the order of 'baz' and 'xyzzy' events should not matter, yet our output will differ -->
 *   <bar>baz<xyzzy/></bar>
 *   <bar><xyzzy/>baz</bar>
 * </foo>
 * }
...
public final class IndentedXML implements Immutable {
    ...
    // Loading cache for non-default instances
    private static final LoadingCache<Integer, IndentedXML> CACHE = CacheBuilder.newBuilder().weakValues()
        .build(new CacheLoader<Integer, IndentedXML>() {
            @Override
            public IndentedXML load(final Integer key) {
                return new IndentedXML(key);
            }
        });
    ...
    /**
     * The number of spaces to indent by. Guaranteed to be more than 0. Enforcement is being done, untypically,
     * in {@link #of(int)}.
     */
    ...
    private IndentedXML(final int indentSize) {
        // Note: we assume we are only called to construct DEFAULT_INDENT and via of(int) and hence indentSize is valid
    ...
    public static IndentedXML of(final int indentSize) {
        return switch (indentSize) {
            ...
            default -> {
                if (indentSize < 1) {
                    throw new IllegalArgumentException("indentCount has to be positive");
                }
                yield CACHE.getUnchecked(indentSize);
            }
        };
    }
