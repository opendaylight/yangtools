...
public class QueuedNotificationManager<L,N> implements NotificationManager<L,N> {
        ...
        try {
            while( true ) {
                ...
                if( existingTask == null || !existingTask.addNotifications( notifications ) ) {
                    ...
                    // yet or some other concurrent thread beat us to the put. Either way we'll
                    // loop back up and try again.
    ...
    private class NotificationTask implements Runnable {
        ...
        boolean addNotifications( Collection<N> notifications ) throws InterruptedException {
            ...
            try {
                ...
                while( true ) {
                    ...
                    if( notification == null ) {
                        ...
                        // then #addingNotifications is in the process of offering to the queue so
                        // we'll loop back up and poll the queue again.

                        Uninterruptibles.sleepUninterruptibly(200, TimeUnit.MICROSECONDS);
                        if( queuingLock.tryLock() ) {
                            try {

                                // Check the queuedNotifications flag to see if #addNotifications
                                ...
                                // #addNotifications will fail and a new task will be created.
        ...
        private void notifyListener( N notification ) {
            ...
            try {
                ...
                LOG.error( name + ": Error notifying listener " +
