...
public class SpecialExecutors {
     ...
     * handles rejected tasks by performing a blocking put to the queue. Therefore if the queue is
     * full, the caller submitting the task will be blocked until space becomes available on the
     * queue. In this manner, tasks are never rejected.
     *
     * @param maximumPoolSize
     *            the maximum number of threads to allow in the pool. Threads will terminate after
     *            being idle for 15 seconds.
