...
public class AsyncNotifyingListeningExecutorService extends AbstractListeningExecutorService {
     ...
     */
    private <T> @NonNull AsyncNotifyingListenableFutureTask<T> newFutureTask(final Runnable task, final T result) {
    ...
    public <T> @NonNull ListenableFuture<T> submit(final Callable<T> task) {
        final @NonNull AsyncNotifyingListenableFutureTask<T> futureTask = AsyncNotifyingListenableFutureTask.create(
    ...
    public <T> @NonNull ListenableFuture<T> submit(final Runnable task, final T result) {
        final @NonNull AsyncNotifyingListenableFutureTask<T> futureTask = newFutureTask(task, result);
        delegate.execute(futureTask);
        return futureTask;
    }
