...
public class DurationStatsTracker {
    ...
    private final AtomicLong timeOfLongestDuration = new AtomicLong();
    private final AtomicLong shortestDuration = new AtomicLong(Long.MAX_VALUE);
    private final AtomicLong timeOfShortestDuration = new AtomicLong();
    ...
    public void addDuration(long duration) {
        ...
        if( duration > longest ) {
            if(longestDuration.compareAndSet( longest, duration )) {
                timeOfLongestDuration.set(System.currentTimeMillis());
            }
        }
        ...
        if( duration < shortest ) {
            if(shortestDuration.compareAndSet( shortest, duration )) {
                timeOfShortestDuration.set(System.currentTimeMillis());
            }
        }
    }
    ...
    public long getTimeOfLongestDuration() {
        return timeOfLongestDuration.get();
    }
    ...
    public long getTimeOfShortestDuration() {
        return timeOfShortestDuration.get();
    }
    ...
    public void reset() {
        ...
        timeOfLongestDuration.set(0);
        shortestDuration.set(Long.MAX_VALUE);
        timeOfShortestDuration.set(0);
