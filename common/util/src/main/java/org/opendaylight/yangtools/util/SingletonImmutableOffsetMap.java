...
public final class SingletonImmutableOffsetMap<K, V> implements Serializable, UnmodifiableMapPhase<K, V> {
    ...
    private SingletonImmutableOffsetMap(final K key, final V value) {
        this.keySet = (SingletonSet<K>) CACHE.getUnchecked(key);
        this.value = Preconditions.checkNotNull(value);
    }

    public static <K, V> SingletonImmutableOffsetMap<K, V> of(final K key, final V value) {
        return new SingletonImmutableOffsetMap<>(key, value);
    }

    public static <K, V> SingletonImmutableOffsetMap<K, V> copyOf(final Map<K, V> m) {
        Preconditions.checkArgument(m.size() == 1);

        final Entry<K, V> e = m.entrySet().iterator().next();
        return new SingletonImmutableOffsetMap<>(e.getKey(), e.getValue());
    }
    ...
    @Override
    public int hashCode() {
        return keySet.getElement().hashCode() ^ value.hashCode();
    }
