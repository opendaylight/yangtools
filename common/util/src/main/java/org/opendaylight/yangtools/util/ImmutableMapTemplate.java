 ...
 * used as a factory for instances via using {@link #instantiateTransformed(Map, Function)} or, more efficiently, using
 * {@link #instantiateWithValues(Object[])} where the argument array has values ordered corresponding to the key order
 * defined by {@link #iterator()}.
 *
 * @param <K> the type of keys maintained by this template
 * @param <V> the type of mapped values
 */
@Beta
public abstract class ImmutableMapTemplate<K, V> implements Iterable<K>, Immutable {
    private abstract static class AbstractMultiple<K, V> extends ImmutableMapTemplate<K, V> {
        ...
        @Override
        public final <T> @NonNull ImmutableOffsetMap<K, V> instantiateTransformed(final Map<K, T> fromMap,
            ...
            for (Entry<K, T> entry : fromMap.entrySet()) {
                ...
                objects[offset.intValue()] = transformValue(entry.getValue(), valueTransformer);
            }
        ...
        @Override
        @SafeVarargs
        public final @NonNull UnmodifiableMapPhase<K, V> instantiateWithValues(final V... values) {
    ...
    private abstract static class AbstractSingle<K, V> extends ImmutableMapTemplate<K, V> {
        ...
        @Override
        public final <T> @NonNull SharedSingletonMap<K, V> instantiateTransformed(final Map<K, T> fromMap,
                final Function<T, V> valueTransformer) {
            ...
            final V value = transformValue(entry.getValue(), valueTransformer);
     ...
     * of {@code keys}. {@link #iterator()} will return these keys in exactly the same order. The resulting map will
     ...
     * will have the iteration order matching the order returned by {@link #iterator()}.  The resulting map will
     ...
     * @param valueTransformer Transformation to apply to values
     ...
     */
    public abstract <T> @NonNull UnmodifiableMapPhase<K, V> instantiateTransformed(Map<K, T> fromMap,
     ...
     */
    @SuppressWarnings("unchecked")
    public abstract @NonNull UnmodifiableMapPhase<K, V> instantiateWithValues(V... values);
