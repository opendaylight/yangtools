...
public class AsyncNotifyingListenableFutureTask<V> extends FutureTask<V> implements ListenableFuture<V> {
    ...
    public void addListener( Runnable listener, Executor executor ) {
        // If a listenerExecutor was specified on construction, create a DelegatingRunnable.
        // If the specified executor runs tasks in the same thread as the caller (eg
        // MoreExecutors#sameThreadExecutor) and the caller submitting the listener is running on
        // the task completion thread (as detected via the ThreadLocal), then the DelegatingRunnable
        // will run the listener on the listenerExecutor.
