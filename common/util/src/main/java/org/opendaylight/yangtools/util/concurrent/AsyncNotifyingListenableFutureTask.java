...
public class AsyncNotifyingListenableFutureTask<V> extends FutureTask<V> implements ListenableFuture<V> {
     ...
     */
    private static final ThreadLocal<SettableBoolean> ON_TASK_COMPLETION_THREAD_TL = new ThreadLocal<>();
    ...
    @Override
    protected void done() {
        SettableBoolean b = ON_TASK_COMPLETION_THREAD_TL.get();
        if (b == null) {
            b = new SettableBoolean();
            ON_TASK_COMPLETION_THREAD_TL.set(b);
        }
        b.set();
