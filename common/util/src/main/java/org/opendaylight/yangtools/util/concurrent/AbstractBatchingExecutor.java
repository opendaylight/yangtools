...
abstract class AbstractBatchingExecutor<K, T> extends AbstractIdentifiable<String> {
    ...
    private final ConcurrentMap<K, WorkerTask> dispatcherTasks = new ConcurrentHashMap<>();
    ...
    final void submitTasks(final K key, final Iterable<T> tasks) {
        ...
        try {
            ...
            while (true) {
                WorkerTask task = dispatcherTasks.get(key);
                if (task == null) {
                    // No task found, try to insert a new one
                    final WorkerTask newTask = new WorkerTask(key, it);
    ...
    final class WorkerTask implements Runnable {
        ...
        WorkerTask(final @NonNull K key, final @NonNull Iterator<T> notifications) {
