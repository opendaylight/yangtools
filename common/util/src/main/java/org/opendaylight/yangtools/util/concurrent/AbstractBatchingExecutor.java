...
abstract class AbstractBatchingExecutor<K, T> extends AbstractIdentifiable<String> {
    ...
    final void submitTask(final K key, final T task) {
        submitTasks(key, Collections.singletonList(task));
    }

    // FIXME: YANGTOOLS-1016: allow explicit blocking control with return of unqueued tasks (or removal from input)
