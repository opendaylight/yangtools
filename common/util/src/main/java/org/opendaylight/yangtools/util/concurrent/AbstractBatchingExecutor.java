...
abstract class AbstractBatchingExecutor<K, T> extends AbstractIdentifiable<String> {
    ...
    private final ConcurrentMap<K, NotificationTask> dispatcherTasks = new ConcurrentHashMap<>();
    ...
    final void submitTasks(final K key, final Iterable<T> tasks) {
        ...
        try {
            ...
            while (true) {
                NotificationTask task = dispatcherTasks.get(key);
                if (task == null) {
            ...
            // telling us to quit.
            LOG.warn("{}: Interrupted trying to add to {} listener's queue", getIdentifier(), key);
        }

        LOG.trace("{}: submitNotifications done for listener {}", getIdentifier(), key);
    }

    final Stream<NotificationTask> streamTasks() {
        return dispatcherTasks.values().stream();
    }

    abstract void executeBatch(K key, ImmutableList<T> tasks) throws Exception;

    private void runTask(final K listener, final NotificationTask task) {
        LOG.debug("{}: Submitting NotificationTask for listener {}", getIdentifier(), listener);
     ...
     */
    final class NotificationTask implements Runnable {
        ...
        NotificationTask(final @NonNull K key, final @NonNull Iterator<T> notifications) {
        ...
        boolean submitNotifications(final @NonNull Iterator<T> notifications) throws InterruptedException {
                    ...
                    if (avail <= 0) {
                        if (canWait <= 0) {
                            LOG.warn("{}: Failed to offer notifications {} to the queue for listener {}. Exceeded"
                                ...
                                + "state (deadlock or endless loop). ", getIdentifier(),
                                ImmutableList.copyOf(notifications), key, MAX_NOTIFICATION_OFFER_MINUTES);
                            return true;
                        }
        ...
        private boolean waitForQueue() {
            ...
            while (queue.isEmpty()) {
                ...
                try {
                    ...
                    LOG.debug("{}: Interrupted trying to remove from {} listener's queue", getIdentifier(), key);
                    return false;
                }
            }
        ...
        public void run() {
            try {
                ...
                while (true) {
                    final @NonNull ImmutableList<T> notifications;
        ...
        private void invokeListener(final @NonNull ImmutableList<T> tasks) {
