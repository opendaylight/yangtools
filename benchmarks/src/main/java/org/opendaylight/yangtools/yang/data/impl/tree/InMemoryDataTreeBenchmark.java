...
public class InMemoryDataTreeBenchmark {
    ...
    private static YangInstanceIdentifier[] initOuterListPaths(final int outerListPathsCount) {
        return Streams.mapWithIndex(IntStream.range(0, outerListPathsCount),
            (i, index) -> YangInstanceIdentifier.builder(BenchmarkModel.OUTER_LIST_PATH)
            .nodeWithKey(BenchmarkModel.OUTER_LIST_QNAME, BenchmarkModel.ID_QNAME, i).build())
    ...
    private static NormalizedNode<?,?>[] initOuterListItems(final int outerListItemsCount, final MapNode innerList) {
        return Streams.mapWithIndex(IntStream.range(0, outerListItemsCount),
            (i, index) -> ImmutableNodes.mapEntryBuilder(BenchmarkModel.OUTER_LIST_QNAME, BenchmarkModel.ID_QNAME, i)
            .withChild(innerList).build()).collect(Collectors.toList()).toArray(new NormalizedNode[0]);
    }
    ...
    public void setup() throws DataValidationFailedException {
        ...
        modification.write(BenchmarkModel.TEST_PATH, provideOuterListNode());
        commit(modification);
    }
    ...
    private static DataContainerChild<?, ?> provideOuterListNode() {
        return ImmutableContainerNodeBuilder
            ...
            .withChild(EMPTY_OUTER_LIST).build();
    }
