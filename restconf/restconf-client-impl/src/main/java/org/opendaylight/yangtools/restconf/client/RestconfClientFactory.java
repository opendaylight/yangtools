...
public class RestconfClientFactory implements RestconfClientContextFactory {
    ...
    @Override
    public RestconfClientContext getRestconfClientContext(URL baseUrl, RuntimeGeneratedMappingServiceImpl mappingService) throws UnsupportedProtocolException {
        if (!baseUrl.getProtocol().equals("http")){
            throw new UnsupportedProtocolException("Unsupported protocol "+baseUrl.getProtocol());
        }
        return new RestconfClientImpl(baseUrl,mappingService);
    }
}
