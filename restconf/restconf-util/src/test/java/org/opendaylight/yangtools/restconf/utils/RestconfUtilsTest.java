...
public class RestconfUtilsTest {
	...
	public void testToDataObjectMappingWithNestedAugmentations() {
		//hack to get codec for augmentations
		this.mappingService.getCodecRegistry().getCodecForAugmentation(Node1.class);
		this.mappingService.getCodecRegistry().getCodecForAugmentation(IgpNodeAttributes1.class);
		
		final InstanceIdentifier<Topology> topologyIdentifier = InstanceIdentifier.builder(NetworkTopology.class).child(Topology.class, new TopologyKey(new TopologyId("topology"))).toInstance();
		...
		final Topology topology = (Topology) RestconfUtils.dataObjectFromInputStream(topologyIdentifier, is, this.mappingService.getSchemaContext(),
				this.mappingService, dataSchema);
		
		assertNotNull(topology);
		assertEquals(1, topology.getNode().size());
		final Node node = topology.getNode().get(0);
		assertEquals("bgpls://IsisLevel2:1/type=node&as=72&domain=673720360&router=0000.0000.0042", node.getNodeId().getValue());
		...
		final IgpNodeAttributes1 igpAttributes1 = node1.getIgpNodeAttributes().getAugmentation(IgpNodeAttributes1.class);
