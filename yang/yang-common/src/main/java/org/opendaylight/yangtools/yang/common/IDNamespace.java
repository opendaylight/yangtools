...
public enum IDNamespace {
    ...
    private static final Map<String, IDNamespace> STRING_TO_ENUM = new HashMap<>();

    static {
        for (IDNamespace idNamespace : values()) {
            STRING_TO_ENUM.put(idNamespace.toString(), idNamespace);
        }
    }

    IDNamespace(final String namespace) {
        this.namespace = namespace;
    }

    public static IDNamespace fromString(String namespace) {
        return STRING_TO_ENUM.get(namespace);
    }
