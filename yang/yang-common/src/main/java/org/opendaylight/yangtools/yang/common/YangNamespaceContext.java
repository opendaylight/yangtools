 ...
 *
 * <p>
 * Each namespace context has a default namespace and a set of prefix/namespace mappings. A namespace can be bound to
 * multiple prefixes at the same time. The default namespace must also have a prefix assigned.
 *
 * @author Robert Varga
 */
@Beta
public interface YangNamespaceContext extends Immutable, Serializable {
     ...
     */
    @NonNull QNameModule getDefaultNamespace();
    ...
    default @NonNull QName createQName(final String localName) {
        return QName.create(getDefaultNamespace(), requireNonNull(localName));
    }

    /**
     * Create a {@link QName} by resolving a prefix against currently-bound prefixes and
     *
