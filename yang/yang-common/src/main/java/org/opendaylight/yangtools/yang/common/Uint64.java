...
public final class Uint64 extends Number implements Comparable<Uint64>, Immutable {
    ...
    public static Uint64 valueOf(final short s) {
        checkArgument(s >= MIN_VALUE, "Negative values are not allowed");
        return instanceFor(s);
    }

    public static Uint64 valueOf(final int i) {
        checkArgument(i >= MIN_VALUE, "Value %s is outside of allowed range", i);
        return instanceFor(i);
    }

    public static Uint64 valueOf(final long l) {
        checkArgument(l >= MIN_VALUE, "Value %s is outside of allowed range", l);
