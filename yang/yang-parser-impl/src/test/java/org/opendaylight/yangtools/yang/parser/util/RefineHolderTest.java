...
public class RefineHolderTest {
    ...
    public void test() {
        ...
        URI simpleUri = null;
        try {
            simpleUri = new URI("very:simple:URI");
        ...
        if (simpleUri != null) {
            ...
            usnb.setDescription("description1");
            usnb1.setDescription("description1");

            assertEquals("rh should equal to rh1", rh, rh1);
            rh1.addUnknownNodeBuilder(usnb1);
            assertFalse("rh shouldn't equal to rh1", rh.equals(rh1));
            rh.addUnknownNodeBuilder(usnb);
            assertEquals("rh should equal to rh1", rh, rh1);
            usnb.setDescription("description");
            assertFalse("rh shouldn't equal to rh1", rh.equals(rh1));
            usnb1.setDescription("description");
            assertEquals("Wrong hash code", 2090554961, rh.hashCode());

            UnknownSchemaNodeBuilder usnbA = new UnknownSchemaNodeBuilder("usnb", 151, new QName(simpleUri, "tst"));
            UnknownSchemaNodeBuilder usnbA1 = new UnknownSchemaNodeBuilder("usnb", 151, new QName(simpleUri, "tst"));
            usnbA.setDescription("description1");
            usnbA1.setDescription("description");

            assertEquals("rh should equal to rh1", rh, rh1);
            rh1.setParent(usnbA1);
            assertFalse("rh shouldn't equal to rh1", rh.equals(rh1));
            rh.setParent(usnbA1);
            assertEquals("rh should equal to rh1", rh, rh1);
            rh.setParent(usnbA);
            assertFalse("rh shouldn't equal to rh1", rh.equals(rh1));
            rh1.setParent(usnbA);
            assertEquals("Wrong hash code", 1903186765, rh.hashCode());
