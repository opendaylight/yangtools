...
public class SameQNamesInChoiceTest {
    ...
    @Test
    public void testInnerChoiceCase() throws Exception {
        thrown.expect(YangParseException.class);
        thrown.expectMessage("Choice has two choice case with same qnames");

        SchemaContext context;
        File yangFile = new File(getClass().getResource("/bugs/qnameDuplicity/innerCases/inner-case.yang").toURI());
        File dependenciesDir = new File(getClass().getResource("/bugs/qnameDuplicity/innerCases").toURI());
        YangContextParser parser = new YangParserImpl();
        context = parser.parseFile(yangFile, dependenciesDir);

        Module testModule = context.findModuleByNamespace(URI.create("urn:test:inner:case")).iterator().next();
        assertNotNull(testModule);
    }
