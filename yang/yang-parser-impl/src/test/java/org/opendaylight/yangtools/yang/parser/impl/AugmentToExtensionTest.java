...
public class AugmentToExtensionTest {
    ...
    public void testIncorrectPath() throws IOException, URISyntaxException {
        modules = TestUtils.loadModules(getClass().getResource(
                "/augment-to-extension-test/incorrect-path").toURI());

    }

    @Test
    public void testCorrectPathIntoUnsupportedTarget() throws IOException,
            URISyntaxException {
        modules = TestUtils
                .loadModules(getClass()
                        .getResource(
                                "/augment-to-extension-test/correct-path-into-unsupported-target")
                        .toURI());

        Module devicesModul = TestUtils
                .findModule(modules, "tailf-ncs-devices");

        ContainerSchemaNode devicesContainer = (ContainerSchemaNode) devicesModul
    ...
    public void testCorrectAugment() throws IOException, URISyntaxException {
        modules = TestUtils.loadModules(getClass().getResource(
                "/augment-to-extension-test/correct-augment").toURI());
