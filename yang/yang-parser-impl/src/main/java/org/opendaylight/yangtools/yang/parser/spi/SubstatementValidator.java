...
public final class SubstatementValidator {
    ...
    public void validate(StmtContext ctx) throws YangValidationException{
        ...
        if (stmtDefMap.isEmpty() && specialCase == SpecialCase.NOTNULL) {
            throw new YangValidationException(String.format("%s must contain atleast 1 element. Error in module " +
                            "%s (%s) at: %s", currentStatement, ctx.getRoot().getStatementArgument(), ctx.getFromNamespace
        ...
        for (Map.Entry entry : stmtDefMap.entrySet()) {
            ...
            if (!cardinalityMap.containsKey(key)) {
                ...
                throw new YangValidationException(String.format("%s is not valid for %s. Error in module %s (%s) at: %s",
            ...
            if (cardinalityMap.get(key).getMin() > (Integer)entry.getValue()) {
                throw new YangValidationException(String.format("Minimal count of %s for %s is %s, detected %s. Error" +
                                " in module %s (%s) at: %s", key, currentStatement, cardinalityMap.get(key).getMin(),
                        entry.getValue(), ctx.getRoot().getStatementArgument(),
                        ctx.getFromNamespace(ModuleCtxToModuleQName.class, ctx.getRoot()),
                        ctx.getStatementSourceReference()));
            }
            if (cardinalityMap.get(key).getMax() < (Integer)entry.getValue()) {
                throw new YangValidationException(String.format("Maximal count of %s for %s is %s, detected %s. Error" +
                                " in module %s (%s) at: %s", key, currentStatement, cardinalityMap.get(key).getMax(),
                        entry.getValue(), ctx.getRoot().getStatementArgument(),
                        ctx.getFromNamespace(ModuleCtxToModuleQName.class, ctx.getRoot()),
                        ctx.getStatementSourceReference()));
            }
        ...
        for (Map.Entry entry : diff.entriesOnlyOnRight().entrySet()) {
            ...
            if (min > 0) {
                exception.append(String.format("%s is missing %s. Minimal count is %s. Error in module %s (%s) at: %s",
                        currentStatement, entry.getKey(), min, ctx.getRoot().getStatementArgument(),
                        ctx.getFromNamespace(ModuleCtxToModuleQName.class, ctx.getRoot()),
                        ctx.getStatementSourceReference()));
            }
