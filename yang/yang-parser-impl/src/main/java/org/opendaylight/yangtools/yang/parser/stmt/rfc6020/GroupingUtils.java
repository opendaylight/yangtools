...
public final class GroupingUtils {
    ...
    private static void performRefine(StatementContextBase<?, ?, ?> refineCtx,
        ...
        if(refineTargetNodeCtx == null) {
            throw new IllegalArgumentException(
                    "Refine target node not found. Path: "+refineTargetNodeIdentifier);
        }
    ...
    private static void addOrReplaceNodes(
        ...
        for (StatementContextBase<?, ?, ?> refineSubstatementCtx : declaredSubstatements) {
            if (isRelevantForRefine(refineSubstatementCtx)) {
    ...
    private static void addOrReplaceNode(
        ...
        if (!isAllowedRefineTarget(refineSubstatementDef, refineTargetNodeDef))
            return;
    ...
    private static void removeFromEffective(
        ...
        Iterator<StatementContextBase<?, ?, ?>> iterator = refineTargetNodeCtx.effectiveSubstatements().iterator();
    ...
    private static boolean isRelevantForRefine(
            StatementContextBase<?, ?, ?> refineSubstatement) {
        Set<StatementDefinition> allowedRefineSubstatementDefs = new HashSet<StatementDefinition>();
        allowedRefineSubstatementDefs.add(Rfc6020Mapping.DEFAULT);
        allowedRefineSubstatementDefs.add(Rfc6020Mapping.DESCRIPTION);
        allowedRefineSubstatementDefs.add(Rfc6020Mapping.REFERENCE);
        allowedRefineSubstatementDefs.add(Rfc6020Mapping.CONFIG);
        allowedRefineSubstatementDefs.add(Rfc6020Mapping.MANDATORY);
        allowedRefineSubstatementDefs.add(Rfc6020Mapping.MUST);
        allowedRefineSubstatementDefs.add(Rfc6020Mapping.PRESENCE);
        allowedRefineSubstatementDefs.add(Rfc6020Mapping.MIN_ELEMENTS);
        allowedRefineSubstatementDefs.add(Rfc6020Mapping.MAX_ELEMENTS);

        return allowedRefineSubstatementDefs.contains(refineSubstatement
                .getPublicDefinition());
    }

    private static boolean isAllowedRefineTarget(
            StatementDefinition refineSubstatementDef,
            StatementDefinition refineTargetNodeDef) {
        // TODO: Add validation.
        return true;
    }
