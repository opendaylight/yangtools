...
public class LeafRefBaseType implements LeafrefTypeDefinition {

    private static final QName QNAME = QName.create(YangConstants.RFC6020_YANG_MODULE, TypeUtils.LEAF_REF);
    ...
    private final RevisionAwareXPath target;

    private LeafRefBaseType(final SchemaPath schemaPath, final RevisionAwareXPath target) {
        ...
        this.target = Preconditions.checkNotNull(target,
                String.format("Target of learef in path %s must not be null", schemaPath));
    }

    public static LeafRefBaseType create(final SchemaPath schemaPath, final RevisionAwareXPath target) {
        return new LeafRefBaseType(schemaPath, target);
    }
