 ...
 * </li>
 * <li>{@link InferenceAction#prerequisiteWasNotMet(Collection)} - semantic parser finished all other satisfied
...
public interface ModelInferenceActionBuilder {
    ...
    public interface InferenceAction {
         ...
         *      so should provide helpful context to fix issue in sources.
         */
        void prerequisiteWasNotMet(Collection<Prereq<?>> failed) throws InferenceException;
    }

    @Nonnull <D extends DeclaredStatement<?>> Prereq<D> requiresDeclared(StmtContext<?,? extends D,?> context);

    @Nonnull <K,D extends DeclaredStatement<?>,N extends StatementNamespace<? super K, D, ?>> Prereq<D> requiresDeclared(StmtContext<?,?,?> context,Class<N> namespace, K key);

    @Nonnull <K,D extends DeclaredStatement<?>,N extends StatementNamespace<? super K, D, ?>> Prereq<StmtContext<?,D,?>> requiresDeclaredCtx(StmtContext<?,?,?> context,Class<N> namespace, K key);

    @Nonnull <E extends EffectiveStatement<?,?>> Prereq<E> requiresEffective(StmtContext<?,?,? extends E> stmt);

    @Nonnull <K,E extends EffectiveStatement<?,?>,N extends StatementNamespace<? super K, ?, E>> Prereq<E> requiresEffective(StmtContext<?,?,?> context,Class<N> namespace, K key);

    @Nonnull <K,E extends EffectiveStatement<?,?>,N extends StatementNamespace<? super K, ?, E>> Prereq<StmtContext<?,?,E>> requiresEffectiveCtx(StmtContext<?,?,?> context,Class<N> namespace, K key);

    @Nonnull <N extends IdentifierNamespace<? ,?>> Prereq<Mutable<?,?,?>> mutatesNamespace(Mutable<?,?, ?> ctx, Class<N> namespace);

    @Nonnull <T extends Mutable<?,?,?>> Prereq<T> mutatesEffectiveCtx(T stmt);

    @Nonnull  <K,E extends EffectiveStatement<?,?>,N extends StatementNamespace<? super K, ?, ? extends E>> Prereq<Mutable<?,?,E>> mutatesEffectiveCtx(StmtContext<?,?,?> context,Class<N> namespace, K key);
