...
public final class YangParserListenerImpl extends YangParserBaseListener {
	...
	private final DateFormat SIMPLE_DATE_FORMAT = new SimpleDateFormat(
			"yyyy-MM-dd");
	...
	private void updateRevisionForRevisionStatement(final ParseTree treeNode) {
		...
		try {
			...
			if ((revisionDate != null)
					&& (this.revision.compareTo(revisionDate) < 0)) {
	...
	public void enterImport_stmt(Import_stmtContext ctx) {
		...
		for (int i = 0; i < ctx.getChildCount(); ++i) {
			...
			if (treeNode instanceof Revision_date_stmtContext) {
				...
				try {
					importRevision = SIMPLE_DATE_FORMAT
							.parse(importRevisionStr);
				} catch (ParseException e) {
					LOGGER.warn("Failed to parse import revision-date at line "
							+ line + ": " + importRevisionStr);
				}
			}
		}
		moduleBuilder.addModuleImport(importName, importRevision, importPrefix);
		setLog("import", "(" + importName + "; " + importRevision + "; "
				+ importPrefix + ")");
	}
	...
	public void enterAugment_stmt(YangParser.Augment_stmtContext ctx) {
		...
		AugmentationSchemaBuilder builder = moduleBuilder.addAugment(line,
				augmentPath);
	...
	public void enterExtension_stmt(YangParser.Extension_stmtContext ctx) {
		...
		ExtensionBuilder builder = moduleBuilder
				.addExtension(qname, line, path);
	...
	public void enterTypedef_stmt(YangParser.Typedef_stmtContext ctx) {
		...
		TypeDefinitionBuilder builder = moduleBuilder.addTypedef(line,
				typedefQName, path);
		parseSchemaNodeArgs(ctx, builder);
		...
		if (YangTypesConverter.isBaseYangType(typeName)) {
			if (typeBody == null) {
				...
				case "identityref":
					...
					addNodeToPath(qname);
					SchemaPath path = createActualSchemaPath(actualPath.peek());
					moduleBuilder.addIdentityrefType(line, path,
							getIdentityrefBase(typeBody));
					break;
				default:
					type = parseTypeWithBody(typeName, typeBody,
							actualPath.peek(), namespace, revision,
			...
			type = parseUnknownTypeWithBody(typeQName, typeBody,
					actualPath.peek(), namespace, revision, yangModelPrefix,
	...
	public void enterGrouping_stmt(YangParser.Grouping_stmtContext ctx) {
		...
		GroupingBuilder builder = moduleBuilder.addGrouping(ctx.getStart()
				.getLine(), groupQName, path);
		parseSchemaNodeArgs(ctx, builder);
	...
	public void enterContainer_stmt(Container_stmtContext ctx) {
		...
		ContainerSchemaNodeBuilder builder = moduleBuilder.addContainerNode(
				line, containerQName, path);
		parseSchemaNodeArgs(ctx, builder);
		parseConstraints(ctx, builder.getConstraints());
		builder.setConfiguration(getConfig(ctx,
				moduleBuilder.getActualParent(), moduleName, line));
	...
	public void enterLeaf_stmt(Leaf_stmtContext ctx) {
		...
		LeafSchemaNodeBuilder builder = moduleBuilder.addLeafNode(line,
				leafQName, path);
		parseSchemaNodeArgs(ctx, builder);
		parseConstraints(ctx, builder.getConstraints());
		builder.setConfiguration(getConfig(ctx,
	...
	public void enterUses_stmt(YangParser.Uses_stmtContext ctx) {
		...
		UsesNodeBuilder builder = moduleBuilder.addUsesNode(line,
				groupingPathStr);
	...
	public void enterLeaf_list_stmt(Leaf_list_stmtContext ctx) {
		...
		LeafListSchemaNodeBuilder builder = moduleBuilder.addLeafListNode(line,
				leafListQName, path);
		moduleBuilder.enterNode(builder);
		...
		parseConstraints(ctx, builder.getConstraints());
		builder.setConfiguration(getConfig(ctx,
				moduleBuilder.getActualParent(), moduleName, ctx.getStart()
						.getLine()));
	...
	public void enterList_stmt(List_stmtContext ctx) {
		...
		QName listQName = new QName(namespace, revision, yangModelPrefix,
				listName);
		addNodeToPath(listQName);
		SchemaPath path = createActualSchemaPath(actualPath.peek());

		ListSchemaNodeBuilder builder = moduleBuilder.addListNode(line,
				listQName, path);
		moduleBuilder.enterNode(builder);
	...
	public void enterAnyxml_stmt(YangParser.Anyxml_stmtContext ctx) {
		...
		AnyXmlBuilder builder = moduleBuilder
				.addAnyXml(line, anyXmlQName, path);
		moduleBuilder.enterNode(builder);
	...
	public void enterChoice_stmt(YangParser.Choice_stmtContext ctx) {
		...
		ChoiceBuilder builder = moduleBuilder
				.addChoice(line, choiceQName, path);
		moduleBuilder.enterNode(builder);
	...
	public void enterCase_stmt(YangParser.Case_stmtContext ctx) {
		...
		QName caseQName = new QName(namespace, revision, yangModelPrefix,
				caseName);
		addNodeToPath(caseQName);
		SchemaPath path = createActualSchemaPath(actualPath.peek());

		ChoiceCaseBuilder builder = moduleBuilder
				.addCase(line, caseQName, path);
		moduleBuilder.enterNode(builder);
	...
	public void enterNotification_stmt(YangParser.Notification_stmtContext ctx) {
		...
		NotificationBuilder builder = moduleBuilder.addNotification(line,
				notificationQName, path);
		moduleBuilder.enterNode(builder);
	...
	public void enterIdentifier_stmt(YangParser.Identifier_stmtContext ctx) {
		...
		if (splittedElement.length == 1) {
			nodeType = new QName(namespace, revision, yangModelPrefix,
					splittedElement[0]);
		} else {
			nodeType = new QName(namespace, revision, splittedElement[0],
					splittedElement[1]);
		}
		...
		try {
			if (!Strings.isNullOrEmpty(nodeParameter)) {
				...
				if (splittedName.length == 2) {
					qname = new QName(null, null, splittedName[0],
							splittedName[1]);
				} else {
	...
	public void enterInput_stmt(YangParser.Input_stmtContext ctx) {
		...
		ContainerSchemaNodeBuilder builder = moduleBuilder.addRpcInput(line,
				rpcQName, path);
		moduleBuilder.enterNode(builder);
	...
	public void enterOutput_stmt(YangParser.Output_stmtContext ctx) {
		...
		ContainerSchemaNodeBuilder builder = moduleBuilder.addRpcOutput(path,
				rpcQName, line);
		moduleBuilder.enterNode(builder);
	...
	public void enterFeature_stmt(YangParser.Feature_stmtContext ctx) {
		...
		FeatureBuilder featureBuilder = moduleBuilder.addFeature(line,
				featureQName, path);
