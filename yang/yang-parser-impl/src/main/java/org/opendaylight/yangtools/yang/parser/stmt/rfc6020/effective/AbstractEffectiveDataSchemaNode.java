...
abstract class AbstractEffectiveDataSchemaNode<D extends DeclaredStatement<QName>> extends
    ...
    public AbstractEffectiveDataSchemaNode(final StmtContext<QName, D, ?> ctx) {
        ...
        if (originalHistory.contains(TypeOfCopy.ADDED_BY_USES_AUGMENTATION)) {
            this.addedByUses = this.augmenting = true;
        } else {
            this.augmenting = originalHistory.contains(TypeOfCopy.ADDED_BY_AUGMENTATION);
            this.addedByUses = originalHistory.contains(TypeOfCopy.ADDED_BY_USES);
        }
    }
