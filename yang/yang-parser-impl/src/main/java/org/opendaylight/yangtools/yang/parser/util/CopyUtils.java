...
public final class CopyUtils {
	 ...
	 * @return copy
	 */
	public static DataSchemaNodeBuilder copy(DataSchemaNodeBuilder old,
			Builder newParent, boolean updateQName) {
		if (old instanceof AnyXmlBuilder) {
			...
			return copy((ContainerSchemaNodeBuilder) old, newParent,
					updateQName);
			...
			throw new YangParseException(old.getModuleName(), old.getLine(),
					"Failed to copy node: Unknown type of DataSchemaNode: "
							+ old);
		}
	}

	private static AnyXmlBuilder copy(AnyXmlBuilder old, Builder newParent,
			boolean updateQName) {
		DataBean data = getdata(old, newParent, updateQName);
		QName newQName = data.qname;
		SchemaPath newSchemaPath = data.schemaPath;

		AnyXmlBuilder copy = new AnyXmlBuilder(newParent.getModuleName(),
	...
	private static ChoiceBuilder copy(ChoiceBuilder old, Builder newParent,
			boolean updateQName) {
		DataBean data = getdata(old, newParent, updateQName);
		QName newQName = data.qname;
		SchemaPath newSchemaPath = data.schemaPath;

		ChoiceBuilder copy = new ChoiceBuilder(newParent.getModuleName(),
				newParent.getLine(), newQName);
		copyConstraints(copy.getConstraints(), old.getConstraints());
		copy.setParent(newParent);
		copy.setPath(newSchemaPath);
		copy.setDescription(old.getDescription());
	...
	private static ChoiceCaseBuilder copy(ChoiceCaseBuilder old,
			Builder newParent, boolean updateQName) {
		DataBean data = getdata(old, newParent, updateQName);
		QName newQName = data.qname;
		SchemaPath newSchemaPath = data.schemaPath;

		ChoiceCaseBuilder copy = new ChoiceCaseBuilder(
				newParent.getModuleName(), newParent.getLine(), newQName);
		copyConstraints(copy.getConstraints(), old.getConstraints());
		copy.setParent(newParent);
		copy.setPath(newSchemaPath);
		copy.setDescription(old.getDescription());
	...
	private static ContainerSchemaNodeBuilder copy(
			ContainerSchemaNodeBuilder old, Builder newParent,
			boolean updateQName) {
		DataBean data = getdata(old, newParent, updateQName);
		QName newQName = data.qname;
		SchemaPath newSchemaPath = data.schemaPath;

		ContainerSchemaNodeBuilder copy = new ContainerSchemaNodeBuilder(
				newParent.getModuleName(), newParent.getLine(), newQName,
				newSchemaPath);
		copyConstraints(copy.getConstraints(), old.getConstraints());
		copy.setParent(newParent);
	...
	private static LeafSchemaNodeBuilder copy(LeafSchemaNodeBuilder old,
			Builder newParent, boolean updateQName) {
		DataBean data = getdata(old, newParent, updateQName);
		QName newQName = data.qname;
		SchemaPath newSchemaPath = data.schemaPath;

		LeafSchemaNodeBuilder copy = new LeafSchemaNodeBuilder(
				newParent.getModuleName(), newParent.getLine(), newQName,
				newSchemaPath);
		copyConstraints(copy.getConstraints(), old.getConstraints());
		copy.setParent(newParent);
	...
	public static LeafListSchemaNodeBuilder copy(LeafListSchemaNodeBuilder old,
			Builder newParent, boolean updateQName) {
		DataBean data = getdata(old, newParent, updateQName);
		QName newQName = data.qname;
		SchemaPath newSchemaPath = data.schemaPath;

		LeafListSchemaNodeBuilder copy = new LeafListSchemaNodeBuilder(
				newParent.getModuleName(), newParent.getLine(), newQName,
				newSchemaPath);
		copyConstraints(copy.getConstraints(), old.getConstraints());
		copy.setParent(newParent);
	...
	private static ListSchemaNodeBuilder copy(ListSchemaNodeBuilder old,
			Builder newParent, boolean updateQName) {
		DataBean data = getdata(old, newParent, updateQName);
		QName newQName = data.qname;
		SchemaPath newSchemaPath = data.schemaPath;

		ListSchemaNodeBuilder copy = new ListSchemaNodeBuilder(
				newParent.getModuleName(), newParent.getLine(), newQName,
				newSchemaPath);
		copyConstraints(copy.getConstraints(), old.getConstraints());
		copy.setParent(newParent);
	...
	public static GroupingBuilder copy(GroupingBuilder old, Builder newParent,
			boolean updateQName) {
		DataBean data = getdata(old, newParent, updateQName);
		QName newQName = data.qname;
		SchemaPath newSchemaPath = data.schemaPath;

		GroupingBuilderImpl copy = new GroupingBuilderImpl(
				newParent.getModuleName(), newParent.getLine(), newQName);
		copy.setParent(newParent);
		copy.setPath(newSchemaPath);
		copy.setDescription(old.getDescription());
	...
	public static TypeDefinitionBuilder copy(TypeDefinitionBuilder old,
			Builder newParent, boolean updateQName) {
		DataBean data = getdata(old, newParent, updateQName);
		QName newQName = data.qname;
		SchemaPath newSchemaPath = data.schemaPath;
		...
		if (old instanceof UnionTypeBuilder) {
			...
			type = new UnionTypeBuilder(newParent.getModuleName(),
					newParent.getLine());
			...
			type = new IdentityrefTypeBuilder(newParent.getModuleName(),
					newParent.getLine(),
					((IdentityrefTypeBuilder) old).getBaseString(),
					newSchemaPath);
			type.setParent(newParent);
			type.setPath(newSchemaPath);
		} else {
			type = new TypeDefinitionBuilderImpl(old.getModuleName(),
					newParent.getLine(), newQName);
			type.setParent(newParent);
			// TODO
			// type.setPath(newSchemaPath);
	...
	private static ConstraintsBuilder copyConstraints(
			ConstraintsBuilder newConstraints, ConstraintsBuilder old) {
	...
	static UsesNodeBuilder copyUses(UsesNodeBuilder old, Builder newParent) {
		UsesNodeBuilder copy = new UsesNodeBuilderImpl(
				newParent.getModuleName(), newParent.getLine(),
	...
	private static AugmentationSchemaBuilder copyAugment(
			AugmentationSchemaBuilder old, Builder newParent) {
		AugmentationSchemaBuilderImpl copy = new AugmentationSchemaBuilderImpl(
				newParent.getModuleName(), newParent.getLine(),
	...
	static UnknownSchemaNodeBuilder copy(UnknownSchemaNodeBuilder old,
			Builder newParent, boolean updateQName) {
		DataBean data = getdata(old, newParent, updateQName);
		QName newQName = data.qname;
		SchemaPath newSchemaPath = data.schemaPath;

		UnknownSchemaNodeBuilder c = new UnknownSchemaNodeBuilder(
				newParent.getModuleName(), newParent.getLine(), newQName);
	...
	private static DataBean getdata(SchemaNodeBuilder old, Builder newParent,
			boolean updateQName) {
		...
		if (newParent instanceof ModuleBuilder) {
			...
			if (updateQName) {
				newQName = new QName(parent.getNamespace(),
						parent.getRevision(), parent.getPrefix(), old
			...
			if (updateQName) {
				newQName = new QName(parentQName.getNamespace(),
						parentQName.getRevision(), parentQName.getPrefix(), old
								.getQName().getLocalName());
	 ...
	 */
	public static AnyXmlBuilder createAnyXml(AnyXmlSchemaNode anyxml,
			QName qname, String moduleName, int line) {
		final AnyXmlBuilder builder = new AnyXmlBuilder(moduleName, line,
				qname, anyxml.getPath());
	 ...
	 */
	public static GroupingBuilder createGrouping(GroupingDefinition grouping,
			QName qname, String moduleName, int line) {
		final GroupingBuilderImpl builder = new GroupingBuilderImpl(moduleName,
				line, qname);
	 ...
	 */
	public static TypeDefinitionBuilder createTypedef(ExtendedType typedef,
			QName qname, String moduleName, int line) {
		final TypeDefinitionBuilderImpl builder = new TypeDefinitionBuilderImpl(
				moduleName, line, qname);
	 ...
	 */
	public static UnknownSchemaNodeBuilder createUnknownSchemaNode(
			UnknownSchemaNode unknownNode, QName qname, String moduleName,
			int line) {
	 ...
	 */
	public static LeafSchemaNodeBuilder createLeafBuilder(LeafSchemaNode leaf,
			QName qname, String moduleName, int line) {
		final LeafSchemaNodeBuilder builder = new LeafSchemaNodeBuilder(
				moduleName, line, qname, leaf.getPath());
	 ...
	 */
	public static ContainerSchemaNodeBuilder createContainer(
			ContainerSchemaNode container, QName qname, String moduleName,
			int line) {
		final ContainerSchemaNodeBuilder builder = new ContainerSchemaNodeBuilder(
				moduleName, line, qname, container.getPath());
	 ...
	 */
	public static ListSchemaNodeBuilder createList(ListSchemaNode list,
			QName qname, String moduleName, int line) {
		ListSchemaNodeBuilder builder = new ListSchemaNodeBuilder(moduleName,
				line, qname, list.getPath());
	 ...
	 */
	public static LeafListSchemaNodeBuilder createLeafList(
			LeafListSchemaNode leafList, QName qname, String moduleName,
			int line) {
		final LeafListSchemaNodeBuilder builder = new LeafListSchemaNodeBuilder(
				moduleName, line, qname, leafList.getPath());
	 ...
	 */
	public static ChoiceBuilder createChoice(ChoiceNode choice, QName qname,
	 ...
	 */
	private static void convertDataSchemaNode(DataSchemaNode node,
			DataSchemaNodeBuilder builder) {
	 ...
	 */
	private static void copyConstraintsFromDefinition(
			final ConstraintDefinition nodeConstraints,
