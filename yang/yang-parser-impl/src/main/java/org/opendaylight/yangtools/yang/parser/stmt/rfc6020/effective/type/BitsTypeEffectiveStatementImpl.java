...
public final class BitsTypeEffectiveStatementImpl extends DeclaredEffectiveStatementBase<String, TypeStatement>
        ...
        for (final EffectiveStatement<?, ?> stmt : effectiveSubstatements()) {
            if (stmt instanceof BitEffectiveStatementImpl) {
                ...
                if (bitSubStmt.getDeclaredPosition() == null) {
                    effectivePos = getBaseTypeBitPosition(bitSubStmt.getName(), baseType);
                } else {
    ...
    private long getBaseTypeBitPosition(final String bitName, final BitsTypeDefinition baseType) {
        ...
        throw new InvalidBitDefinitionException("Bit '%s' is not a subset of its base bits type %s.", bitName,
                baseType.getQName());
    }
