...
public class SourceSpecificContext implements NamespaceStorageNode, NamespaceBehaviour.Registry, Mutable {
    ...
    ContextBuilder<?, ?, ?> createDeclaredChild(final StatementContextBase<?, ?, ?> current, final int childId,
        ...
        if (current != null && current.definition().getRepresentingClass().equals(UnknownStatementImpl.class)) {
            ...
            ret =  current.substatementBuilder(childId, new StatementDefinitionContext<>(support), ref);
        } else {
            ...
            final StatementDefinitionContext<?, ?, ?> def = currentContext.getStatementDefinition(getRootVersion(),
                support);
            if (current == null) {
