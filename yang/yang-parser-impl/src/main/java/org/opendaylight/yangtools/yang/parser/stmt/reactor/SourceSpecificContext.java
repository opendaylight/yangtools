    ...
    ContextBuilder<?, ?, ?> createDeclaredChild(final StatementContextBase<?, ?, ?> current, final QName name,
        ...
        if (def == null) {
            ...
            if (qNameToStmtDefMap.get(Utils.trimPrefix(name)) != null) {
                ...
                if (key != null) {
                    final StatementContextBase<?, ?, ?> extension = (StatementContextBase<?, ?, ?>)
                            currentContext.getAllFromNamespace(ExtensionNamespace.class).get(key);
                    if (extension != null) {
        ...
        return new ContextBuilder(def, ref) {
            ...
            public StatementContextBase build() throws SourceException {
                if (root == null) {
                    ...
                    Preconditions.checkState(root.getIdentifier().equals(createIdentifier()), "Root statement was " +
                            "already defined as %s.", root.getIdentifier());
                }
                root.resetLists();
                return root;
            }
