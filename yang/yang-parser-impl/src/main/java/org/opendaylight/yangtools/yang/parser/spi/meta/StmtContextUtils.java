...
public final class StmtContextUtils {
    ...
    public static boolean areFeaturesSupported(final StmtContext<?, ?, ?> stmtContext) {
        Set<QName> supportedFeatures = stmtContext.getFromNamespace(SupportedFeaturesNamespace.class,
        ...
        for (StatementContextBase<?, ?, ?> stmt: substatements) {
            if (stmt.getPublicDefinition().equals(Rfc6020Mapping.IF_FEATURE)) {
                ...
                if (supportedFeatures.contains(stmt.getStatementArgument())) {
                    isSupported = true;
                } else {
                    isSupported = false;
                    break;
        ...
        if (containsIfFeature && !isSupported) {
            return false;
        } else {
            return true;
        }
    ...
    public static boolean areAllFeaturesSupported(final StmtContext<?, ?, ?> stmtContext) {
        return stmtContext.getFromNamespace(SupportedFeaturesNamespace.class, SupportedFeatures.SUPPORTED_FEATURES)
                == CrossSourceStatementReactor.ALL_FEATURES_SUPPORTED;
    }
}
