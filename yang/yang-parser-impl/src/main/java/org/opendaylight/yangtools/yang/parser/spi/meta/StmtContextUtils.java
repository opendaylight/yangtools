...
public final class StmtContextUtils {
     ...
     */
    public static boolean hasAncestorOfTypeWithChildOfType(final StmtContext<?, ?, ?> ctx,
        ...
        while (!(current instanceof RootStatementContext)) {
            if (ancestorType.equals(current.getPublicDefinition())) {
                if (findFirstSubstatement(current, ancestorChildType.getDeclaredRepresentationClass()) == null) {
                    return false;
                }
            }
            current = current.getParentContext();
