        ...
        for (final String singleRange : PIPE_SPLITTER.split(rangeArgument)) {
            ...
            if (boundaries.hasNext()) {
                ...
                Preconditions.checkArgument(compareNumbers(min, max) != 1,
                        "Length constraint %s has descending order of boundaries; should be ascending. Statement " +
                                "source at %s", singleRange, ctx.getStatementSourceReference());
                Preconditions.checkArgument(!boundaries.hasNext(), "Wrong number of boundaries in length constraint " +
                                "%s. Statement source at %s", singleRange, ctx.getStatementSourceReference());
            } else {
                max = min;
            }
