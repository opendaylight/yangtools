 ...
 * So we must exclude 'm' character from the beginning of identifier, because of the range_arg 
 * and length_arg. Otherwise a range_arg or length_arg (for an example 'min..123') is matched by
 * Lexer as IDENTIFIER token.     
 */
IDENTIFIER : [a-ln-zA-Z_][a-zA-Z0-9_\-.]* -> pushMode(VALUE_MODE);
DATE_ARG : [0-9] [0-9] [0-9] [0-9] '-' [0-9] [0-9] '-' [0-9] [0-9] ;

NON_NEGATIVE_INTEGER_VALUE : [1-9][0-9]* | '0';
NEGATIVE_INTEGER_VALUE : '-' NON_NEGATIVE_INTEGER_VALUE;
DECIMAL_VALUE : (NON_NEGATIVE_INTEGER_VALUE | NEGATIVE_INTEGER_VALUE) '.' [0-9]+;

mode VALUE_MODE;

IDENTIFIER2 : [a-ln-zA-Z_][a-zA-Z0-9_\-.]* ->type(IDENTIFIER);
