...
stmtend : SEP? SEMICOLON | LEFT_BRACE SEP? (unknown_statement SEP?)* RIGHT_BRACE;

stmtsep : (unknown_statement | SEP)*;

identifier_ref: (prefix COLON)? identifier | string;

string : STRING (SEP? PLUS SEP? STRING)* 
         | (SLASH)? (IDENTIFIER COLON)? IDENTIFIER (SLASH (IDENTIFIER COLON)? IDENTIFIER)* 
         | DEF_MODE_STRING
         | DATE_ARG
         | ZERO
