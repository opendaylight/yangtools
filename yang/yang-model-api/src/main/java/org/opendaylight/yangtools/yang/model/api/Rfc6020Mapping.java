...
public enum Rfc6020Mapping implements StatementDefinition {
    ...
    private Rfc6020Mapping(Class<? extends DeclaredStatement<?>> clz,final String nameStr,final String argumentStr,final boolean yinElement) {
        ...
        name = QName.cachedReference(QName.create(YangConstants.RFC6020_YIN_MODULE, nameStr));
        if(argumentStr != null) {
            this.argument = QName.cachedReference(QName.create(YangConstants.RFC6020_YIN_MODULE,argumentStr));
        } else {
            this.argument = null;
        }
