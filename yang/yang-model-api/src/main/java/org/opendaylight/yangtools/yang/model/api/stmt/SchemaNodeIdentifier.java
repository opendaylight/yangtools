...
public abstract class SchemaNodeIdentifier implements Immutable {
    ...
    private SchemaPath createSchemaPath() {
        if (parent == null) {
            return SchemaPath.create(Collections.<QName>emptyList(), isAbsolute());
        }

        return parent.asSchemaPath().createChild(qname);
    }
    ...
    public final SchemaPath asSchemaPath() {
        SchemaPath ret = schemaPath;
        if (ret == null) {
            synchronized (this) {
                ret = schemaPath;
                if (ret == null) {
                    ret = createSchemaPath();
                    schemaPath = ret;
                }
            }
        }
