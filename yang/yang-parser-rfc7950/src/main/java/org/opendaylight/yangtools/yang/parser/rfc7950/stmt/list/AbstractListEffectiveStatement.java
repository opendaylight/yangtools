...
abstract class AbstractListEffectiveStatement
    ...
    public final List<QName> getKeyDefinition() {
        if (keyDefinition instanceof ImmutableList) {
            return (ImmutableList<QName>) keyDefinition;
        }
        verify(substatements instanceof QName, "Unexpected substatement %s", keyDefinition);
        return ImmutableList.of((QName) keyDefinition);
    }
