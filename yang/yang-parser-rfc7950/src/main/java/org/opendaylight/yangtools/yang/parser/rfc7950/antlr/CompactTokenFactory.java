        ...
        if (type >= Byte.MIN_VALUE && type <= Byte.MAX_VALUE) {
            ...
            if (line >= 0 && line <= 65535) {
                ...
                if (charPositionInLine >= 0 && charPositionInLine <= 255) {
                    ...
                    return start == stop ? new Token12140(source, type, line, charPositionInLine, start)
                            : new Token12144(source, type, line, charPositionInLine, start, stop);
                }
            }
        }

        return start == stop ? new Token44440(source, type, line, charPositionInLine, start)
                : new Token44444(source, type, line, charPositionInLine, start, stop);
    }
