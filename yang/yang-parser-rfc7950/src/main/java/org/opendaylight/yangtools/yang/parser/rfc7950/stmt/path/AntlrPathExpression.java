...
final class AntlrPathExpression implements PathExpression {
    private static final YangFunctionCallExpr CURRENT_CALL = YangFunctionCallExpr.of(
        YangFunction.CURRENT.getIdentifier());
    private static final YangFilterExpr CURRENT_EXPR = YangFilterExpr.of(CURRENT_CALL);

    private final @NonNull YangLocationPath location;

    private AntlrPathExpression(final YangLocationPath location) {
    ...
    static AntlrPathExpression parse(final StmtContext<?, ?, ?> ctx, final String pathArg) {
        ...
        return new AntlrPathExpression(location);
    }

    private static YangLocationPath parseAbsolute(final StmtContext<?, ?, ?> ctx, final String pathArg,
    ...
    private static YangLocationPath parseRelative(final StmtContext<?, ?, ?> ctx, final String pathArg,
    ...
    private static YangFilterExpr parsePathKeyExpr(final StmtContext<?, ?, ?> ctx, final Path_key_exprContext expr) {
        ...
        return steps.isEmpty() ? CURRENT_EXPR : YangFilterExpr.of(CURRENT_CALL, YangLocationPath.of(true, steps));
    }
