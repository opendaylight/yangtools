...
abstract class AbstractAugmentInference implements InferenceAction {
    private static final Logger LOG = LoggerFactory.getLogger(UsesAugmentBehavior.class);
    ...
    final Mutable<SchemaNodeIdentifier, AugmentStatement, AugmentEffectiveStatement> augment;
    final Prerequisite<Mutable<?, ?, EffectiveStatement<?, ?>>> target;
    final boolean allowsMandatory;

    AbstractAugmentInference(final Mutable<SchemaNodeIdentifier, AugmentStatement, AugmentEffectiveStatement> augment,
        ...
        this.allowsMandatory = allowsMandatory;
    }
    ...
    public void apply(final InferenceContext ctx) {
        ...
        if (augmentTargetCtx.hasImplicitParentSupport()) {
            augment.addToNs(AugmentImplicitHandlingNamespace.class, augment, augmentTargetCtx);
        }

        final StatementContextBase<?, ?, ?> augmentSourceCtx = (StatementContextBase<?, ?, ?>) augment;
        ...
        try {
            ...
            // FIXME: 1190: this is wrong and needs to be fixed
    ...
    public void prerequisiteFailed(final Collection<? extends Prerequisite<?>> failed) {
        throw new InferenceException(augment.sourceReference(), "Augment target '%s' not found", augment.argument());
    }
    ...
    private void copyFromSourceToTarget(final StatementContextBase<?, ?, ?> sourceCtx,
        ...
        for (final Mutable<?, ?, ?> originalStmtCtx : declared) {
            copyStatement(originalStmtCtx, targetCtx, typeOfCopy, buffer, allowsMandatory,
                unsupported || !originalStmtCtx.isSupportedByFeatures());
        }
        for (final Mutable<?, ?, ?> originalStmtCtx : effective) {
            copyStatement(originalStmtCtx, targetCtx, typeOfCopy, buffer, allowsMandatory, unsupported);
        }
    ...
    private static void checkForMandatoryNodes(final StmtContext<?, ?, ?> sourceCtx) {
        if (StmtContextUtils.isNonPresenceContainer(sourceCtx)) {
             ...
             */
            sourceCtx.allSubstatementsStream().forEach(AbstractAugmentInference::checkForMandatoryNodes);
        }
