...
public final class AnyxmlStatementSupport
    ...
    protected AnyxmlEffectiveStatement createEffective(final Current<QName, AnyxmlStatement> stmt,
        ...
        final int flags = computeFlags(stmt.history(), ctx.isConfiguration(), substatements);
        final SchemaPath path = stmt.getSchemaPath();

        return substatements.isEmpty()
            ? new EmptyAnyxmlEffectiveStatement(stmt.declared(), path, flags, findOriginal(ctx))
                : new RegularAnyxmlEffectiveStatement(stmt.declared(), path, flags, findOriginal(ctx),substatements);
    }
