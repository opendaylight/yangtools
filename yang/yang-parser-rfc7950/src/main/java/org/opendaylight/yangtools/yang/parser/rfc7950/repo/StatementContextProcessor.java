...
abstract class StatementContextProcessor {
    static final class Loose extends StatementContextProcessor {
    ...
    static final class Strict extends StatementContextProcessor {
        ...
        @Override
        QName getValidStatementDefinition(final PrefixToModule prefixes, final QNameToStatementDefinition stmtDef,
                final String keywordText, final StatementSourceReference ref) {
            return SourceException.throwIfNull(super.getValidStatementDefinition(prefixes, stmtDef, keywordText, ref),
                ref, "%s is not a YANG statement or use of extension.", keywordText);
        }
    }
    ...
    private StatementContextProcessor(final String sourceName, final StatementWriter writer,
            final QNameToStatementDefinition stmtDef, final PrefixToModule prefixes, final YangVersion yangVersion) {
    ...
    void process(final StatementContext context) {
    ...
    private void processStatement(final int myOffset, final StatementContext ctx) {
        ...
        final QName def = getValidStatementDefinition(prefixes, stmtDef, keywordTxt, ref);
        if (def == null) {
            return;
        }
