...
import org.opendaylight.yangtools.yang.parser.rfc7950.ir.IRArgument.Literal;
...
abstract class ArgumentContextUtils {
    ...
    final @NonNull String stringFromStringContext(final IRArgument argument, final StatementSourceReference ref) {
        if (argument instanceof Unquoted) {
            final String str = ((Unquoted) argument).string();
            checkUnquoted(str, ref);
            return str;
        }
        if (argument instanceof SingleQuoted) {
            return ((SingleQuoted) argument).string();
        }
        if (argument instanceof DoubleQuoted) {
            return normalizeDoubleQuoted(((DoubleQuoted) argument).string(), ref);
        }
    ...
    private @NonNull String concatStrings(final List<IRArgument> parts, final StatementSourceReference ref) {
        final StringBuilder sb = new StringBuilder();
        for (IRArgument child : parts) {
            if (child instanceof DoubleQuoted) {
                sb.append(normalizeDoubleQuoted(((DoubleQuoted) child).string(), ref));
            } else if (child instanceof Literal) {
                sb.append(((Literal) child).string());
            } else {
                throw new VerifyException("Unhnadled part " + child.getClass());
            }
        }
        return sb.toString();
    }

    private @NonNull String normalizeDoubleQuoted(final String str, final StatementSourceReference ref) {
