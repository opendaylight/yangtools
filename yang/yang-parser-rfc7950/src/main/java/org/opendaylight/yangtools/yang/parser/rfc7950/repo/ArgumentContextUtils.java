...
abstract class ArgumentContextUtils {
    ...
    static void unescapeBackslash(final StringBuilder sb, final String str, final int backslash) {
        ...
        if (nextIndex < str.length()) {
            ...
            final char c = str.charAt(nextIndex);
            switch (c) {
                case '\\':
                case '"':
                    sb.append(c);
    ...
    private static void findNextBackslash(final StringBuilder sb, final String str, final int backslashIndex) {
        String substring = str.substring(backslashIndex + 1);
        if (substring.length() > 0) {
            final int index = substring.indexOf('\\');
            if (index != -1) {
                unescapeBackslash(sb, substring, index);
            } else {
                sb.append(substring);
            }
        }
