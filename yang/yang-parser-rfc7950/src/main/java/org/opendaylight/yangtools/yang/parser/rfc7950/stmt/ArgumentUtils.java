...
public final class ArgumentUtils {
    ...
    private static final Splitter SLASH_SPLITTER = Splitter.on('/').omitEmptyStrings().trimResults();
        ...
        // TODO: eliminate this static binding when convenient
    private static final BiFunction<StmtContext<?, ?, ?>, String, RevisionAwareXPath> XPATH_FACTORY = xpathFactory();
    ...
    public static RevisionAwareXPath parseXPath(final StmtContext<?, ?, ?> ctx, final String path) {
        return XPATH_FACTORY.apply(ctx, path);
    }

    private static RevisionAwareXPath parseXPath(final YangXPathParserFactory factory, final StmtContext<?, ?, ?> ctx,
            final String xpath) {
        ...
        try {
            ...
            LOG.warn("Argument \"{}\" is not valid XPath string at \"{}\"", xpath, ctx.getStatementSourceReference(),
                e);
            return fallbackXPath(xpath);
        }
    ...
    private static RevisionAwareXPath fallbackXPath(final String xpath) {
        return new RevisionAwareXPathImpl(xpath, isAbsoluteXPath(xpath));
    }
    ...
    private static BiFunction<StmtContext<?, ?, ?>, String, RevisionAwareXPath> xpathFactory() {
        ...
        if (!it.hasNext()) {
            ...
            return (ctx, xpath) -> fallbackXPath(xpath);
        }

        final YangXPathParserFactory factory = it.next();
        return (ctx, xpath) -> parseXPath(factory, ctx, xpath);
    }
