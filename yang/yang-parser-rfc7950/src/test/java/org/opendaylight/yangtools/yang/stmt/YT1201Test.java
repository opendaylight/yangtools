...
public class YT1201Test {
    ...
    public void testWhenPrefixes() throws Exception {
        ...
        final RevisionAwareXPath when = ((ContainerSchemaNode) bar).getWhenCondition().get();
        assertThat(when, instanceOf(WithExpression.class));
        final YangExpr expr = ((WithExpression) when).getXPathExpression().getRootExpr();
        assertThat(expr, instanceOf(Relative.class));
        assertEquals(List.of(YangXPathAxis.CHILD.asStep(FOO)), ((Relative) expr).getSteps());
    }
}
