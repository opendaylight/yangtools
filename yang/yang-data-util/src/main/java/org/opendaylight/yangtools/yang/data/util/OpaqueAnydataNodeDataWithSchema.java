...
public class OpaqueAnydataNodeDataWithSchema extends AbstractAnydataNodeDataWithSchema<OpaqueData> {
    ...
    protected void write(final NormalizedNodeStreamWriter writer, final NormalizedMetadataStreamWriter metaWriter)
        ...
        if (ext != null) {
            ...
            streamNode(ext.startOpaqueAnydataNode(provideNodeIdentifier(), value.hasAccurateLists()), value.getRoot());
            writer.endNode();
        }
    }

    private static void streamNode(final StreamWriter opaqueWriter, final OpaqueDataNode node) throws IOException {
        if (node instanceof OpaqueDataValue) {
            opaqueWriter.startOpaqueContainer(node.getIdentifier());
            opaqueWriter.opaqueValue(((OpaqueDataValue) node).getValue());
        } else if (node instanceof OpaqueDataContainer) {
            opaqueWriter.startOpaqueContainer(node.getIdentifier());
            for (OpaqueDataNode child : ((OpaqueDataContainer) node).getChildren()) {
                streamNode(opaqueWriter, child);
            }
        } else if (node instanceof OpaqueDataList) {
            opaqueWriter.startOpaqueList(node.getIdentifier());
            for (OpaqueDataNode child : ((OpaqueDataList) node).getChildren()) {
                streamNode(opaqueWriter, child);
            }
        } else {
