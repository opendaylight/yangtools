 ...
 *
 */
class PathArgumentParsingIterator extends AbstractIterator<PathArgument> {
    ...
    PathArgumentParsingIterator(AbstractStringInstanceIdentifierCodec codec, String data) {
        super();
    ...
    @Override
    protected PathArgument computeNext() {
        ...
        if(allCharactersConsumed() || SLASH == currentChar) {
            // FIXME: We should lookup choices and cases to emit correct
    ...
    private PathArgument computeIdentifierWithPredicate(QName name) {
        Builder<QName,Object> keyValues = ImmutableMap.builder();
     ...
     */
    private void checkValid(boolean condition, String errorMsg) {
    ...
    private void nextSequenceEnd(CharMatcher matcher) {
        while(matcher.matches(data.charAt(offset))) {
            offset++;
        }
    }
