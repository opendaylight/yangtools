    ...
    private void writeStatic(final @NonNull MountPointStreamWriter mountWriter,
        ...
        try (NormalizedNodeStreamWriter writer = optWriter.get()) {
            for (MountPointChild child : children) {
                ...
                final Deque<DataSchemaNode> dataPath = entry.getValue();
                if (dataPath.size() != 1) {
                    // FIXME: this is a bit rough:
                    throw new UnsupportedOperationException("Choice/Case handling not implemented yet");
                }

                NormalizedNodeWriter.forStreamWriter(writer).write(entry.getKey().getData());
            }
        }
    }
}
