...
public final class MountPointData implements Identifiable<QName> {
    ...
    private void writeStatic(final @NonNull MountPointStreamWriter mountWriter,
        ...
        final Optional<NormalizedNodeStreamWriter> optWriter = mountWriter.startMountPoint(label, schemaContext);
        if (!optWriter.isPresent()) {
            LOG.debug("Ignoring mount point {} data due to writer decision", label);
            return;
        }

        final CompositeNodeDataWithSchema<?> root = new CompositeNodeDataWithSchema<>(schemaContext);
        for (MountPointChild child : children) {
            final Entry<NormalizedAnydata, Deque<DataSchemaNode>> entry;
            try {
                entry = child.normalizeTo(schemaContext);
            } catch (AnydataNormalizationException e) {
                throw new IOException("Failed to normalize child " + child, e);
            }

            final AbstractNodeDataWithSchema<?> dataChild = root.addChild(entry.getValue());
            if (dataChild instanceof SimpleNodeDataWithSchema) {
                ((SimpleNodeDataWithSchema<?>) dataChild).setValue(entry.getKey().getData().getValue());
            }
        }
