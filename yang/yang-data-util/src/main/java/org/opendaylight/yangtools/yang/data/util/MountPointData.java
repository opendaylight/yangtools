...
import org.opendaylight.yangtools.rfc8528.data.api.InlineMountPointMetadataResolver;
import org.opendaylight.yangtools.rfc8528.data.api.InlineMountPointMetadataResolver.LibraryContext;
...
public final class MountPointData extends AbstractIdentifiable<MountPointIdentifier> {
    ...
    void write(final @NonNull NormalizedNodeStreamWriter writer) throws IOException {
        ...
        final Optional<MountPointMetadataResolver> optResolver = mountWriter.findMountPoint(getIdentifier());
        if (!optResolver.isPresent()) {
            LOG.debug("Mount point for {} is not present, ignoring it", getIdentifier());
            return;
        }

        final MountPointMetadataResolver resolver = optResolver.get();
        if (resolver instanceof MountPointMetadata) {
            writeTo(mountWriter, (MountPointMetadata) resolver);
        } else if (resolver instanceof InlineMountPointMetadataResolver) {
            writeInline(mountWriter, (InlineMountPointMetadataResolver) resolver);
        } else {
    ...
    private void writeInline(final @NonNull MountPointStreamWriter mountWriter,
            final InlineMountPointMetadataResolver resolver) throws IOException {
        for (Entry<ContainerName, MountPointChild> entry : yangLib.entrySet()) {
            ...
            final MountPointMetadata mountMeta;
            try {
    ...
    private void writeTo(final @NonNull MountPointStreamWriter mountWriter,
            final @NonNull MountPointMetadata mountMeta) throws IOException {
