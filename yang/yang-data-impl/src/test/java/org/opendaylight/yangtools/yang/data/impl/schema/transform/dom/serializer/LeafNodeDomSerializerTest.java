...
public class LeafNodeDomSerializerTest {
    ...
    public void LeafNodeDomSerializerTest() throws ReactorException, IOException, YangSyntaxErrorException, ParseException {
        DataSchemaNode currentContainer = DomSerializerTestUtils.getSchemaContext().findModuleByName("serializer-test", null)
        ...
        LeafSchemaNode currentLeaf = (LeafSchemaNode) ((ContainerEffectiveStatementImpl) currentContainer).getDataChildByName
                (DomSerializerTestUtils.generateQname("first-leaf"));
        assertNotNull(currentLeaf.getType());

        LeafNode<String> tempLeafNode = new ImmutableLeafNodeBuilder<String>()
                .withValue("testing data")
                .withNodeIdentifier(DomSerializerTestUtils.getNodeIdentifier("first-leaf", "dom-serializer-test", "2016-01-01"))
                .build();

        LeafNodeDomSerializer temp = new LeafNodeDomSerializer(DomSerializerTestUtils.doc, DomSerializerTestUtils.codecProvider);
        DomSerializerTestUtils.element = temp.serializeLeaf(currentLeaf, tempLeafNode);
        ...
        assertTrue(DomSerializerTestUtils.element.getLocalName().equals(singleElementFromAbstractClass.getLocalName()));
        assertTrue(DomSerializerTestUtils.element.getNamespaceURI().equals(singleElementFromAbstractClass.getNamespaceURI()));
