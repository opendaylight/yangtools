...
public class RFC8639Test {
    ...
    private static final ContainerNode DATA = prepareData(
            "stream-name", "subscriber");

    private static ContainerNode prepareData(final String streamName, final String subscriber) {
        final DataContainerNodeBuilder<NodeIdentifierWithPredicates, MapEntryNode> subscriptionEntry =
                        ...
                        .withChild(Builders.augmentationBuilder()
                                ...
                                .withChild(Builders.leafBuilder()
                                        .withNodeIdentifier(NodeIdentifier.create(
                                                QName.create(MODULE, "stream")))
                                        .withValue(streamName)
                                        .build())
                                .build())
                        .build());

        final DataContainerNodeBuilder<NodeIdentifierWithPredicates, MapEntryNode> receiverEntry =
                Builders.mapEntryBuilder()
                        .withNodeIdentifier(NodeIdentifierWithPredicates.of(
