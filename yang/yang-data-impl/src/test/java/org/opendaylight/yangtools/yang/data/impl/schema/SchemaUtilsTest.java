...
public class SchemaUtilsTest {
    ...
    public void test() {
        ...
        assertTrue(SchemaUtils.findDataParentSchemaOnPath(schemaContext,
                Absolute.of(qN("my-name"), qN("my-name-a"))) instanceof ContainerSchemaNode);
        assertTrue(SchemaUtils.findDataParentSchemaOnPath(schemaContext,
                Absolute.of(qN("my-name-2"), qN("my-name-b"))) instanceof NotificationDefinition);
        assertTrue(SchemaUtils.findDataParentSchemaOnPath(schemaContext,
                Absolute.of(qN("my-name-2"), qN("my-name-2-b"))) instanceof ActionDefinition);
    }
    ...
    public void testNameConflicts() {
        ...
        assertEquals(8, SchemaUtils.findParentSchemaNodesOnPath(schemaContext,
                ImmutableList.of(qN("my-name"), qN("my-name-nested"), qN("my-name-nested2"))).size());

        // test target container
        final Collection<SchemaNode> target = SchemaUtils.findParentSchemaNodesOnPath(schemaContext,
                ImmutableList.of(qN("my-name-2"), qN("my-name-nested"), qN("target")));
        ...
        Collection<SchemaNode> schema = SchemaUtils.findParentSchemaNodesOnPath(schemaContext,
                ImmutableList.of(qN("my-name-3"), qN("input"), qN("con-3"), qN("l")));
        ...
        schema = SchemaUtils.findParentSchemaNodesOnPath(schemaContext,
                ImmutableList.of(qN("my-name-3"), qN("input"), qN("con-1"), qN("l")));
        assertEquals(1, schema.size());
        assertTrue(schema.iterator().next() instanceof LeafSchemaNode);

        schema = SchemaUtils.findParentSchemaNodesOnPath(schemaContext,
                ImmutableList.of(qN("my-name-3"), qN("input"), qN("con-2"), qN("l")));
        assertTrue(schema.isEmpty());

        schema = SchemaUtils.findParentSchemaNodesOnPath(schemaContext,
                ImmutableList.of(qN("my-name-3"), qN("output"), qN("con-2"), qN("l")));
        assertEquals(1, schema.size());
        assertTrue(schema.iterator().next() instanceof LeafSchemaNode);
    }
