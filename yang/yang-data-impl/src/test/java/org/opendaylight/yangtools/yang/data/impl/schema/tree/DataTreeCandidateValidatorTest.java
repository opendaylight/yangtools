...
public class DataTreeCandidateValidatorTest {
    ...
    private void writeIntoMapEntry() {
        ...
        NodeIdentifierWithPredicates mapEntryPath = new NodeIdentifierWithPredicates(
                project, keys);

        YangInstanceIdentifier leaderPath = YangInstanceIdentifier.of(odl)
                .node(project).node(mapEntryPath).node(lead);

        LeafNode<String> leader = ImmutableNodes.leafNode(lead,
                "Updated leader");
