...
public final class StructuralApplyModificationTest {
    ...
    public void testNestedStrucutralNodes() throws Exception {
        ...
        final YangInstanceIdentifier path = YangInstanceIdentifier.create(
            ...
            getNId(TestModel.TOP_LIST_QNAME),
            getNId(TestModel.TOP_LIST_QNAME, TestModel.ID_QNAME, "1"),
            getNId(TestModel.MIDDLE_NON_PRESENCE_QNAME),
            getNId(TestModel.MIDDLE_LIST_QNAME),
            getNId(TestModel.MIDDLE_LIST_QNAME, TestModel.ID_QNAME, "2"),
            getNId(TestModel.BOTTOM_NON_PRESENCE_QNAME),
            getNId(TestModel.DEEP_CHOICE_QNAME),
            getNId(TestModel.A_CONTAINER_QNAME)
        );

        addListEntryModification.write(path,
            Builders.containerBuilder()
                .withNodeIdentifier(getNId(TestModel.A_CONTAINER_QNAME))
                .withChild(Builders.leafBuilder()
                    .withNodeIdentifier(getNId(TestModel.A_NAME_QNAME))
                    .withValue("deep-choice")
                    .build())
                .build());

        // FIXME this fails if inMemoryDataTree is CONFIG instead of OPERATIONAL
        // The problem is in ChoiceModificationStrategy. Its constructor only acknowledges
        // child nodes if enforcer is present (it is present only if OPERATIONAL DS is used
        // or if schema node is marked as "config). However for Choice schema nodes, it is hardcoded to return always
        // false to isConfig()
        //
        // Only then it adds them to childBuilder.
        // Without the nodes being present there, no modification can be applied under a choice
        addListEntryModification.ready();
        inMemoryDataTree.validate(addListEntryModification);
        inMemoryDataTree.commit(inMemoryDataTree.prepare(addListEntryModification));
