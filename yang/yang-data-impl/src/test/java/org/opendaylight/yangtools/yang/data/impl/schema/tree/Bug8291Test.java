...
public class Bug8291Test {
    ...
    private static InMemoryDataTree initDataTree(final SchemaContext schemaContext)
        ...
        //final InMemoryDataTree inMemoryDataTree = (InMemoryDataTree) InMemoryDataTreeFactory.getInstance().create(TreeType.CONFIGURATION, YangInstanceIdentifier.of(ROOT).node(OUTER_LIST).node(new NodeIdentifierWithPredicates(OUTER_LIST, ImmutableMap.of(OUTER_LIST_ID, 1))));
        final InMemoryDataTree inMemoryDataTree = (InMemoryDataTree) InMemoryDataTreeFactory.getInstance().create(TreeType.CONFIGURATION, YangInstanceIdentifier.of(ROOT).node(OUTER_LIST));
...
//                YangInstanceIdentifier.of(INNER_LIST),
//                Builders.mapBuilder().withNodeIdentifier(new NodeIdentifier(INNER_LIST)).build());

        modificationTree.write(
                YangInstanceIdentifier.create(new NodeIdentifierWithPredicates(OUTER_LIST, ImmutableMap.of(OUTER_LIST_ID, 1))).node(INNER_LIST),
                Builders.mapBuilder().withNodeIdentifier(new NodeIdentifier(INNER_LIST)).build());
    ...
    private static void writeOuterListMapEntry(final InMemoryDataTree inMemoryDataTree) throws DataValidationFailedException {
        final InMemoryDataTreeModification modificationTree = inMemoryDataTree.takeSnapshot().newModification();
