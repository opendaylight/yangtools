...
abstract class AbstractDispatcherSerializer<I extends InstanceIdentifier.PathArgument, N extends DataContainerNode<I>, S>
    ...
    public List<Element> toDom(S schema, N node, XmlCodecProvider codec, Document doc) {
        ...
        for (DataContainerChild<? extends InstanceIdentifier.PathArgument, ?> choiceChild : node.getValue()) {
            ...
            if (choiceChild instanceof AugmentationNode) {
                ...
                AugmentationSchema childSchema = getAugmentedCase(schema, (AugmentationNode) choiceChild);
                Set<DataSchemaNode> realChildSchemas = getRealSchemasForAugment(schema, childSchema);
                List<Element> childElements = new AugmentationNodeDomSerializer().toDom(childSchema,
                        (AugmentationNode) choiceChild, realChildSchemas, codec, doc);
                for (Element childElement : childElements) {
                    choiceChildren.add(childElement);
                }
            } else {

                DataSchemaNode childSchema = getSchemaForChild(schema, choiceChild);
    ...
    static final class NodeDispatcher {
        ...
        private static List<Element> onChoiceNode(DataSchemaNode childSchema, DataContainerChild<? extends InstanceIdentifier.PathArgument, ?> dataContainerChild, XmlCodecProvider codecProvider, Document doc) {
            checkSchemaCompatibility(childSchema, org.opendaylight.yangtools.yang.model.api.ChoiceNode.class,
                    dataContainerChild);
            return CHOICE_NODE_DOM_SERIALIZER.toDom(
                    (org.opendaylight.yangtools.yang.model.api.ChoiceNode) childSchema,
                    (ChoiceNode) dataContainerChild, codecProvider, doc);
        }

        private static List<Element> onListNode(DataSchemaNode childSchema, DataContainerChild<? extends InstanceIdentifier.PathArgument, ?> dataContainerChild, XmlCodecProvider codecProvider, Document doc) {
        ...
        private static List<Element> onLeafListNode(DataSchemaNode childSchema, DataContainerChild<? extends InstanceIdentifier.PathArgument, ?> dataContainerChild, XmlCodecProvider codecProvider, Document doc) {
        ...
        private static List<Element> onLeafNode(DataSchemaNode childSchema, DataContainerChild<? extends InstanceIdentifier.PathArgument, ?> dataContainerChild, XmlCodecProvider codecProvider, Document doc) {
            checkSchemaCompatibility(childSchema, LeafSchemaNode.class, dataContainerChild);
            List<Element> elements = LEAF_NODE_DOM_SERIALIZER.toDom((LeafSchemaNode) childSchema, (LeafNode<?>) dataContainerChild,
                    codecProvider, doc);
        ...
        private static List<Element> onContainerNode(DataSchemaNode childSchema, DataContainerChild<? extends InstanceIdentifier.PathArgument, ?> dataContainerChild, XmlCodecProvider codecProvider, Document doc) {
        ...
        private static void checkSchemaCompatibility(DataSchemaNode childSchema,
                Class<? extends DataSchemaNode> containerSchemaNodeClass,
                DataContainerChild<? extends InstanceIdentifier.PathArgument, ?> dataContainerChild) {
