            ...
            if (SchemaAwareApplyOperation.belongsToTree(type, child)) {
                if (child instanceof ContainerSchemaNode) {
                    ...
                    if (!needEnforce && child instanceof ElementCountConstraintAware) {
                        final Integer minElements = ((ElementCountConstraintAware) child)
                                .getElementCountConstraint().map(ElementCountConstraint::getMinElements).orElse(null);
                        needEnforce = minElements != null && minElements > 0;
                    }
                    if (needEnforce) {
