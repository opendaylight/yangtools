    ...
    void seal(final ModificationApplyOperation schema, final Version version) {
        ...
        switch (operation) {
            ...
            case WRITE:
                ...
                /*if (!children.isEmpty()) {
                    final TreeNode treeNode = schema.apply(this, getOriginal(), version).get();
                    value = treeNode.getData();
                    children.clear();
                }
                ...
                if (!children.isEmpty()) {
                    ...
                    // Nodes managed by StructuralModificationStrategyWrapper cna be deleted during write
