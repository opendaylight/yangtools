...
abstract class UniqueConstraintEnforcer implements Immutable {
    private static final class NoOp extends UniqueConstraintEnforcer {
        ...
        @Override
        protected Map<List<YangInstanceIdentifier>, TreeNodeIndex> createUniqueIndexesFromData(
    ...
    private static final class Strict extends UniqueConstraintEnforcer {
        ...
        protected void enforceOnTreeNode(final NormalizedNode<?, ?> data) {
            for (final Entry<List<YangInstanceIdentifier>, UniqueIndex> uniqueIndexEntry : uniqueIndexes.entrySet()) {
                uniqueIndexEntry.getValue().ifApplicableCheckIndex(data);
            }
        }

        @Override
        protected Map<List<YangInstanceIdentifier>, TreeNodeIndex> createUniqueIndexesFromData(final NormalizedNode<?, ?> newValue) {
            final ImmutableMap.Builder<List<YangInstanceIdentifier>, TreeNodeIndex> uniqueIndexesBuilder = ImmutableMap
                    .builder();
            for (final Entry<List<YangInstanceIdentifier>, UniqueIndex> uniqueIndex : uniqueIndexes.entrySet()) {
                //:TODO create unique indexes with data
            }
            return uniqueIndexesBuilder.build();
        }
    }
    ...
    protected abstract Map<List<YangInstanceIdentifier>, TreeNodeIndex> createUniqueIndexesFromData(final NormalizedNode<?, ?> newValue);
