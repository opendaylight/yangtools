...
public class MapEntryNodeDomSerializer extends
        AbstractDispatcherSerializer<InstanceIdentifier.NodeIdentifierWithPredicates, MapEntryNode, ListSchemaNode> {

    public List<Element> toDom(ListSchemaNode schema, MapEntryNode node, XmlCodecProvider codec, Document doc) {
        Element itemEl = XmlDocumentUtils.createElementFor(doc, node);

        for (Element element : super.toDom(schema, node, codec, doc)) {
    ...
    protected Set<DataSchemaNode> getRealSchemasForAugment(ListSchemaNode schema, AugmentationSchema augmentSchema) {
        return DomUtils.getRealSchemasForAugment(schema, augmentSchema);
    }
}
