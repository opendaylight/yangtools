        ...
        if (data instanceof NormalizedNodeContainer) {
            ...
            if (modification.getOperation() == LogicalOperation.WRITE) {
                ...
                for (final NormalizedNode<PathArgument, ?> child : oldDataContainer.getValue()) {
                    ...
                    final LogicalOperation childOper;
                    if (maybeChild.isPresent()) {
                        childOper = maybeChild.get().getOperation();
                    } else {
                        childOper = LogicalOperation.NONE;
                    }

                    // Overwrite NONE (a.k.a. not present) and TOUCH operations only
                    if (childOper == LogicalOperation.NONE || childOper == LogicalOperation.TOUCH) {
