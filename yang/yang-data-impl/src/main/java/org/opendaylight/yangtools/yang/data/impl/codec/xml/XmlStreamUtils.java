...
public class XmlStreamUtils {
    ...
    private void writeAttributes(final XMLStreamWriter writer, final AttributesContainer data, final RandomPrefix randomPrefix) throws XMLStreamException {
        for (Entry<QName, String> attribute : data.getAttributes().entrySet()) {
            writeAttribute(writer, attribute, randomPrefix);
        }
    }

    private boolean hasAttributes(final Node<?> data) {
        return data instanceof AttributesContainer && ((AttributesContainer) data).getAttributes() != null;
    }
