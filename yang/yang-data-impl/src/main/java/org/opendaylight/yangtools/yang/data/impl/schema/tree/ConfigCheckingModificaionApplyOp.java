...
public class ConfigCheckingModificaionApplyOp extends ModificationApplyOperation {

    private final DataSchemaNode schema;
    private final ModificationApplyOperation delegate;

    public ConfigCheckingModificaionApplyOp(DataSchemaNode schema, ModificationApplyOperation delegate) {
        this.schema = schema;
    ...
    void verifyStructure(final ModifiedNode modification) throws IllegalArgumentException {
        ...
        if (modification.getOperation().equals(LogicalOperation.WRITE)) {
            checkConfigStatementsForData(schema, modification.getWrittenValue());
        }
    }
    ...
    public Optional<ModificationApplyOperation> getChild(final YangInstanceIdentifier.PathArgument child) {
        ...
        if (childModOpPotential.isPresent()) {
            ...
            return Optional.<ModificationApplyOperation>of(new ConfigCheckingModificaionApplyOp(childSchema, childModOp));
        } else {
