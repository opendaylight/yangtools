...
public class LeafRefContextTreeBuilder {
    ...
    public LeafRefContext buildLeafRefContextTree() throws IOException,
        ...
        LeafRefContextBuilder rootBuilder = new LeafRefContextBuilder(
    ...
    private LeafRefContext buildLeafRefContextReferencingTree(
            DataSchemaNode node, Module currentModule) throws IOException,
            LeafRefYangSyntaxErrorException {
        ...
        if (node instanceof DataNodeContainer) {
            ...
            if (type instanceof Leafref) {
                ...
                String leafRefPathString = leafrefType.getPathStatement()
                        .toString();
                ...
                LeafRefPathParserImpl leafRefPathParser = new LeafRefPathParserImpl(
                        schemaContext, currentModule, node);

                ByteArrayInputStream leafRefPathInputStream = new ByteArrayInputStream(
                        leafRefPathString.getBytes(Charset.forName("UTF-8")));
                LeafRefPath leafRefPath = leafRefPathParser
                        .parseLeafRefPathSourceToSchemaPath(leafRefPathInputStream);

                currentLeafRefContextBuilder.setLeafRefTargetPath(leafRefPath);

                LeafRefContext currentLeafRefContext = currentLeafRefContextBuilder
                        .build();
    ...
    private LeafRefContext buildLeafRefContextReferencedByTree(
        ...
        if (node instanceof DataNodeContainer) {
            ...
            LinkedList<LeafRefContext> foundLeafRefs = getLeafRefsFor(node,
                    currentModule);
            if (!foundLeafRefs.isEmpty()) {
                ...
                for (LeafRefContext leafRef : foundLeafRefs) {
                    currentLeafRefContextBuilder.addReferencedByLeafRefCtx(
                            leafRef.getCurrentNodeQName(), leafRef);
                }
            }
        }
    ...
    private LinkedList<LeafRefContext> getLeafRefsFor(DataSchemaNode node,
            Module module) {
        LeafRefPath nodeXPath = LeafRefUtils.schemaPathToLeafRefPath(
                node.getPath(), module);

        LinkedList<LeafRefContext> foundLeafRefs = new LinkedList<LeafRefContext>();

        for (LeafRefContext leafref : leafRefs) {
            LeafRefPath leafRefTargetPath = leafref
