...
public class DataTreeCandidateValidator {
    ...
    private void validateNode(DataTreeCandidateNode node,
        ...
        for (DataTreeCandidateNode childNode : childNodes) {
            ModificationType modificationType = childNode.getModificationType();

            if (modificationType != ModificationType.UNMODIFIED) {

                LeafRefContext childReferencedByCtx = getReferencedByCtxChild(
                        referencedByCtx, childNode);
                LeafRefContext childReferencingCtx = getReferencingCtxChild(
    ...
    private LeafRefContext getReferencingCtxChild(
            LeafRefContext referencingCtx, DataTreeCandidateNode childNode) {

        LeafRefContext childReferencingCtx = null;
        if (referencingCtx != null) {
            PathArgument identifier = childNode.getIdentifier();
            QName childQName = identifier.getNodeType();
            ...
            if (childReferencingCtx == null) {
                NormalizedNode<?, ?> data = childNode.getDataAfter().get();
    ...
    private LeafRefContext getReferencedByCtxChild(
            LeafRefContext referencedByCtx, DataTreeCandidateNode childNode) {
    ...
    private LeafRefContext findReferencedByCtxUnderChoice(
        ...
        Set<Entry<QName, LeafRefContext>> childs = referencedByChilds
                .entrySet();
        for (Entry<QName, LeafRefContext> child : childs) {
    ...
    private StringBuilder createInvalidLeafRefMessage(
            NormalizedNode<?, ?> leaf, LeafRefContext referencingCtx,
            HashSet values) {
        StringBuilder sb = new StringBuilder();
        sb.append("Invalid leafref value [");
