...
abstract class SchemaAwareApplyOperation extends ModificationApplyOperation {
    ...
    protected void checkWriteApplicable(final YangInstanceIdentifier path, final NodeModification modification,
        ...
        if (original.isPresent() && current.isPresent()) {
            ...
            LOG.debug("current tree node indetifier: {}" + treeNode.getIdentifier());
            LOG.debug("current tree node data: {}", treeNode.getData());
            LOG.debug("current tree node version: {}", treeNode.getVersion());
            LOG.debug("modification identifier: {}", modification.getIdentifier());
            LOG.debug("modification operation: {}", modification.getOperation());
            LOG.debug("modification original: {}", modification.getOriginal());
