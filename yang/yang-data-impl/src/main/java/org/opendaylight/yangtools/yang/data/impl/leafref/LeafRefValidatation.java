...
public class LeafRefValidatation {
    ...
    private void validateLeafRefTargetNodeData(final NormalizedNode<?, ?> leaf,
        ...
        final Collection<LeafRefContext> leafrefs = allReferencedByLeafRefCtxs.values();
    ...
    private void leafRefTargetNodeDataLog(final NormalizedNode<?, ?> leaf, final LeafRefContext referencedByCtx,
                                            ...
                                            final Map<LeafRefContext, Set> leafRefsValues,
        ...
        if (leafRefsValues != null && !leafRefsValues.isEmpty()) {
            ...
            LOG.debug("Operation [" + modificationType + "] validate data of leafref TARGET node: name[" +
                    referencedByCtx.getNodeName() + "] = value[" + leaf.getValue() + "]");
            for (final Entry<LeafRefContext, Set> entry : entrySet) {
                ...
                for (final Object leafRefsValue : leafRefValuesSet) {
                    if (leafRefTargetNodeValues != null && !leafRefTargetNodeValues.contains(leafRefsValue)) {
                        LOG.debug("Invalid leafref value [" + leafRefsValue + "] allowed values " +
                                leafRefTargetNodeValues + " by validation of leafref TARGET node: " + leaf
                                .getNodeType() + " path of invalid LEAFREF node: " + leafRefContext
                                .getCurrentNodePath() + " leafRef target path: " + leafRefContext
                                .getAbsoluteLeafRefTargetPath() + FAILED);
                        errorsMessages.add("Invalid leafref value [" + leafRefsValue + "] allowed values " +
                                leafRefTargetNodeValues + " by validation of leafref TARGET node: " + leaf
                                .getNodeType() + " path of invalid LEAFREF node: " + leafRefContext
                                .getCurrentNodePath() + " leafRef target path: " + leafRefContext
                                .getAbsoluteLeafRefTargetPath() + FAILED);
            ...
            LOG.debug("Operation [" + modificationType + "] validate data of leafref TARGET node: name[" +
                    referencedByCtx.getNodeName() + "] = value[" + leaf.getValue() + "] -> SKIP: Already validated");
        }
    }
    ...
    private void validateLeafRefNodeData(final NormalizedNode<?, ?> leaf, final LeafRefContext referencingCtx,
        ...
        if (!values.contains(leaf.getValue())) {
            LOG.debug("Operation [" + modificationType + "] validate data of LEAFREF node: name[" + referencingCtx
                    .getNodeName() + "] = value[" + leaf.getValue() + ']' + FAILED);
            LOG.debug("Invalid leafref value [" + leaf.getValue() + "] allowed values " + values + " of LEAFREF node:" +
                    " " + leaf.getNodeType() + " leafRef target path: " + referencingCtx.getAbsoluteLeafRefTargetPath());
            errorsMessages.add("Invalid leafref value [" + leaf.getValue() + "] allowed values " + values + " of LEAFREF node:" +
                    " " + leaf.getNodeType() + " leafRef target path: " + referencingCtx.getAbsoluteLeafRefTargetPath());
        } else {
            LOG.debug("Operation [" + modificationType + "] validate data of LEAFREF node: name[" + referencingCtx
                    .getNodeName() + "] = value[" + leaf.getValue() + ']' + SUCCESS);
        }
    }
