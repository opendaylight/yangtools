...
final class InMemoryDataTreeModification implements DataTreeModification {
    ...
    public synchronized Optional<NormalizedNode<?, ?>> readNode(final InstanceIdentifier path) {
        if (modifications != null) {
            ...
            Entry<InstanceIdentifier, ModifiedNode> matchModif =
                    TreeNodeUtils.findClosestsOrFirstMatch(modifications, path, ModifiedNode.IS_TERMINAL_PREDICATE);

            if (matchModif.getValue() != null && matchModif.getValue().getWrittenValue() != null) {
                NormalizedNode<?, ?> returnVal = matchModif.getValue().getWrittenValue();
                return Optional.<NormalizedNode<?, ?>> of(returnVal);
            }
    ...
    private OperationWithModification resolveModificationFor(final InstanceIdentifier path) {
        ...
        for (PathArgument pathArg : path.getPathArguments()) {
            modifications = this.getRootModification().modifyChild(pathArg);
        }
        return OperationWithModification.from(operation, modifications);
    }
