...
final class InMemoryDataTreeModification implements DataTreeModification {
    ...
    public synchronized Optional<NormalizedNode<?, ?>> readNode(final InstanceIdentifier path) {
        InstanceIdentifier rootPath = InstanceIdentifier.of(snapshot.getRootNode().getKey());
        final Optional<InstanceIdentifier> relativePath = path.relativeTo(rootPath);
        if (relativePath.isPresent()) {
            return Optional.<NormalizedNode<?, ?>> of(snapshot.getRootNode());
        } else {
