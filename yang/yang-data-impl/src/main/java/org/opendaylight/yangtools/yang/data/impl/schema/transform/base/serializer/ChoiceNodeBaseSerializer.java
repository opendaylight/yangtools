...
public abstract class ChoiceNodeBaseSerializer<E>
    ...
    protected Set<DataSchemaNode> getRealSchemasForAugment(org.opendaylight.yangtools.yang.model.api.ChoiceNode schema, AugmentationSchema augmentSchema) {
        ...
        aggregatedSchemas.addAll(SchemaUtils.getRealSchemasForAugment(schema, augmentSchema));

        for (ChoiceCaseNode choiceCaseNode : schema.getCases()) {
            aggregatedSchemas.addAll(SchemaUtils.getRealSchemasForAugment((AugmentationTarget) choiceCaseNode, augmentSchema));
        }
