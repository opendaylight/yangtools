...
final class UniqueIndex implements TreeNodeIndex{
    ...
    protected static TreeNodeIndex createFromData(final List<YangInstanceIdentifier> uniqueLeafIdentifiers, final NormalizedNode<?, ?> data) {
    ...
    private void updateEntry(final MapEntryNode data) {
        ...
        Preconditions.checkArgument(!indexData.isPresent(),
                "Node %s violates unique constraint. Stored node %s already contains the same value combination of leafs: %s",
                data.getIdentifier(), indexData.orNull(), uniqueLeafIdentifiers);
