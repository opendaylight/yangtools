...
abstract class AbstractMapModificationStrategy extends AbstractNodeContainerModificationStrategy {
    ...
    protected AbstractMapModificationStrategy(final ListSchemaNode schema, final Class<? extends NormalizedNode<?, ?>> nodeClass,
        ...
        uniqueEnforcer = UniqueConstraintEnforcer.forList(schema, treeConfig);
    }
    ...
    protected TreeNode applyWrite(final ModifiedNode modification,
        ...
        final Map<List<YangInstanceIdentifier>, TreeNodeIndex> uniqueIndexes = uniqueEnforcer.createIndexesFromData(newValue);
    ...
    private TreeNode mutateChildren(final TreeNode currentMeta,
        ...
        Map<List<YangInstanceIdentifier>, TreeNodeIndex> treeNodeIndexes = mutable.getIndexes();
        if(treeNodeIndexes.isEmpty() && !uniqueEnforcer.getUniqueConstraintsLeafIds().isEmpty()){
            treeNodeIndexes = uniqueEnforcer.createIndexesFromData(currentMeta.getData());
        }
        for (final ModifiedNode mod : modifications) {
            ...
            if (result.isPresent()) {
                ...
                uniqueEnforcer.updateIndexes(treeNodeIndexes, tn.getData());
            } else {
                mutable.removeChild(id);
                data.removeChild(id);
                uniqueEnforcer.removeFromIndexes(treeNodeIndexes, id);
            }
        }

        mutable.setData(data.build());
        //FIXME: try to refactor to Imutable and Mutable indexes like as TreeNode and MutableTreeNode
        //not required now: meta.setIndexes(treeNodeIndexes);
        return mutable.seal();
    }
}
