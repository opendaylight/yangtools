...
abstract class NormalizedNodeContainerModificationStrategy extends SchemaAwareApplyOperation {
    ...
    // TODO: move this to validator class along with other validation methods
    public static <T extends DataSchemaNode> void checkMinMaxElements(YangInstanceIdentifier path, T schema, ModifiedNode modification,
        ...
        if (modification.getWrittenValue() != null) {
            ...
            // TODO: implement getNumOfChildren for NormalizedNodeContainer ?
            childrenAfter = Iterables.size(children);
        }
        ...
        int childrenBefore = current.isPresent() ?
                Iterables.size(((NormalizedNodeContainer) current.get().getData()).getValue()) : 0;
