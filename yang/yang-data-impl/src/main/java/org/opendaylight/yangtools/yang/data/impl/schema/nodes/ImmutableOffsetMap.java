 ...
 * @param <V> Value type
 */
public class ImmutableOffsetMap<K, V> extends AbstractMap<K, V> implements Cloneable, CloneableMap<K, V>, Serializable {
    private static final long serialVersionUID = 1L;

    private final Map<K, Integer> offsets;
    private final V[] array;

    ImmutableOffsetMap(@Nonnull final Map<K, Integer> offsets, @Nonnull final V[] array) {
        this.offsets = Preconditions.checkNotNull(offsets);
        this.array = Preconditions.checkNotNull(array);
        Preconditions.checkArgument(offsets.size() == array.length);
    }
     ...
     * It also recognizes {@link MutableOffsetMap} (as returned by {@link #createMutableClone()}) and makes an efficient
    ...
    @Nonnull public static <K, V> Map<K, V> copyOf(@Nonnull final Map<K, V> m) {
        ...
        if (m instanceof MutableOffsetMap) {
            return ((MutableOffsetMap<K, V>) m).immutableCopy();
        }
    ...
    public final V get(final Object key) {
        ...
        return getValue((K) key, array[offset]);
    }
    ...
    @Override
    public MutableOffsetMap<K, V> createMutableClone() {
        return new MutableOffsetMap<>(offsets, array);
    }

    @Override
    public final ImmutableOffsetMap<K, V> clone() {
        return new ImmutableOffsetMap<>(offsets, array);
    }

    protected V getValue(final K key, final V value) {
        return value;
    }
        ...
        public Iterator<Entry<K, V>> iterator() {
            ...
            return new UnmodifiableIterator<Entry<K, V>>() {
                ...
                public Entry<K, V> next() {
                    ...
                    return new SimpleEntry<>(e.getKey(), getValue(e.getKey(), array[e.getValue()]));
                }
            };
        }
    ...
    private static final Field ARRAY_FIELD = fieldFor("array");
