 ...
 *
 * For ContainersWe do not use {@link PresenceContainerModificationStrategy} because
...
final class StructuralModificationStrategyWrapper extends ModificationApplyOperation {
    ...
    void checkApplicable(final YangInstanceIdentifier path, final NodeModification modification,
        ...
        if (modification.getOperation() == LogicalOperation.TOUCH && !current.isPresent()) {
            ...
            // Getting here means modification is happening under a structural parent that's not providing accurate
            // current input param and it's not possible to get the real current node value here
