...
public class LeafNodeDomSerializer {

    public Element toDomElement(LeafSchemaNode schema, LeafNode<?> node) {
        return toDomElement(schema, node, DomUtils.defaultValueCodecProvider());
    }

    public Element toDomElement(LeafSchemaNode schema, LeafNode<?> node, XmlCodecProvider codecProvider) {
        Document doc = XmlDocumentUtils.getDocument();
        return toDomElement(schema, node, codecProvider, doc);
    }

    public Element toDomElement(LeafSchemaNode schema, LeafNode<?> node, XmlCodecProvider codecProvider, Document doc) {
