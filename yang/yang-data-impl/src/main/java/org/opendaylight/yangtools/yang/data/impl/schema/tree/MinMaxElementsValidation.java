...
public class MinMaxElementsValidation extends SchemaAwareApplyOperation {
    ...
    protected MinMaxElementsValidation(final SchemaAwareApplyOperation delegate, final Integer minElements,
    ...
    private void checkMinMaxElements(final YangInstanceIdentifier path,
            final NodeModification nodeMod, final Optional<TreeNode> current) throws DataValidationFailedException {
        ...
        if (minElements != null && minElements > childrenTotal) {
            throw new DataValidationFailedException(path, String.format(
                    "%s does not have enough elements (%s), needs at least %s", modification.getIdentifier(), childrenTotal,
                    minElements));
        }
        if (maxElements != null && maxElements < childrenTotal) {
            throw new DataValidationFailedException(path,
                    String.format("%s has too many elements (%s), can have at most %s", modification.getIdentifier(),
                            childrenTotal, maxElements));
        }
    }
    ...
    @Override
    protected TreeNode applyWrite(final ModifiedNode modification, final Optional<TreeNode> currentMeta, final Version version) {
