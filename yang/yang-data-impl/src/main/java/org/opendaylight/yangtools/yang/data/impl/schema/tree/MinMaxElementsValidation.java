...
final class MinMaxElementsValidation extends SchemaAwareApplyOperation {
    ...
    MinMaxElementsValidation(final SchemaAwareApplyOperation delegate,
            final ElementCountConstraint elementCountConstraint) {
        this.delegate = Preconditions.checkNotNull(delegate);
        this.minElements = elementCountConstraint.getMinElements();
        this.maxElements = elementCountConstraint.getMaxElements();
    }

    static SchemaAwareApplyOperation from(final SchemaAwareApplyOperation delegate, final DataSchemaNode schema) {
        if (!(delegate instanceof ElementCountConstraintAware)) {
            return delegate;
        }
        final Optional<ElementCountConstraint> optConstraint = ((ElementCountConstraintAware) delegate)
