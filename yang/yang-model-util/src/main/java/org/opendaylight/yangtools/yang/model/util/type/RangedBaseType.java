...
abstract class RangedBaseType<T extends TypeDefinition<T>> extends BaseType<T> {
    private final List<RangeConstraint> rangeConstraints;

    RangedBaseType(final QName qname, final Number minValue, final Number maxValue) {
        super(qname);
    ...
    RangedBaseType(final SchemaPath path, final List<RangeConstraint> rangeConstraints) {
        super(path);
