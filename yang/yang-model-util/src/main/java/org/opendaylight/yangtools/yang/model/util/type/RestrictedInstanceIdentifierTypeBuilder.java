...
final class RestrictedInstanceIdentifierTypeBuilder extends InstanceIdentifierTypeBuilder {
    RestrictedInstanceIdentifierTypeBuilder(final InstanceIdentifierTypeDefinition baseType, final SchemaPath path) {
        super(Preconditions.checkNotNull(baseType), path);
    }

    @Override
    public void setRequireInstance(final boolean requireInstance) {
        if (getRequireInstance()) {
            Preconditions.checkArgument(!requireInstance, "Cannot switch require-instance off in type %s", getPath());
        }
        super.setRequireInstance(requireInstance);
    }

    @Override
    public RestrictedInstanceIdentifierType build() {
