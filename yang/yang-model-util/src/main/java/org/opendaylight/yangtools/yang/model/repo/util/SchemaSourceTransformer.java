...
public class SchemaSourceTransformer<S extends SchemaSourceRepresentation, D extends SchemaSourceRepresentation> implements SchemaSourceListener, SchemaSourceProvider<D> {
    ...
    private final Map<SourceIdentifier, RefcountedRegistration> sources = new HashMap<>();
    ...
    private void registerSource(final PotentialSchemaSource<?> src) {
        RefcountedRegistration reg = sources.get(identifier);
        if (reg != null) {
            reg.incRef();
            return;
        }

        final SchemaSourceRegistration<D> r = consumer.registerSchemaSource(identifier, this, dstClass, 0 + PotentialSchemaSource.Costs.COMPUTATION.getValue());
        sources.put(identifier, new RefcountedRegistration(r));
    }

    private void unregisterSource(final PotentialSchemaSource<?> src) {
        final RefcountedRegistration reg = sources.get(identifier);
