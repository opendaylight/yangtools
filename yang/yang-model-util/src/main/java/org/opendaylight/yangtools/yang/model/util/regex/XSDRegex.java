...
import org.opendaylight.yangtools.yang.model.util.regex.regexParser.CharGroupPartContext;
...
public final class XSDRegex {
    ...
    private static void addCharClassExpr(final StringBuilder sb, final CharClassExprContext charClassExpr) {
        ...
        if (group instanceof NegCharGroupContext) {
            checkState(group.getChildCount() == 2, "Unsupported group shape %s", group);
            addPosCharGroup(sb.append("[^"), getChild(group, 1, PosCharGroupContext.class));
        } else {
            checkState(group instanceof PosCharGroupContext, "Unsupported group %s", group);
            addPosCharGroup(sb.append('['), (PosCharGroupContext) group);
        }

        if (charGroup.getChildCount() == 3) {
            // FIXME: handle dash + charClassExpr
        }
    ...
    private static void addPosCharGroup(final StringBuilder sb, final PosCharGroupContext group) {
        ...
        for (ParseTree child : group.children) {
            checkState(child instanceof CharGroupPartContext, "Unsupported group child %s", child);
            final CharGroupPartContext part = (CharGroupPartContext) child;
