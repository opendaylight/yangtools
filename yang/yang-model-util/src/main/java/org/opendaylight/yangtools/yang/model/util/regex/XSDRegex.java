...
public final class XSDRegex {
    ...
    private static void addCharClassExpr(final StringBuilder sb, final CharClassExprContext charClassExpr) {
        ...
        if (group instanceof PosCharGroupContext) {
            ...
            checkState(group instanceof TerminalNode, "Unsupported group %s", group);
            checkState(((TerminalNode) group).getSymbol().getType() == regexLexer.DASH, "Unsupported group terminal %s",
                    group);
            sb.append('-');
        }
