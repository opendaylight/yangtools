...
public final class XSDRegex {
    ...
    private static void addCharClass(final StringBuilder sb, final CharClassContext charClass) {
        ...
        if (child instanceof CharClassEscContext) {
            ...
            checkState(type == regexLexer.SingleCharEsc || type == regexLexer.WildcardEsc, "Unsupported terminal %s",
                    child);
            sb.append(child.getText());
        } else {
            throw new IllegalStateException("Unsupported charClass " + child.toStringTree());
        }
    }
    ...
    private static void addCharClassEsc(final StringBuilder sb, final CharClassEscContext charClassEsc) {
        ...
        if (child instanceof CatEscContext) {
        ...
        } else if (child instanceof ComplEscContext) {
            addEsc(sb, (ComplEscContext) child, true);
        } else if (child instanceof TerminalNode) {
            final TerminalNode charEsc = (TerminalNode) child;
            checkState(charEsc.getSymbol().getType() == regexLexer.MultiCharEsc, "Unsupported charClass type %s",
                    charEsc);
            final String text = charEsc.getText();
            ...
            throw new IllegalStateException("Unsupported escape " + child.toStringTree());
        }
    }
    ...
    private static void addEsc(final StringBuilder sb, final ParserRuleContext esc, final boolean complement) {
        ...
        switch (((TerminalNode) prop).getSymbol().getType()) {
            ...
            case regexLexer.IsBlock:
                ...
                break;
            default:
                throw new IllegalStateException("Unsupported property type " + prop);
        }
    }
    ...
    private static void addCharClassExpr(final StringBuilder sb, final CharClassExprContext charClassExpr) {
        ...
        if (group instanceof PosCharGroupContext) {
            // FIXME: this needs to be decomposed down, as CatEsc and ComplEsc need to be transformed
            sb.append(group.getText());
        } else if (group instanceof NegCharGroupContext) {
            // FIXME: this needs to be decomposed down, as CatEsc and ComplEsc need to be transformed
            sb.append(group.getText());
        } else {
            throw new IllegalStateException("Unsupported group " + group.toStringTree());
        }
        sb.append(']');
    }
