...
@Immutable
public class FilteringSchemaContextProxy extends AbstractSchemaContext {
    ...
    ImmutableListMultimap<String, Module> nameToModule;
     ...
     *
     * @param delegate
     * @param moduleIds modules - yang schemas to be available and all their dependencies
    ...
    public FilteringSchemaContextProxy(final SchemaContext delegate, final Collection<ModuleId> moduleIds, final Collection<ModuleId> additionalModuleIds) {

        Preconditions.checkArgument(!(moduleIds==null),"Base modules cannot be null" );
        Preconditions.checkArgument(!(additionalModuleIds==null),"Additional modules cannot be null" );
        Preconditions.checkArgument(!(moduleIds.isEmpty() && additionalModuleIds.isEmpty()), "Arguments are empty. Nothing to do.");
        ...
        nameToModule = getStringModuleImmutableMap(delegate);
        ...
        if(!moduleIds.isEmpty()) {
            filteredModulesBuilder.addAll(Collections2.filter(delegate.getModules(), new Predicate<Module>() {
                ...
                public boolean apply(@Nullable Module module) {
                    return checkModuleDependency(module, moduleIds);
                }
            }));
        }
        ...
        //modules which
        /*
        if(additionalModuleIds!=null && additionalModuleIds.size()>0) {
        ...
        */
        ...
        //this.moduleDependencyName = moduleDependencyName;
    ...
    private ImmutableListMultimap<String, Module> getStringModuleImmutableMap(SchemaContext delegate) {
        ...
        for (Module module : baseModules) {
            for (ModuleImport moduleImport : module.getImports()) {
                ...
                if(moduleImport.getRevision()==null) {
                    ...
                    ImmutableList mods = nameToModule.get(moduleImport.getModuleName());
                    revisionDate = getLatestModuleRevisionDate(mods);
                } else {
                ...
                //for (Map.Entry<ModuleId, Module> entry : allModules.entrySet())
                //{
                //    System.out.println(entry.getKey() + "/" + entry.getValue());
                //}

                System.err.println("Import: Base module:"+module.getName()+",Imported Module:"+moduleImport.getModuleName()+",Revision: "+moduleImport.getRevision());
    ...
    private Date getLatestModuleRevisionDate(ImmutableList mods) {
        Date maxDate = null;

        for (Object obj : mods){

            if(maxDate==null||((Module) obj).getRevision().after(maxDate)){
                maxDate = ((Module) obj).getRevision();
            }
    ...
    public boolean checkModuleDependency(Module module, Collection<ModuleId> moduleIds) {
        ...
        for (ModuleId moduleId : moduleIds) {
            ...
            //if(mod.getName().equals(moduleDependencyName) && revision.equals(mod.getRevision())) {
            ...
            for (ModuleImport moduleImport : module.getImports()) {
                ...
                if(moduleImport.getModuleName().equals(moduleDependencyName)) {
                    ...
                    //the case when root module revision = null, selecting dependant module with highest revision but it's very rare
                        ...
                        Date maxDate = getLatestModuleRevisionDate(mods);
