...
public final class SchemaInferenceStack implements Mutable, EffectiveModelContextProvider {
    ...
    private @NonNull EffectiveStatement<QName, ?> resolveDeref(final DerefSteps deref) {
        final EffectiveStatement<QName, ?> leafRefSchemaNode = currentStatement();
        final YangLocationPath.Relative derefArg = deref.getDerefArgument();
        final EffectiveStatement<QName, ?> derefStmt = resolveLocationPath(derefArg);
        ...
        EffectiveStatement<QName, ?> derefNode = resolvePathExpression(dereferencedLeafRefPath);
    ...
    private @NonNull EffectiveStatement<QName, ?> resolveLocationPath(final YangLocationPath path) {
        ...
        final QNameModule module = deque.isEmpty() ? null : deque.peek().argument().getModule();
        if (path.isAbsolute()) {
            clear();
        }
    ...
    private @NonNull EffectiveStatement<QName, ?> enterChild(final Step step, final QNameModule module) {
    ...
    private @NonNull EffectiveStatement<QName, ?> enterParent(final Step step) {
        ...
        final EffectiveStatement<QName, ?> exit = exit();
        final EffectiveStatement<QName, ?> peek = deque.peek();
        if (peek == null || peek instanceof GroupingEffectiveStatement) {
            throw new IllegalArgumentException("Trying to illegally enter parent. LeafRef has invalid path.");
        }
        return exit;
    }
