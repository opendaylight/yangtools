...
public final class SchemaInferenceStack implements Mutable, EffectiveModelContextProvider {
    ...
    private void resolveSchemaTreeSteps(final @NonNull SchemaTreeAwareEffectiveStatement<?, ?> parent,
        ...
        for (EffectiveStatement<?, ?> stmt : parent.effectiveSubstatements()) {
            if (stmt instanceof ChoiceEffectiveStatement) {
                final ChoiceEffectiveStatement choice = (ChoiceEffectiveStatement) stmt;
                match.push(choice);
                if (findDataTreeChild(match, choice, nodeIdentifier)) {
    ...
    private static boolean findDataTreeChild(final @NonNull Deque<EffectiveStatement<QName, ?>> result,
            final @NonNull SchemaTreeAwareEffectiveStatement<?, ?> parent, final @NonNull QName nodeIdentifier) {
        for (EffectiveStatement<?, ?> stmt : parent.effectiveSubstatements()) {
            ...
            if (stmt instanceof SchemaTreeEffectiveStatement && stmt instanceof SchemaTreeAwareEffectiveStatement) {
                result.push((SchemaTreeEffectiveStatement<?>) stmt);
                if (findDataTreeChild(result, (SchemaTreeAwareEffectiveStatement<?, ?>) stmt, nodeIdentifier)) {
                    return true;
                }
                result.pop();
            }
