...
public interface StmtContext<A, D extends DeclaredStatement<A>, E extends EffectiveStatement<A, D>> {
    ...
    default <V, T extends EffectiveStatement<V, ?>> Optional<V> findFirstSubstatementArgument(final @NonNull Class<T> type) {
        return allSubstatementsStream()
                .filter(o -> ((StmtContext) o).producesEffective(type))
                .findFirst()
                .map(StmtContext::coerceParentContext)
                .map(type::cast)
