...
final class FileGeneratorTask extends AbstractGeneratorTask {
    ...
    Collection<File> execute(final BuildContext buildContext) throws IOException {
        ...
        for (Cell<GeneratedFileKind, GeneratedFilePath, GeneratedFile> cell : generatedFiles.cellSet()) {
            final File file = new File(baseFile(cell.getRowKey(), cell.getValue().getLifecycle()),
                String.join(File.separator, cell.getColumnKey().getComponents()));
            cell.getValue().copyTo(buildContext.newFileOutputStream(file));
            ret.add(file);
        }
