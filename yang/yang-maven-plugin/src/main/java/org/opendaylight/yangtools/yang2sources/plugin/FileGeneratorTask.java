...
final class FileGeneratorTask extends AbstractGeneratorTask {
    ...
    FileGeneratorTask(final FileGeneratorFactory factory, final FileGeneratorArg arg) {
    ...
    Collection<File> execute(final BuildContext buildContext) throws IOException {
        final Map<GeneratedFileSpec, GeneratedFile> generatedFiles = gen.generateFiles(
    ...
    private static File baseFile(final GeneratedFileType type, final GeneratedFileLifecycle lifecycle) {
        final String path;
        switch (kind) {
            case RESOURCE:
                switch (lifecycle) {
