...
class YangToSourcesProcessor {
    ...
    private ContextHolder processYang() throws MojoExecutionException {
        ...
        final Collection<File> projectModelPaths = StreamSupport.stream(
            Files.fileTreeTraverser().children(yangFilesRootDir).spliterator(), false).filter(File::isFile).filter(
                f -> f.getName().endsWith(YangConstants.RFC6020_YANG_FILE_EXTENSION) && !excludedFiles.contains(f))
