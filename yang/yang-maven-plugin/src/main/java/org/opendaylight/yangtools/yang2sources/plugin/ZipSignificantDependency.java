...
final class ZipSignificantDependency extends SignificantDependency {
    ...
    @Override
    Collection<ByteSource> asSources() throws IOException {
        final Collection<ByteSource> ret = new ArrayList<>(entries.size());
        for (ZipEntry e : entries) {
            ret.add(ByteSource.wrap(ByteStreams.toByteArray(zip.getInputStream(e))));
        }
        return ret;
    }
