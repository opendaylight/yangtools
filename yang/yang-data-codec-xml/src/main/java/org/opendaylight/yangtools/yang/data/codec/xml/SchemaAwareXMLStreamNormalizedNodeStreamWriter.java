...
final class SchemaAwareXMLStreamNormalizedNodeStreamWriter
    ...
    String encodeValue(final ValueWriter xmlWriter, final Object value, final TypedDataSchemaNode schemaNode)
        ...
        return streamUtils.encodeValue(xmlWriter, resolveType(schemaNode), value, schemaNode.getQName().getModule());
    }
    ...
    String encodeAnnotationValue(final ValueWriter xmlWriter, final QName qname, final Object value)
        ...
        if (optAnnotation.isPresent()) {
            return streamUtils.encodeValue(xmlWriter, resolveType(optAnnotation.get()), value, qname.getModule());
        }
    ...
    private @NonNull TypeDefinition<?> resolveType(final @NonNull TypeAware node) throws XMLStreamException {
        final TypeDefinition<?> type = node.getType();
        if (type instanceof LeafrefTypeDefinition) {
            final EffectiveStatement<?, ?> stmt = tracker.toSchemaInferenceStack()
                .resolvePathExpression(((LeafrefTypeDefinition) type).getPathStatement());
            if (stmt instanceof TypeAware) {
                return ((TypeAware) stmt).getType();
            }
            throw new XMLStreamException("Type " + type + " resolved to unsupported " + stmt);
        }
        return type;
    }
}
