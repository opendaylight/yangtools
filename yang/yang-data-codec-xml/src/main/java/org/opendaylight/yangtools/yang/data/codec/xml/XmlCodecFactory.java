...
public final class XmlCodecFactory {
    ...
    private final LoadingCache<Map.Entry<DataSchemaNode, NamespaceContext>, XmlCodec<?>> codecs =
            CacheBuilder.newBuilder().softValues().build(
                    new CacheLoader<Map.Entry<DataSchemaNode, NamespaceContext>, XmlCodec<?>>() {
                @Override
                public XmlCodec<?> load(final Map.Entry<DataSchemaNode, NamespaceContext> schemaNodeAndNamespaceCtxPair)
                        throws Exception {
    ...
    XmlCodec<?> codecFor(final DataSchemaNode schema, final NamespaceContext namespaceContext) {
        return codecs.getUnchecked(Maps.immutableEntry(schema, namespaceContext));
    }
}
