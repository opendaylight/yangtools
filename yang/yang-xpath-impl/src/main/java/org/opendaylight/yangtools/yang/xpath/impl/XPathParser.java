...
abstract class XPathParser {
    ...
    public YangExpr parseExpr(final ExprContext expr) {
        ...
        return ors.size() == 1 ? parseAnd(ors.get(0))
                : new YangNaryExprBuilder().operator(YangNaryOperator.OR)
                .addAllExpressions(Lists.transform(ors, this::parseAnd))
                .build();
    }
    ...
    private YangExpr parseAnd(final AndExprContext expr) {
        ...
        return eqs.size() == 1 ? parseEquality(eqs.get(0))
                : new YangNaryExprBuilder().operator(YangNaryOperator.AND)
                .addAllExpressions(Lists.transform(eqs, this::parseEquality))
                .build();
    }
    ...
    private YangExpr parseFilter(final FilterExprContext expr) {
        ...
        do {
            builder.addPredicates(parsePredicate(nextContext(it, PredicateContext.class)));
        } while (it.hasNext());
    ...
    private YangLocationPath parseLocationPath(final LocationPathContext expr) {
        ...
        return builder.addAllSteps(parseRelativeLocationPath(getChild(abs, RelativeLocationPathContext.class, 1))
            .getSteps()).build();
    }
    ...
    private YangExpr parsePathExpr(final PathExprNoRootContext expr) {
        ...
        return new YangBinaryExprBuilder().leftExpr(filter).operator(parseOperator(expr.getChild(1)))
                .rightExpr(parseRelativeLocationPath(getChild(expr, RelativeLocationPathContext.class, 2))).build();
    }
    ...
    private YangExpr parsePrimary(final PrimaryExprContext expr) {
        ...
        if (first instanceof VariableReferenceContext) {
            return parseVariableReference((VariableReferenceContext) first);
        }
    ...
    private YangExpr parseVariableReference(final VariableReferenceContext expr) {
    ...
    private static YangXPathAxis parseAxis(final AxisSpecifierContext expr) {
        switch (expr.getChildCount()) {
            ...
            case 2:
                ...
                return YangXPathAxis.forString(str)
                        .orElseThrow(() -> new IllegalStateException("Unhandled axis " + str));
            default:
                throw illegalShape(expr);
        }
    }
    ...
    private static YangXPathNodeType parseNodeType(final ParseTree tree) {
        ...
        return YangXPathNodeType.forString(str)
            .orElseThrow(() -> new IllegalStateException("Unhandled node type " + str));
    }
