...
/**
 * API entry point for acquiring {@link LhotkaJSONCodecFactory} instances.
 */
public final class LhotkaJSONCodecFactorySupplier implements JSONCodecFactorySupplier {

    private static final Logger LOG = LoggerFactory.getLogger(LhotkaJSONCodecFactorySupplier.class);

    private static final class EagerCacheLoader extends CacheLoader<SchemaContext, LhotkaJSONCodecFactory> {
        private final BiFunction<SchemaContext, LhotkaJSONCodecFactory, LhotkaJSONInstanceIdentifierCodec>
            iidCodecSupplier;

        EagerCacheLoader(final BiFunction<SchemaContext, LhotkaJSONCodecFactory, LhotkaJSONInstanceIdentifierCodec>
                             iidCodecSupplier) {
            this.iidCodecSupplier = requireNonNull(iidCodecSupplier);
        }

        @Override
        public LhotkaJSONCodecFactory load(final SchemaContext key) {
            final Stopwatch sw = Stopwatch.createStarted();
            final LazyCodecCache<JSONCodec<?>> lazyCache = new LazyCodecCache<>();
            final LhotkaJSONCodecFactory lazy = new LhotkaJSONCodecFactory(key, lazyCache, iidCodecSupplier);
            ...
            return new LhotkaJSONCodecFactory(key, cache, iidCodecSupplier);
        }

        private static int requestCodecsForChildren(final LhotkaJSONCodecFactory lazy, final DataNodeContainer parent) {
            int ret = 0;
    ...
    private final BiFunction<SchemaContext, LhotkaJSONCodecFactory, LhotkaJSONInstanceIdentifierCodec> iidCodecSupplier;

    // Weak keys to retire the entry when SchemaContext goes away
    private final LoadingCache<SchemaContext, LhotkaJSONCodecFactory> precomputed;

    // Weak keys to retire the entry when SchemaContext goes away and to force identity-based lookup
    private final LoadingCache<SchemaContext, LhotkaJSONCodecFactory> shared;

    private LhotkaJSONCodecFactorySupplier(
        final BiFunction<SchemaContext, LhotkaJSONCodecFactory, LhotkaJSONInstanceIdentifierCodec> iidCodecSupplier) {
        this.iidCodecSupplier = requireNonNull(iidCodecSupplier);
        precomputed = CacheBuilder.newBuilder().weakKeys()
            .build(new LhotkaJSONCodecFactorySupplier.EagerCacheLoader(iidCodecSupplier));
        shared = CacheBuilder.newBuilder().weakKeys().build(new CacheLoader<SchemaContext, LhotkaJSONCodecFactory>() {
            @Override
            public LhotkaJSONCodecFactory load(final SchemaContext key) {
                return new LhotkaJSONCodecFactory(key, new SharedCodecCache<>(), iidCodecSupplier);
            }
        });
    }

    private static final LhotkaJSONCodecFactorySupplier INSTANCE =
        new LhotkaJSONCodecFactorySupplier(LhotkaJSONInstanceIdentifierCodec::new);

    public static LhotkaJSONCodecFactorySupplier getInstance() {
        return INSTANCE;
    }

    /**
     * Get a thread-safe, eagerly-caching {@link LhotkaJSONCodecFactory} for a SchemaContext. This method can, and will,
     ...
     * of {@link LhotkaJSONCodecFactory}, it will be completely precomputed before this method will return.
     *
     * <p>
     ...
     * @return A sharable {@link LhotkaJSONCodecFactory}
     * @throws NullPointerException if context is null
     */
    @Override
    public @NonNull LhotkaJSONCodecFactory getPrecomputed(final @NonNull SchemaContext context) {
        return verifyNotNull(precomputed.getUnchecked(context));
    }

    /**
     * Get a thread-safe, eagerly-caching {@link LhotkaJSONCodecFactory} for a SchemaContext, if it is available. This
     ...
     *
     * @param context SchemaContext instance
     * @return A sharable {@link LhotkaJSONCodecFactory}, or absent if such an implementation is not available.
     * @throws NullPointerException if context is null
     */
    @Override
    public @NonNull Optional<LhotkaJSONCodecFactory> getPrecomputedIfAvailable(final @NonNull SchemaContext context) {
        return Optional.ofNullable(precomputed.getIfPresent(context));
    }

    /**
     * Get a thread-safe, lazily-caching {@link LhotkaJSONCodecFactory} for a SchemaContext. This method can, and will,
     ...
     */
    @Override
    public @NonNull LhotkaJSONCodecFactory getShared(final @NonNull SchemaContext context) {
        return verifyNotNull(shared.getUnchecked(context));
    }

    /**
     * Create a new thread-unsafe, lazily-caching {@link LhotkaJSONCodecFactory} for a SchemaContext. This method will
     ...
     *
     * @param context SchemaContext instance
     * @return A non-sharable {@link LhotkaJSONCodecFactory}
     * @throws NullPointerException if context is null
     */
    @Override
    public @NonNull LhotkaJSONCodecFactory createLazy(final @NonNull SchemaContext context) {
        return new LhotkaJSONCodecFactory(context, new LazyCodecCache<>(), iidCodecSupplier);
    }

    /**
     * Create a simplistic, thread-safe {@link LhotkaJSONCodecFactory} for a {@link SchemaContext}. This method will
     ...
     */
    @Override
    public @NonNull LhotkaJSONCodecFactory createSimple(final @NonNull SchemaContext context) {
        return new LhotkaJSONCodecFactory(context, NoopCodecCache.getInstance(), iidCodecSupplier);
    }
}
