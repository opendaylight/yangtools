...
public abstract class JSONNormalizedNodeStreamWriter implements NormalizedNodeStreamWriter {
    ...
    private void writeXmlNode(final Node node) throws IOException {
        ...
        if (firstChildElement != null && ANYXML_ARRAY_ELEMENT_ID.equals(firstChildElement.getNodeName())) {
            writer.beginArray();
            writeArray(firstChildElement);
            writer.endArray();
        } else if (firstChildElement != null && !ANYXML_ARRAY_ELEMENT_ID.equals(firstChildElement.getNodeName())) {
            writer.beginObject();
