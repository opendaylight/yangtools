 ...
 * of a factory for creating JSON equivalents of codecs. Each instance of this object is bound to
 * a particular {@link SchemaContext}, but can be reused by multiple {@link JSONNormalizedNodeStreamWriter}s.
 */
public abstract class AbstractJSONCodecFactory extends AbstractCodecFactory<JSONCodec<?>> {

    AbstractJSONCodecFactory(final SchemaContext context, final CodecCache<JSONCodec<?>> cache) {
        super(context, cache);
    }

    /**
     * Method returns a one-off factory for the purposes of normalizing an anydata tree.
     * @param newSchemaContext new schema context
     * @return a one-off factory
     */
    abstract AbstractJSONCodecFactory rebaseTo(SchemaContext newSchemaContext);
