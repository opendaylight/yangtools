...
public final class JsonParserStream implements Closeable, Flushable {
    ...
    private void traverseAnyXmlValue(final JsonReader in, final Document doc, final Element parent) throws IOException {
        final Stack<Element> nodes = new Stack<>();
        nodes.push(parent);
        while (!nodes.isEmpty()) {
            switch (in.peek()) {
                case STRING:
                case NUMBER:
                    ...
                    nodes.lastElement().appendChild(textNode);
    ...
    private void createAnyXmlElement(final Document doc, final Stack<Element> nodes, final String element) {
        final Element childElement = doc.createElement(element);
        nodes.lastElement().appendChild(childElement);
    ...
    private void setNodeValue(final JsonReader in, final Stack<AbstractNodeDataWithSchema> nodes, final String value)
        throws IOException {
        setValue(nodes.lastElement(), value);
        if (nodes.pop() instanceof LeafListEntryNodeDataWithSchema) {
            if (in.hasNext()) {
                ...
                nodes.push(newArrayEntry(nodes.lastElement()));
            }
        } else {
            removeNamespace();
        }
    ...
    public void read(final JsonReader in, AbstractNodeDataWithSchema<?> parent) throws IOException {
        final Stack<Set<String>> nodeNamesakes = new Stack<>();
        final Stack<AbstractNodeDataWithSchema> nodes = new Stack<>();
        nodes.push(parent);
        while (!nodes.isEmpty()) {
            switch (in.peek()) {
                case STRING:
                ...
                case BEGIN_ARRAY:
                    in.beginArray();
                    if (in.hasNext()) {
                        if (nodes.lastElement() instanceof LeafNodeDataWithSchema) {
                            nodes.push(nodes.lastElement());
                        } else {
                            nodes.push(newArrayEntry(nodes.lastElement()));
                        }
                    }
                ...
                case NAME:
                    ...
                    DataSchemaNode parentSchema = nodes.lastElement().getSchema();
                    ...
                    if (!nodeNamesakes.lastElement().add(jsonElementName)) {
                    ...
                    final AbstractNodeDataWithSchema<?> newChild = ((CompositeNodeDataWithSchema<?>)
                            nodes.lastElement()).addChild(childDataSchemaNodes);
