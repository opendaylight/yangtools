 ...
 */
package org.opendaylight.controller.sal.rest.gson.helpers;
...
import org.opendaylight.controller.sal.rest.gson.helpers.IdentityValuesDTO.IdentityValue;
import org.opendaylight.controller.sal.rest.gson.helpers.IdentityValuesDTO.Predicate;
...
public class RestCodec {
    ...
    public static final class ObjectCodec implements Codec<Object, Object> {
        ...
        private ObjectCodec(final TypeDefinition<?> typeDefinition) {
            type = RestUtil.resolveBaseTypeFrom(typeDefinition);
            if (type instanceof IdentityrefTypeDefinition) {
                identityrefCodec = new IdentityrefCodecImpl();
            } else {
                identityrefCodec = null;
            }
            if (type instanceof InstanceIdentifierTypeDefinition) {
                instanceIdentifier = new InstanceIdentifierCodecImpl();
            } else {
    ...
    public static class InstanceIdentifierCodecImpl implements InstanceIdentifierCodec<IdentityValuesDTO> {
        ...
        public YangInstanceIdentifier deserialize(final IdentityValuesDTO data) {
            ...
            for (int i = 0; i < identities.size(); i++) {
                ...
                DataSchemaNode node = SchemaContextUtils.getInstance().findInstanceDataChildByNameAndNamespace(
                ...
                if (identityValue.getPredicates().isEmpty()) {
                    ...
                    if (node instanceof LeafListSchemaNode) { // predicate is value of leaf-list entry
                        ...
                        for (Predicate predicate : identityValue.getPredicates()) {
                            ...
                            DataSchemaNode listKey = SchemaContextUtils.getInstance()
    ...
    private static Module getModuleByNamespace(final String namespace) {
        URI validNamespace = resolveValidNamespace(namespace);

        Module module = null;
        module = SchemaContextUtils.getInstance().findModuleByNamespace(validNamespace);
        if (module == null) {
    ...
    private static URI resolveValidNamespace(final String namespace) {
        URI validNamespace;
        validNamespace = SchemaContextUtils.getInstance().findNamespaceByModuleName(namespace);
