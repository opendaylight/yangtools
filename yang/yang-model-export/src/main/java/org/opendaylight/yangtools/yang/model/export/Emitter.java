...
class Emitter {
    ...
    Emitter(final YangModuleWriter writer, final Map<QName, StatementDefinition> extensions) {
        this(writer, extensions,false, true);
    }

    Emitter(final YangModuleWriter writer, final Map<QName, StatementDefinition> extensions, final boolean emitInstantiated, final boolean emitUses) {
        this.writer = writer;
    ...
    private void emitTypeNode(final SchemaPath parentPath, final TypeDefinition<?> subtype) {
        ...
        if (Utils.isPrefix(parentPath.getPathFromRoot(), path.getPathFromRoot())) {
