...
@Beta
@NotThreadSafe
class SingleModuleYinStatementWriter implements StatementWriter {

    final XMLStreamWriter writer;
    final URI currentModuleNs;
    final BiMap<String, URI> prefixToNamespace;

    YangStatement currentStatement;
    ...
    private void initializeYinNamespaceInXml() {
        ...
        try {
            ...
            if (defaultNs == null) {
                writer.setDefaultNamespace(YangConstants.YIN_NAMESPACE.toString());
            } else if (!YangConstants.YIN_NAMESPACE.toString().equals(defaultNs)) {
    ...
    static final StatementWriter create(final XMLStreamWriter writer, final URI moduleNs,
            final Map<String, URI> prefixToNs) {
        return new SingleModuleYinStatementWriter(writer, moduleNs, prefixToNs);
    }

    @Override
    public void startStatement(final YangStatement statement) {
        currentStatement = Preconditions.checkNotNull(statement);
        try {
            writeStartXmlElement(statement.getStatementName());
            if (YangStatement.Module.equals(statement) || YangStatement.Submodule.equals(statement)) {
    ...
    public void writeArgument(final QName value) {
        ...
        if (currentModuleNs.equals(valueNs)) {
            writeArgument0(valueLocal);
        } else {
            final String prefix = ensureAndGetXmlNamespacePrefix(valueNs);
            final String stringValue = toPrefixedString(prefix, valueLocal);
            writeArgument0(stringValue);
        }
    }
    ...
    private void writeArgument0(final String strRep) {
        try {
            if (currentStatement.isArgumentYinElement()) {
    ...
    private @Nullable String ensureAndGetXmlNamespacePrefix(final URI namespace) {
        if(YangConstants.YANG_NAMESPACE.equals(namespace)) {
    ...
    private void declareXmlNamespaces(final Map<String, URI> prefixToNamespace) {
        try {
            writer.writeDefaultNamespace(YangConstants.YIN_NAMESPACE.toString());
