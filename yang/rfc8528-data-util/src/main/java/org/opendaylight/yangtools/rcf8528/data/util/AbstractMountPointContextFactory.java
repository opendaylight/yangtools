    ...
    protected final MountPointContext createMountPointContext(final SchemaContext schemaContext,
        ...
        return new ImmutableMountPointContext(schemaContext, ((MapNode) mountPoint).getValue().stream().map(entry -> {
            ...
            return new MountPointDefinition(
                ...
                entry.getChild(SCHEMA_REF).map(schemaRef -> {
                    checkArgument(schemaRef instanceof ChoiceNode, "Unexpected schema-ref choice %s", schemaRef);
                    return ((ChoiceNode) schemaRef).getChild(SHARED_SCHEMA).map(sharedSchema -> {
                        ...
                        checkArgument(((ChoiceNode) schemaRef).getChild(INLINE).isPresent(),
                                "Unhandled schema-ref type in %s", entry);
                        return ImmutableSet.of();
                    });
                }).orElseThrow(() -> new IllegalArgumentException("Schema-ref missing in " + entry)));
