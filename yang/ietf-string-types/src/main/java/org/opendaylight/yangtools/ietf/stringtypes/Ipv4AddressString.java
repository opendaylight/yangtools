...
public class Ipv4AddressString extends AbstractNetworkUnsigned32String<Ipv4AddressString> {
    ...
    private transient volatile byte strLen;
    ...
    public final String toCanonicalString() {
        final byte local = strLen;
        if (local != 0) {
            return ByteUtils.appendIpv4Address(new StringBuilder(local), intBits()).toString();
        }

        final String ret = ByteUtils.appendIpv4Address(new StringBuilder(15), intBits()).toString();
