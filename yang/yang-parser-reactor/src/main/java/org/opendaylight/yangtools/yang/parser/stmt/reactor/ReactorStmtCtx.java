...
abstract class ReactorStmtCtx<A, D extends DeclaredStatement<A>, E extends EffectiveStatement<A, D>>
     ...
     *
     * @param targetExecutionOrder to be executed (completed)
     * @return true if phase was successfully completed
     * @throws SourceException when an error occurred in source parsing
     */
    final boolean tryToCompletePhase(final byte targetExecutionOrder) {
        return executionOrder() >= targetExecutionOrder || doTryToCompletePhase(targetExecutionOrder);
    }

    abstract boolean doTryToCompletePhase(byte targetExecutionOrder);
