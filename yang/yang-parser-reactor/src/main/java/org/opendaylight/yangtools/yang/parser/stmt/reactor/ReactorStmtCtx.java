...
abstract class ReactorStmtCtx<A, D extends DeclaredStatement<A>, E extends EffectiveStatement<A, D>>
    ...
    final @NonNull E attachCopy(final @NonNull EffectiveStatementState state, final @NonNull E copy) {
        ...
        if (!(effective instanceof EffectiveInstances)) {
            // We need to inflate the holder
            final E cast = (E) effective;
            instances = new EffectiveInstances<>(definition().support().extractEffectiveState(cast), cast);
            effectiveInstance = instances;
        } else {
