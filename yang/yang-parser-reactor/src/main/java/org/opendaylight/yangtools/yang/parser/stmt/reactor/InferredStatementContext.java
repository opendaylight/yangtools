...
class InferredStatementContext<A, D extends DeclaredStatement<A>, E extends EffectiveStatement<A, D>>
    ...
    E createEffective() {
        if (prototype instanceof InferredStatementContext) {
            final AdaptedPrototypeStatementContext<A, D, E> adeMyStatementContext =
                    new AdaptedPrototypeStatementContext<>(parent, effectiveConfig(), argument, prototype, history(),
                            schemaPath());
            return definition().getFactory()
                    .createEffective(adeMyStatementContext, streamDeclared(), streamEffective());
        } else {
