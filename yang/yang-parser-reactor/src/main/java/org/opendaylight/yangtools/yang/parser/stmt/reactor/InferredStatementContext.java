    ...
    public <D extends DeclaredStatement<QName>, E extends EffectiveStatement<QName, D>>
        ...
        if (prototype instanceof InferredStatementContext) {
            ...
            template = prototype.allSubstatementsStream()
                .filter(stmt -> stmt.isSupportedToBuildEffective()
                    && stmt.producesEffective(SchemaTreeEffectiveStatement.class)
                    && templateQName.equals(stmt.getStatementArgument()) && stmt.isSupportedByFeatures())
                .findAny()
                .orElse(null);
        }
        ...
        // FIXME: this is the price for direct access above...
        final Mutable<QName, ?, ?> materialized = findMaterialized(template);
        if (materialized != null) {
            // ... and this is weird: the statement should've been caught up
            ensureCompletedPhase(materialized);
            return (StmtContext<QName, D, E>) materialized;
        }

        @SuppressWarnings("unchecked")
        final Mutable<QName, D, E> ret = (Mutable<QName, D, E>) copySubstatement((Mutable<?, ?, ?>) template)
