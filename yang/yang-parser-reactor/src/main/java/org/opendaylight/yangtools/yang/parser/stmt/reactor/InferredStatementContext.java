...
final class InferredStatementContext<A, D extends DeclaredStatement<A>, E extends EffectiveStatement<A, D>>
    ...
    @Override
    public <D extends DeclaredStatement<QName>, E extends SchemaTreeEffectiveStatement<D>>
            StmtContext<?, D, E> requestSchemaTreeChild(final QName qname) {
        LOG.info("Materializing on lookup of {}", qname);
        ensureSubstatements();

        // Now we have to do a lookup as we do not have access to the namespace being populated (yet)
        for (StmtContext<?, ?, ?> stmt : effectiveSubstatements()) {
            if (stmt.producesEffective(SchemaTreeEffectiveStatement.class)) {
                if (qname.equals(stmt.coerceStatementArgument())) {
                    return (StmtContext<?, D, E>) stmt;
                }
            }
        }
        return null;
