...
final class InferredStatementContext<A, D extends DeclaredStatement<A>, E extends EffectiveStatement<A, D>>
    ...
    private @NonNull E tryToReusePrototype(final StatementFactory<A, D, E> factory) {
        ...
        if (!factory.canReuseCurrent(this, prototype, origSubstatements)) {
            return deduplicate(factory, tryToReuseSubstatements(factory, origEffective));
        }
        ...
        return deduplicate(factory, factory.createEffective(this, declared.stream(), effective.stream()));
    }
    ...
    private @NonNull E deduplicate(final StatementFactory<A, D, E> factory, final @NonNull E effective) {
        final EffectiveStatementState key = factory.extractEffectiveState(effective);
        if (key != null) {
            final ReactorStmtCtx<A, D, E> source = unmodifiedEffectiveSource();
            if (source != null) {
                return source.attachCopy(key, effective);
            }
        }
        return effective;
    }
