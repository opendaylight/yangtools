...
public abstract class StatementContextBase<A, D extends DeclaredStatement<A>, E extends EffectiveStatement<A, D>>
    ...
    @Override
    final boolean doTryToCompletePhase(final byte targetExecutionOrder) {
        final boolean finished = phaseMutation.isEmpty() ? true : runMutations(targetExecutionOrder);
        if (completeChildren(targetExecutionOrder) && finished) {
            onPhaseCompleted(targetExecutionOrder);
            return true;
        }
        return false;
    }

    private boolean completeChildren(final byte exectionOrder) {
        boolean finished = true;
        for (final StatementContextBase<?, ?, ?> child : mutableDeclaredSubstatements()) {
            finished &= child.tryToCompletePhase(exectionOrder);
        }
    ...
    private boolean runMutations(final byte targetExecutionOrder) {
        final ModelProcessingPhase phase = verifyNotNull(ModelProcessingPhase.ofExecutionOrder(targetExecutionOrder));
     ...
     *
     * @param completedExecutionOrder that was to be completed (finished)
     * @throws SourceException when an error occurred in source parsing
     */
    private void onPhaseCompleted(final byte completedExecutionOrder) {
        if (completedExecutionOrder == ExecutionOrder.EFFECTIVE_MODEL) {
            // We have completed effective model, substatements are guaranteed not to change
            summarizeSubstatementPolicy();
        }

        final ModelProcessingPhase phase =
            verifyNotNull(ModelProcessingPhase.ofExecutionOrder(completedExecutionOrder));
