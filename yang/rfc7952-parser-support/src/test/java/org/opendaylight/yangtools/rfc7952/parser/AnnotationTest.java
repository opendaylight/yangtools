...
public class AnnotationTest {
    ...
    public static void createReactor() {
        final CustomStatementParserBuilder builder = new CustomStatementParserBuilder().addDefaultRFC6020Bundles();
        for (StatementSupport<?, ?, ?> support : Metadata.getStatements()) {
            builder.addStatementSupport(ModelProcessingPhase.FULL_DECLARATION, support);
        }
    ...
    public void testAnnotationResolution() throws ReactorException, IOException, YangSyntaxErrorException {
        ...
        final AnnotationSchemaNode annotation = annotations.get(QName.create("http://example.org/example-last-modified",
            "last-modified"));
