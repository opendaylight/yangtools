...
public abstract class QNameTransformingStreamWriter implements NormalizedNodeStreamWriter {
    ...
    public static NormalizedNodeStreamWriter fromQNameMap(NormalizedNodeStreamWriter delegate,
            final Map<QName, QName> mapping) {
        return new QNameTransformingStreamWriter(delegate) {

            @Override
            protected QName transform(QName key) {
                QName potential = mapping.get(key);
                return potential != null ? potential : key;
            }
        };
    }

    public static NormalizedNodeStreamWriter fromQNameModuleMap(NormalizedNodeStreamWriter delegate,
            final Map<QNameModule, QNameModule> mapping) {
        return new QNameTransformingStreamWriter(delegate) {

            @Override
            protected QName transform(QName key) {
                QNameModule potential = mapping.get(key.getModule());
                return potential != null ? QName.create(potential, key.getLocalName()) : key;
            }
        };
    }
