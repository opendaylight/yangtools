...
class ContainerNodeModification implements Modification<ContainerSchemaNode, ContainerNode> {
    ...
    // FIXME REFACTOR all modifications, lots of duplicate code

    @Override
    public Optional<ContainerNode> modify(ContainerSchemaNode schema,  Optional<ContainerNode> actual,
                                   Optional<ContainerNode> modification, OperationStack operationStack)
            throws DataModificationException {

        operationStack.enteringNode(modification);

        Optional<ContainerNode> result = null;
    ...
    private Optional<ContainerNode> modifyContainer(DataNodeContainer schema, Optional<ContainerNode> actual, Optional<ContainerNode> modification, OperationStack operationStack)
            throws DataModificationException {
    ...
    private List<? extends DataContainerChild<?, ?>> modifyContainerChildNodes(DataNodeContainer schema, OperationStack operationStack,
                                                    ContainerNode actual, ContainerNode modification, Set<InstanceIdentifier.PathArgument> toProcess) throws DataModificationException {
    ...
    private Optional<? extends DataContainerChild<?, ?>> modifyContainerNode(OperationStack operationStack, ContainerNode actual, ContainerNode modification, InstanceIdentifier.PathArgument childToProcess, Object schemaChild) throws DataModificationException {
    ...
    private Optional<? extends DataContainerChild<?, ?>> dispatchChildModification(Object schemaChild, Optional<DataContainerChild<?, ?>> actual,
        ...
        if (schemaChild instanceof LeafSchemaNode) {
            ...
            return modify((ContainerSchemaNode) schemaChild, Optional.fromNullable(actualContainer),
            ...
            org.opendaylight.yangtools.yang.model.api.ChoiceNode listSchema = (org.opendaylight.yangtools.yang.model.api.ChoiceNode) schemaChild;
            return new ChoiceNodeModification().modify(listSchema, Optional.fromNullable(actualContainer),
                    Optional.fromNullable(modificationContainer), operations);
        }
    ...
    private static Optional<ContainerNode> build(DataNodeContainer schema, List<? extends DataContainerChild<?, ?>> result) {
        DataContainerNodeAttrBuilder<InstanceIdentifier.NodeIdentifier, ContainerNode> b = Builders.containerBuilder((ContainerSchemaNode) schema);
    ...
    private static Set<InstanceIdentifier.PathArgument> getQNamesToProcess(Optional<ContainerNode> actual, Optional<ContainerNode> modification) {
    ...
    private static Set<? extends InstanceIdentifier.PathArgument> getChildQNames(Optional<ContainerNode> actual) {
        Set<InstanceIdentifier.PathArgument> qNames = Sets.newHashSet();
