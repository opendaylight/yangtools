...
public class UnionDependencySortTest {
    ...
    public void testSortMethod() {
        ...
        UnionDependencySort unionDependencySort = new UnionDependencySort();
        Set<TypeDefinition<?>> typeDefs = new HashSet<>();

        StringType stringType = StringType.getInstance();
        ExtendedType extendedType = ExtendedType.builder(QName.create("ExtendedType1"), stringType, Optional.<String> absent(), Optional.<String> absent(), SchemaPath.create(false, QName.create("Cont1"), QName.create("List1"))).build();

        typeDefs.add(stringType);
        typeDefs.add(extendedType);

        List<ExtendedType> sortedExtendedTypes = unionDependencySort.sort(typeDefs);
