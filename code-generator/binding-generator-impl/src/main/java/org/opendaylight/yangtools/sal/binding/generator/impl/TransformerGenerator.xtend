...
class TransformerGenerator {
    ...
    def Class<? extends BindingCodec<Map<QName, Object>, Object>> augmentationTransformerFor(Class<?> inputType, InstanceIdentifier<?> bindingId) {
        return withClassLoaderAndLock(inputType.classLoader, lock) [ |
            val ret = getGeneratedClass(inputType)
            if (ret !== null) {
                return ret as Class<? extends BindingCodec<Map<QName,Object>, Object>>;
            ...
            val typeSpecBuilder = typeToDefinition.get(ref)
            val typeSpec = typeSpecBuilder.toInstance();
            val newret = generateAugmentationTransformerFor(inputType, typeSpec, node, bindingId);
            listener.onClassProcessed(inputType);
            return newret as Class<? extends BindingCodec<Map<QName,Object>, Object>>;
        ]
    }
    ...
    private def mappingForNodes(Collection<? extends DataSchemaNode> childNodes, Map<String, Type> properties,
    ...
    private def Class<? extends BindingCodec<Map<QName, Object>, Object>> generateAugmentationTransformerFor(
        Class<?> inputType, GeneratedType type, AugmentationSchema node, InstanceIdentifier<?> bindingId) {
        try {
                    ...
                    bodyChecked = '''
                        {
                            ...
                            «FOR child : node.childNodes»
                                «val signature = properties.getFor(child)»
                                «val Type childType = signature.value»
                                «val x = listener.putPathToBindingIdentifier(child.path, bindingId, loadClass(childType))»
