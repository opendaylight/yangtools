...
public class StreamWriterGenerator extends AbstractStreamWriterGenerator {
    ...
    public StreamWriterGenerator(final ClassPool pool, final StaticCodecBinder binder, final CodecRegistry codecRegistry) {
        super(pool, binder);
    }

    protected static final String WRITER_CLASSNAME = BindingStreamEventWriter.class.getName();

    private abstract class DataNodeContainerEmitterSource extends DataObjectSerializerSource {
        ...
        public DataNodeContainerEmitterSource(final GeneratedType type, final DataNodeContainer node) {
            this.dtoType = type;
        ...
        @Override
        protected CharSequence getStaticEmmitBody() {
        ...
        private void emitChildInner(final StringBuilder b, final String getterName, final Type childType,
            ...
            if (child instanceof LeafSchemaNode) {
                b.append(statement(leafNode(child.getQName().getLocalName(), serialize(childType, getterName))));
                ...
                b.append(forEach(getterName, valueType, statement(leafSetEntryNode(serialize(valueType, CURRENT)))));
                b.append(statement(endNode()));
            ...
            } else if (child instanceof ChoiceCaseNode) {
                staticConstant(CHOICE_SERIALIZER, DataObjectSerializerImplementation.class, CHOICE_SERIALIZER_IMPL);
                b.append(statement(invoke(CHOICE_SERIALIZER, EMIT_METHOD_NAME, REGISTRY, INPUT, STREAM)));
            }
        }
    ...
    protected abstract class AugmentableDataNodeContainerEmmiterSource extends DataNodeContainerEmitterSource {
    ...
    DataObjectSerializerSource generateContainerSerializer(final GeneratedType type, final ContainerSchemaNode node) {
        ...
        return new DataNodeContainerEmitterSource(type, node) {
            @Override
    ...
    @Override
    DataObjectSerializerSource generateChoiceSerializer(final GeneratedType type, final ChoiceNode node) {
    ...
    DataObjectSerializerSource generateSerializer(final GeneratedType type, final AugmentationSchema schema) {
        return new DataNodeContainerEmitterSource(type,schema) {
