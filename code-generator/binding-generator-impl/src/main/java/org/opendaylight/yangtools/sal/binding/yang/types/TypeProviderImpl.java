...
public final class TypeProviderImpl implements TypeProvider {
	...
	public void putReferencedType(final SchemaPath refTypePath,
			final Type refType) {
		Preconditions
				.checkArgument(refTypePath != null,
						"Path reference of Enumeration Type Definition cannot be NULL!");
		Preconditions.checkArgument(refType != null,
				"Reference to Enumeration Type cannot be NULL!");
	...
	public Type javaTypeForSchemaDefinitionType(
			final TypeDefinition<?> typeDefinition, final SchemaNode parentNode) {
		Type returnType = null;
		Preconditions.checkArgument(typeDefinition != null,
		...
		Preconditions.checkArgument(
				typeDefinition.getQName().getLocalName() != null,
	 ...
	 */
	private Type javaTypeForLeafrefOrIdentityRef(
			TypeDefinition<?> typeDefinition, SchemaNode parentNode) {
	...
	private Type javaTypeForExtendedType(TypeDefinition<?> typeDefinition) {
		...
		if (returnType == null) {
			if (baseTypeDef instanceof EnumTypeDefinition) {
				...
				final Module module = findParentModule(schemaContext,
						typeDefinition);
				if (module != null) {
	...
	private Type provideTypeForIdentityref(IdentityrefTypeDefinition idref) {
		...
		Module module = schemaContext.findModuleByNamespaceAndRevision(
				baseIdQName.getNamespace(), baseIdQName.getRevision());
		...
		Preconditions.checkArgument(identity != null, "Target identity '"
				+ baseIdQName + "' do not exists");
	...
	private TypeDefinition<?> baseTypeDefForExtendedType(
		...
		Preconditions.checkArgument(extendTypeDef != null,
				"Type Definiition reference cannot be NULL!");
	...
	public Type provideTypeForLeafref(final LeafrefTypeDefinition leafrefType,
		...
		Type returnType = null;
		Preconditions.checkArgument(leafrefType != null,
				"Leafref Type Definition reference cannot be NULL!");

		Preconditions
				.checkArgument(leafrefType.getPathStatement() != null,
				...
				if (module != null) {
					...
					if (xpath.isAbsolute()) {
						...
						dataNode = findDataSchemaNodeForRelativeXPath(
								schemaContext, module, parentNode, xpath);
					}
	 ...
	 *             </ul>
	 */
	private Enumeration provideTypeForEnum(
			final EnumTypeDefinition enumTypeDef, final String enumName,
			final SchemaNode parentNode) {
		Preconditions.checkArgument(enumTypeDef != null,
				"EnumTypeDefinition reference cannot be NULL!");
		Preconditions
				.checkArgument(enumTypeDef.getValues() != null,
						"EnumTypeDefinition MUST contain at least ONE value definition!");
		Preconditions.checkArgument(enumTypeDef.getQName() != null,
				"EnumTypeDefinition MUST contain NON-NULL QName!");
		Preconditions.checkArgument(
	 ...
	 * 
	 */
	private Enumeration addInnerEnumerationToTypeBuilder(
			final EnumTypeDefinition enumTypeDef, final String enumName,
		...
		Preconditions.checkArgument(
				enumTypeDef.getQName().getLocalName() != null,
				"Local Name in EnumTypeDefinition QName cannot be NULL!");
		Preconditions.checkArgument(typeBuilder != null,
				"Generated Type Builder reference cannot be NULL!");
	...
	private Type resolveTypeFromDataSchemaNode(final SchemaNode dataNode) {
		...
		if (dataNode != null) {
			if (dataNode instanceof LeafSchemaNode) {
				...
				final LeafListSchemaNode leafList = (LeafListSchemaNode) dataNode;
				returnType = javaTypeForSchemaDefinitionType(
						leafList.getType(), leafList);
			}
		}
		return returnType;
	}
	...
	private void resolveTypeDefsFromContext() {
		...
		Preconditions.checkArgument(modules != null,
				"Sef of Modules cannot be NULL!");
		final Module[] modulesArray = new Module[modules.size()];
		int i = 0;
		for (Module modul : modules) {
			modulesArray[i++] = modul;
		}
		final List<Module> modulesSortedByDependency = ModuleDependencySort
				.sort(modulesArray);
		...
		if ((basePackageName != null) && (moduleName != null)
			...
			if (!(innerTypeDefinition instanceof LeafrefTypeDefinition)
				...
				if (innerTypeDefinition instanceof ExtendedType) {
					...
					returnType = provideGeneratedTOFromExtendedType(
							innerExtendedType, basePackageName, typedefName);
				} else if (innerTypeDefinition instanceof UnionTypeDefinition) {
					final GeneratedTOBuilder genTOBuilder = provideGeneratedTOBuilderForUnionTypeDef(
							basePackageName,
							(UnionTypeDefinition) innerTypeDefinition,
							typedefName, typedef);
	...
	private GeneratedTransferObject wrapJavaTypeIntoTO(
		...
		if (javaType != null) {
			...
			if (javaType == BaseYangTypes.STRING_TYPE
					&& typedef instanceof ExtendedType) {
	 ...
	 *         builders
	 */
	public GeneratedTOBuilder provideGeneratedTOBuilderForUnionTypeDef(
			final String basePackageName, final UnionTypeDefinition typedef,
			String typeDefName, SchemaNode parentNode) {
		final List<GeneratedTOBuilder> genTOBuilders = provideGeneratedTOBuildersForUnionTypeDef(
				basePackageName, typedef, typeDefName, parentNode);
	 ...
	 *             </ul>
	 */
	public List<GeneratedTOBuilder> provideGeneratedTOBuildersForUnionTypeDef(
			final String basePackageName, final UnionTypeDefinition typedef,
			final String typeDefName, final SchemaNode parentNode) {
		Preconditions.checkArgument(basePackageName != null,
				"Base Package Name cannot be NULL!");
		Preconditions.checkArgument(typedef != null,
				"Type Definition cannot be NULL!");
		Preconditions
				.checkArgument(typedef.getQName() != null,
						"Type Definition cannot have non specified QName (QName cannot be NULL!)");
		...
		if (typedef != null) {
			...
			if (typeDefName != null && !typeDefName.isEmpty()) {
				...
				unionGenTOBuilder = new GeneratedTOBuilderImpl(basePackageName,
						typeName);
			} else {
				unionGenTOBuilder = typedefToTransferObject(basePackageName,
						typedef);
			}
			...
			for (final TypeDefinition<?> unionType : unionTypes) {
				final String unionTypeName = unionType.getQName()
						.getLocalName();
				if (unionType instanceof UnionType) {
					generatedTOBuilders.addAll(resolveUnionSubtypeAsUnion(
							unionGenTOBuilder, (UnionType) unionType,
							basePackageName, parentNode));
				} else if (unionType instanceof ExtendedType) {
					resolveExtendedSubtypeAsUnion(unionGenTOBuilder,
							(ExtendedType) unionType, unionTypeName,
							regularExpressions, parentNode);
				} else if (unionType instanceof EnumTypeDefinition) {
					final Enumeration enumeration = addInnerEnumerationToTypeBuilder(
							(EnumTypeDefinition) unionType, unionTypeName,
							unionGenTOBuilder);
					updateUnionTypeAsProperty(unionGenTOBuilder, enumeration,
							unionTypeName);
				} else {
					final Type javaType = BaseYangTypes.BASE_YANG_TYPES_PROVIDER
							.javaTypeForSchemaDefinitionType(unionType,
									parentNode);
					updateUnionTypeAsProperty(unionGenTOBuilder, javaType,
							unionTypeName);
				}
			}
	 ...
	 */
	private List<GeneratedTOBuilder> resolveUnionSubtypeAsUnion(
			final GeneratedTOBuilder parentUnionGenTOBuilder,
			final UnionTypeDefinition unionSubtype,
			final String basePackageName, final SchemaNode parentNode) {
		final String newTOBuilderName = provideAvailableNameForGenTOBuilder(parentUnionGenTOBuilder
				.getName());
	 ...
	 */
	private void resolveExtendedSubtypeAsUnion(
			final GeneratedTOBuilder parentUnionGenTOBuilder,
			final ExtendedType unionSubtype, final String unionTypeName,
			final List<String> regularExpressions, final SchemaNode parentNode) {
		final Type genTO = findGenTO(unionTypeName, parentNode);
		if (genTO != null) {
			updateUnionTypeAsProperty(parentUnionGenTOBuilder, genTO,
					genTO.getName());
		} else {
			...
			if (unionTypeName.equals(baseType.getQName().getLocalName())) {
				final Type javaType = BaseYangTypes.BASE_YANG_TYPES_PROVIDER
						.javaTypeForSchemaDefinitionType(baseType, parentNode);
				if (javaType != null) {
					updateUnionTypeAsProperty(parentUnionGenTOBuilder,
							javaType, unionTypeName);
				}
			}
			if (baseType instanceof StringType) {
				regularExpressions
						.addAll(resolveRegExpressionsFromTypedef(unionSubtype));
			}
		}
	}
	 ...
	 *            stored
	 */
	private void storeGenTO(TypeDefinition<?> newTypeDef,
			GeneratedTOBuilder genTOBuilder, SchemaNode parentNode) {
		if (!(newTypeDef instanceof UnionType)) {
			Map<String, Type> genTOsMap = null;
			final Module parentModule = findParentModule(schemaContext,
					parentNode);
			if (parentModule != null && parentModule.getName() != null) {
				genTOsMap = genTypeDefsContextMap.get(parentModule.getName());
				genTOsMap.put(newTypeDef.getQName().getLocalName(),
						genTOBuilder.toInstance());
			}
		}
	}
	...
	private void updateUnionTypeAsProperty(
		...
		if (unionGenTransObject != null && type != null
				&& !unionGenTransObject.containsProperty(propertyName)) {
	 ...
	 *             </ul>
	 */
	public GeneratedTOBuilder provideGeneratedTOBuilderForBitsTypeDefinition(
			final String basePackageName, final TypeDefinition<?> typeDef,
			String typeDefName) {
		...
		if (typeDef instanceof BitsTypeDefinition) {
			...
			final GeneratedTOBuilder genTOBuilder = new GeneratedTOBuilderImpl(
					basePackageName, typeName);

			final List<Bit> bitList = bitsTypeDefinition.getBits();
			GeneratedPropertyBuilder genPropertyBuilder;
			for (final Bit bit : bitList) {
				String name = bit.getName();
				genPropertyBuilder = genTOBuilder
						.addProperty(parseToValidParamName(name));
	...
	private List<String> resolveRegExpressionsFromTypedef(ExtendedType typedef) {
		...
		if (strTypeDef instanceof StringType) {
			final List<PatternConstraint> patternConstraints = typedef
	 ...
	 *             </ul>
	 */
	private void addStringRegExAsConstant(GeneratedTOBuilder genTOBuilder,
			List<String> regularExpressions) {
		if (genTOBuilder == null) {
			throw new IllegalArgumentException(
					"Generated transfer object builder can't be null");
		}
		if (regularExpressions == null) {
			throw new IllegalArgumentException(
					"List of regular expressions can't be null");
		}
		if (!regularExpressions.isEmpty()) {
			genTOBuilder.addConstant(
					Types.listTypeFor(BaseYangTypes.STRING_TYPE),
	...
	private List<TypeDefinition<?>> sortTypeDefinitionAccordingDepth(
		...
		for (TypeDefinition<?> unsortedTypeDefinition : unsortedTypeDefinitions) {
			...
			List<TypeDefinition<?>> typeDefinitionsConcreteDepth = typeDefinitionsDepths
					.get(depth);
	...
	private int getTypeDefinitionDepth(final TypeDefinition<?> typeDefinition) {
		...
		if (baseType instanceof ExtendedType) {
			depth = depth
					+ getTypeDefinitionDepth(typeDefinition.getBaseType());
		} else if (baseType instanceof UnionType) {
			List<TypeDefinition<?>> childTypeDefinitions = ((UnionType) baseType)
					.getTypes();
			int maxChildDepth = 0;
			int childDepth = 1;
			for (TypeDefinition<?> childTypeDefinition : childTypeDefinitions) {
				childDepth = childDepth
						+ getTypeDefinitionDepth(childTypeDefinition
								.getBaseType());
