...
public class BindingGeneratorImpl implements BindingGenerator {
    ...
    private Map<Module, List<GeneratedTOBuilder>> genTOs = new HashMap<Module, List<GeneratedTOBuilder>>();
    private Map<Module, Map<SchemaPath, GeneratedTypeBuilder>> childNodes = new HashMap<Module, Map<SchemaPath, GeneratedTypeBuilder>>();
    private Map<Module, Map<SchemaPath, GeneratedTypeBuilder>> groupings = new HashMap<Module, Map<SchemaPath, GeneratedTypeBuilder>>();
    private Map<Module, Map<SchemaPath, Type>> typedefs = new HashMap<Module, Map<SchemaPath, Type>>();
    private Map<Module, Map<SchemaPath, GeneratedTypeBuilder>> cases = new HashMap<Module, Map<SchemaPath, GeneratedTypeBuilder>>();
    private Map<Module, Set<GeneratedTOBuilder>> identities = new HashMap<Module, Set<GeneratedTOBuilder>>();
    private Map<Module, GeneratedTypeBuilder> moduleNodes = new HashMap<Module, GeneratedTypeBuilder>();
    private Map<Module, Set<GeneratedTypeBuilder>> topLevelNodes = new HashMap<Module, Set<GeneratedTypeBuilder>>();
    private Map<Module, List<GeneratedTypeBuilder>> augmentations = new HashMap<Module, List<GeneratedTypeBuilder>>();
    ...
    override generateTypes(SchemaContext context, Set<Module> modules) {
        ...
        for (Module m : modules) {
            for (b : genTOs.get(m)) {
                filteredGenTypes.add(b.toInstance)
            }
            for (b : childNodes.get(m).values) {
                filteredGenTypes.add(b.toInstance)
            }
            for (b : groupings.get(m).values) {
                filteredGenTypes.add(b.toInstance)
            }
            for (b : typedefs.get(m).values) {
                if (b !== null) {
                    filteredGenTypes.add(b)
                }
            }
            for (b : cases.get(m).values) {
                filteredGenTypes.add(b.toInstance)
            }
            for (b : identities.get(m)) {
                filteredGenTypes.add(b.toInstance)
            }
            for (b : topLevelNodes.get(m)) {
                filteredGenTypes.add(b.toInstance)
            }
            for (b : augmentations.get(m)) {
                filteredGenTypes.add(b.toInstance)
            }

            var GeneratedTypeBuilder mType = moduleNodes.get(m);
            if (mType != null) {
                filteredGenTypes.add(mType.toInstance);
            }
        }
    ...
    private def void moduleToGenTypes(Module m, SchemaContext context) {
        ...
        if (!m.childNodes.isEmpty()) {
            ...
            moduleNodes.put(m, moduleType);
            resolveDataSchemaNodes(m, basePackageName, moduleType, moduleType, m.childNodes);
        }
    }

    private def void init(Module module) {
        genTOs.put(module, new ArrayList<GeneratedTOBuilder>());
        childNodes.put(module, new HashMap<SchemaPath, GeneratedTypeBuilder>());
        groupings.put(module, new HashMap<SchemaPath, GeneratedTypeBuilder>());
        typedefs.put(module, new HashMap<SchemaPath, Type>());
        cases.put(module, new HashMap<SchemaPath, GeneratedTypeBuilder>());
        identities.put(module, new HashSet<GeneratedTOBuilder>());
        topLevelNodes.put(module, new HashSet<GeneratedTypeBuilder>());
        augmentations.put(module, new ArrayList<GeneratedTypeBuilder>());
    }
    ...
    private def void allTypeDefinitionsToGenTypes(Module module) {
        ...
        for (TypeDefinition<?> typedef : typeDefinitions) {
            if (typedef !== null) {
                ...
                if (type !== null) {
                    typedefs.get(module).put(typedef.path, type);
                }
            }
        }
    }
    ...
    private def void containerToGenType(Module module, String basePackageName, GeneratedTypeBuilder parent,
        ...
        childNodes.get(module).put(node.path, genType);
    ...
    private def void rpcMethodsToGenType(Module module) {
        ...
        for (rpc : rpcDefinitions) {
            if (rpc !== null) {
                ...
                if (input !== null) {
                    ...
                    childNodes.get(module).put(input.path, inType);
                ...
                if (output !== null) {
                    ...
                    childNodes.get(module).put(output.path, outType);
        ...
        topLevelNodes.get(module).add(interfaceBuilder);
    }
    ...
    private def void notificationsToGenType(Module module) {
        ...
        for (notification : notifications) {
            if (notification !== null) {
                ...
                childNodes.get(module).put(notification.path, notificationInterface);
        ...
        topLevelNodes.get(module).add(listenerInterface);
    }
    ...
    private def void identityToGenType(Module module, String basePackageName, IdentitySchemaNode identity,
        ...
        identities.get(module).add(newType);
    }
    ...
    private def void groupingToGenType(String basePackageName, GroupingDefinition grouping, Module module) {
        ...
        groupings.get(module).put(grouping.path, genType);
    ...
    private def void augmentationToGenTypes(String augmentPackageName, AugmentationSchema augSchema, Module module,
        ...
        if (targetSchemaNode !== null) {
            var targetTypeBuilder = findNodeByPath(targetSchemaNode.path, childNodes);
            if (targetTypeBuilder === null) {
                targetTypeBuilder = findNodeByPath(targetSchemaNode.path, cases);
            }
            if (targetTypeBuilder === null) {
            ...
            if (!(targetSchemaNode instanceof ChoiceNode)) {
                ...
                augmentations.get(module).add(augTypeBuilder)
            } else {
    ...
    private def void choiceToGeneratedType(Module module, String basePackageName, GeneratedTypeBuilder parent,
        ...
        childNodes.get(module).put(choiceNode.path, choiceTypeBuilder);
    ...
    private def void generateTypesFromChoiceCases(Module module, String basePackageName,
        ...
        for (caseNode : caseNodes) {
            if (caseNode !== null && !caseNode.isAddedByUses() && !caseNode.isAugmenting()) {
                ...
                cases.get(module).put(caseNode.path, caseTypeBuilder);
                ...
                if (caseChildNodes !== null) {
                    ...
                    if (parentNode instanceof AugmentationSchema) {
                        ...
                        val parent = targetSchemaNode as SchemaNode
                        val GeneratedTypeBuilder childOfType = findNodeByPath(parent.path, childNodes)
                        resolveDataSchemaNodes(module, basePackageName, caseTypeBuilder, childOfType, caseChildNodes);
                    } else {
                        val parent = choiceNode.parent as SchemaNode;
                        val GeneratedTypeBuilder childOfType = findNodeByPath(parent.path, childNodes)
                        resolveDataSchemaNodes(module, basePackageName, caseTypeBuilder, childOfType, caseChildNodes);
                    }
                }
        ...
        for (caseNode : augmentedNodes) {
            if (caseNode !== null) {
                ...
                if (parent instanceof Module) {
                    childOfType = moduleNodes.get(parent as Module);
                } else if (parent instanceof ChoiceCaseNode) {
                    childOfType = findNodeByPath(parent.path, cases);
                } else if (parent instanceof DataSchemaNode || parent instanceof NotificationDefinition) {
                    childOfType = findNodeByPath(parent.path, childNodes);
                } else if (parent instanceof GroupingDefinition) {
                    childOfType = findNodeByPath(parent.path, groupings);
                }
    ...
    private def typeBuildersToGenTypes(Module module, GeneratedTypeBuilder typeBuilder, GeneratedTOBuilder genTOBuilder) {
        ...
        if (genTOBuilder !== null) {
            ...
            genTOs.get(module).add(genTOBuilder);
        }
    }
    ...
    private def addImplementedInterfaceFromUses(DataNodeContainer dataNodeContainer, GeneratedTypeBuilder builder) {
        for (usesNode : dataNodeContainer.uses) {
            if (usesNode.groupingPath !== null) {
                val genType = findNodeByPath(usesNode.groupingPath, groupings).toInstance;
                if (genType === null) {
    ...
    private def GeneratedTypeBuilder findNodeByPath(SchemaPath schemaPath,
        Map<Module, Map<SchemaPath, GeneratedTypeBuilder>> map) {
        for (types : map.values) {
            val type = types.get(schemaPath);
            if (type !== null) {
                return type;
            }
        }
