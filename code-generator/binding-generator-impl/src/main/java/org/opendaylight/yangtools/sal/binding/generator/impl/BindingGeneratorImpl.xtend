	...
	private def List<Type> augmentationToGenTypes(String augmentPackageName, AugmentationSchema augSchema) {
		...
		if ((targetSchemaNode !== null) && (targetSchemaNode.QName !== null) &&
			...
			if (!(targetSchemaNode instanceof ChoiceNode)) {
				...
				genTypes.addAll(
					generateTypesFromAugmentedChoiceCases(augmentPackageName, refChoiceType,
						choiceCaseNodes));
			}
	...
	private def void addSchemaNodeToBuilderAsMethod(String basePackageName, DataSchemaNode node,
		...
		if (node !== null && typeBuilder !== null) {
			switch (node) {
				...
				case node instanceof LeafListSchemaNode: resolveLeafListSchemaNode(typeBuilder,
					node as LeafListSchemaNode)
				case node instanceof ContainerSchemaNode: resolveContainerSchemaNode(basePackageName, typeBuilder,
					node as ContainerSchemaNode)
				case node instanceof ListSchemaNode: resolveListSchemaNode(basePackageName, typeBuilder,
					node as ListSchemaNode)
				case node instanceof ChoiceNode: resolveChoiceSchemaNode(basePackageName, typeBuilder,
					node as ChoiceNode)
			}
		}
	}
