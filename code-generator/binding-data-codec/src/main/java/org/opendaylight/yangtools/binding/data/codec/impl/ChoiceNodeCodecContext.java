...
class ChoiceNodeCodecContext extends DataContainerCodecContext<ChoiceNode> {
    ...
    public ChoiceNodeCodecContext(final DataContainerCodecPrototype<ChoiceNode> prototype) {
        ...
        Set<Class<?>> cazes = factory().getRuntimeContext().getCases(bindingClass());
        for (Class<?> caze : cazes) {
    ...
    @Override
    DataContainerCodecContext<?> getStreamChild(final Class<?> childClass) {
    ...
    @Override
    Object dataFromNormalizedNode(final NormalizedNode<?, ?> data) {
        Preconditions
