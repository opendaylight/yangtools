...
import org.opendaylight.yangtools.binding.data.codec.impl.CodecContextNode.CaseContextNode;
import org.opendaylight.yangtools.binding.data.codec.impl.CodecContextNode.ListNode;
...
class BindingToNormalizedStreamWriter implements BindingStreamEventWriter, Delegator<NormalizedNodeStreamWriter> {
    ...
    private NodeIdentifier duplicateSchemaEnter() {
        ...
        return (NodeIdentifier) getCurrentSchema().getDomPathArgument();
    }
    ...
    private <T extends YangInstanceIdentifier.PathArgument> T enter(final Class<?> name, final Class<T> identifier) {
        ...
        if(current == null) {
            ...
            Preconditions.checkArgument((current instanceof CodecContextNode.DataContainerNode),"Could not start node %s",name);
            next =((CodecContextNode.DataContainerNode<?>) current).getStreamChild(name);
        }
