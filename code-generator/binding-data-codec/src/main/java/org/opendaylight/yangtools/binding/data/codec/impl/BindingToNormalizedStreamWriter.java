...
class BindingToNormalizedStreamWriter implements BindingStreamEventWriter, Delegator<NormalizedNodeStreamWriter> {
    ...
    private NodeCodecContext getCurrentSchema() {
        return schema.peek();
    }

    private NodeIdentifier duplicateSchemaEnter() {
        NodeCodecContext current = getCurrentSchema();
    ...
    public void endNode() {
        ...
        if(!(left instanceof CaseNodeCocecContext)) {
            getDelegate().endNode();
        }
    }
    ...
    private <T extends YangInstanceIdentifier.PathArgument> T enter(final Class<?> name, final Class<T> identifier) {
        ...
        if(current == null) {
            ...
            Preconditions.checkArgument((current instanceof DataContainerCodecContext),"Could not start node %s",name);
    ...
    public void startMapEntryNode(final Identifier<?> key,final int childSizeHint) throws IllegalArgumentException {
        ...
        NodeIdentifierWithPredicates identifier = ((ListNodeCodecContext) getCurrentSchema()).serialize(key);
