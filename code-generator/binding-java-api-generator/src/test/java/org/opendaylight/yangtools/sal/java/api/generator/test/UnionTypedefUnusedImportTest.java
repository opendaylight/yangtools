...
public class UnionTypedefUnusedImportTest extends BaseCompilationTest {
    ...
    private boolean containsImport(final String fullImport) throws FileNotFoundException, URISyntaxException {
        boolean result = false;
        final File relativeFilePath = new File(getClass().getResource("/").toURI());
        final String filePath = relativeFilePath.getParent() + "/test/src/union-typedef/org/opendaylight/yang/gen/v1/org/opendaylight/yangtools/union/typedef/rev141124/TypedefUnion.java";
        final JavaProjectBuilder builder = new JavaProjectBuilder();
        final JavaSource src = builder.addSource(new FileReader(filePath));
        final List<String> imports = src.getImports();

        for (String anImport : imports) {
            if (anImport.equals(fullImport)) {
                result = true;
            }
        }
        return result;
