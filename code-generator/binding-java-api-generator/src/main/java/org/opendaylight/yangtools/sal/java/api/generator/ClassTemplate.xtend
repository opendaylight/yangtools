        ...
        «generateClassDeclaration(isInnerClass)» {
            ...
            «IF restrictions != null»
                «IF !restrictions.lengthConstraints.nullOrEmpty»
                    «IF findProperty(genTO, "value").returnType.name.contains("[")»
                        «LengthGenerator.generateArrayLengthChecker("_value", restrictions.lengthConstraints)»
                    «ELSE»
                        «LengthGenerator.generateStringLengthChecker("_value", restrictions.lengthConstraints)»
                    «ENDIF»
                «ENDIF»
    ...
    def private generateRestrictions(Type type, String paramName, Type returnType) '''
        ...
        «IF restrictions !== null»
            ...
            «IF !restrictions.lengthConstraints.empty»
                if («paramName» != null) {
                    «IF isNestedType»
                        «LengthGenerator.generateLengthCheckerCall(paramName, paramName + ".getValue()")»
                    «ELSE»
                        «LengthGenerator.generateLengthCheckerCall(paramName, paramName)»
                    «ENDIF»
                }
            «ENDIF»
