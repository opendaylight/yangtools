...
class YangModuleInfoTemplate {
    ...
    def body() '''
        public class YangModuleInfoImpl implements «YangModuleInfo.importedName» {
            ...
            private static final «YangModuleInfo.importedName» INSTANCE = new YangModuleInfoImpl();
    ...
    private def innerClass(Module imported) '''
        new «YangModuleInfo.importedName»() {
            «imported.classBody»
        }
    '''
    ...
    private def CharSequence classBody(Module m) '''
        ...
        public «InputStream.importedName» getModuleSourceStream() throws «IOException.importedName» {
            ...
            «IF path == null»
                return null;
            «ELSE»
                return new «FileInputStream.importedName»("«path»");
            «ENDIF»
        }
            ...
            «FOR imp : m.imports»
                ...
                «IF rev == null»
                    ...
                    «FOR module : modules»
                        ...
                        «ENDIF»
                    «ENDFOR»
                    set.add(«innerClass(sorted.lastEntry().value)»);
                «ELSE»
                    set.add(«innerClass(ctx.findModuleByName(name, rev))»);
                «ENDIF»
            «ENDFOR»
