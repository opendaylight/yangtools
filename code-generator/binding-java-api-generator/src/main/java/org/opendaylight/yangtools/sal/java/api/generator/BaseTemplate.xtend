...
abstract class BaseTemplate {
        ...
        «IF restrictions != null && !(restrictions.lengthConstraints.empty)»
            ...
            public static «List.importedName»<«Range.importedName»<«numberClass.importedNumber»>> «methodName»() {
                «IF numberClass.equals(typeof(BigDecimal))»
                    «lengthBody(restrictions, numberClass, className, varName, true)»
                «ELSE»
                    «lengthBody(restrictions, typeof(BigInteger), className, varName, true)»
                «ENDIF»
            }
        «ENDIF»
    '''

    def protected lengthBody(Restrictions restrictions, Class<? extends Number> numberClass, String className, String varName, boolean isMethod) '''
        if («varName» == null) {
        ...
        «IF restrictions != null && !(restrictions.rangeConstraints.empty)»
            ...
            public static «List.importedName»<«Range.importedName»<«number»>> «methodName»() {
                «IF returnType.fullyQualifiedName.equals(BigDecimal.canonicalName)»
                    «rangeBody(restrictions, BigDecimal, className, varName, true)»
                «ELSE»
                    «rangeBody(restrictions, BigInteger, className, varName, true)»
                «ENDIF»
            }
        «ENDIF»
    '''
    ...
    def protected rangeBody(Restrictions restrictions, Class<? extends Number> numberClass, String className, String varName, boolean isMethod) '''
        if («varName» == null) {
            synchronized («className».class) {
                if («varName» == null) {
                    «ImmutableList.importedName».Builder<«Range.importedName»<«numberClass.importedName»>> builder = «ImmutableList.importedName».builder();
