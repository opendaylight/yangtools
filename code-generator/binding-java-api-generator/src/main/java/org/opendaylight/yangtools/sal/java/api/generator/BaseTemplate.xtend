...
abstract class BaseTemplate {
    ...
    private def generateLengthRestriction(Type returnType, Restrictions restrictions, String paramName, boolean isNestedType) '''
        «val boolean isArray = returnType.name.contains("[")»
        «val clazz = restrictions.lengthConstraints.iterator.next.min.class»
        if («paramName» != null) {
            boolean isValidLength = false;
            String i = "" + «paramName»«IF isNestedType».getValue()«ENDIF».length«IF !isArray»()«ENDIF»;
            for («Range.importedName»<«clazz.importedNumber»> r : «IF isNestedType»«returnType.importedName».«ENDIF»length()) {
                if (r.contains(new «clazz.importedNumber»(i))) {
                    isValidLength = true;
                }
            }
            if (!isValidLength) {
    ...
    private def generateRangeRestriction(Type returnType, Restrictions restrictions, String paramName, boolean isNestedType) '''
        ...
        if («paramName» != null) {
            ...
            for («Range.importedName»<«clazz.importedNumber»> r : «IF isNestedType»«returnType.importedName».«ENDIF»range()) {
                if (r.contains(new «clazz.importedNumber»(«paramName».toString()))) {
                    isValidRange = true;
                }
            }
            if (!isValidRange) {
    ...
    def generateLengthMethod(String methodName, Type type, String className, String varName) '''
        «val Restrictions restrictions = type.restrictions»
        «IF restrictions != null && !(restrictions.lengthConstraints.empty)»
            «val clazz = restrictions.lengthConstraints.iterator.next.min.class»
            public static «List.importedName»<«Range.importedName»<«clazz.importedNumber»>> «methodName»() {
            «IF clazz.equals(typeof(BigDecimal))»
                «lengthMethodBody(restrictions, clazz, className, varName)»
            «ELSE»
    ...
    def private lengthMethodBody(Restrictions restrictions, Class<? extends Number> clazz, String className, String varName) '''
        if («varName» == null) {
            synchronized («className».class) {
                if («varName» == null) {
                    «ImmutableList.importedName».Builder<«Range.importedName»<«clazz.importedName»>> builder = «ImmutableList.importedName».builder();
                    «FOR r : restrictions.lengthConstraints»
                        builder.add(«Range.importedName».closed(«numericValue(clazz, r.min)», «numericValue(clazz, r.max)»));
                    «ENDFOR»
    ...
    def generateRangeMethod(String methodName, Type type, String className, String varName) '''
        «val Restrictions restrictions = type.restrictions»
        «IF restrictions != null && !(restrictions.rangeConstraints.empty)»
            «val clazz = restrictions.rangeConstraints.iterator.next.min.class»
            public static «List.importedName»<«Range.importedName»<«clazz.importedNumber»>> «methodName»() {
            «IF clazz.equals(typeof(BigDecimal))»
                «rangeMethodBody(restrictions, clazz, className, varName)»
            «ELSE»
     ...
     * @param clazz restriction number class
     * @param className current class name
     * @param varName restriction variable name
     */
    def private rangeMethodBody(Restrictions restrictions, Class<? extends Number> clazz, String className, String varName) '''
        if («varName» == null) {
            synchronized («className».class) {
                if («varName» == null) { 
                    «ImmutableList.importedName».Builder<«Range.importedName»<«clazz.importedName»>> builder = «ImmutableList.importedName».builder();
