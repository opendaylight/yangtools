...
abstract class BaseTemplate {
    ...
    def printRangeConstraint(Type returnType, Class<? extends Number> clazz, String paramName, boolean isNestedType) '''
        «IF clazz.canonicalName.equals("java.math.BigInteger")»
            «IF returnType.fullyQualifiedName.equals("java.math.BigInteger")»
                «clazz.importedNumber» _constraint = «paramName»;
            «ELSE»
                «clazz.importedNumber» _constraint = «clazz.importedNumber».valueOf(«paramName»«IF isNestedType».getValue()«ENDIF».longValue());
            «ENDIF»
        «ELSE»
