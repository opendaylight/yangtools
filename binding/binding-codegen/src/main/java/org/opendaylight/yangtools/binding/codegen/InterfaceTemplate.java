...
sealed class InterfaceTemplate extends BaseTemplate permits DataRootTemplate {
    ...
    final @Nullable BlockBuilder generateBindingHashCode() {
        ...
        final var bb = newBlockBuilder()
            .at().eol(importedName(OVERRIDE))
            .str("default int bindingHashCode()").oB()
                .eol("int result = 1;");
        if (!props.isEmpty()) {
        ...
        if (augmentable) {
            bb
                .str("for (var augmentation : augmentations().values())").oB()
            ...
            .str("default boolean bindingEquals(final ").str(fullyQualifiedNonNull(type())).str(" obj)").jBlock(bb -> {
                bb
                    ...
                    for (var property : ByTypeMemberComparator.sort(props)) {
                        ...
                        bb.nl().ind("&& ").str(importedUtilClass(property)).str(".equals(").str(getterName)
                            .str("(), obj.").str(getterName).str("())");
                    }
                    if (augmentable) {
