...
final class BlockBuilder extends Block.Builder {
    ...
    // offset of the start of the third line, i.e. the one past the second newline in current block
    private int thirdLine = -1;
    ...
    private Block build(final int length) {
        ...
        if (length == secondLine) {
            // also deals with empty line
            return Block.ofLine(str);
        }
        if (length == thirdLine) {
            return new Block2(str, secondLine - 1);
        }
