...
abstract sealed class ScalarTypeObjectTemplate extends ArchetypeTemplate<ScalarTypeObjectArchetype> {
    ...
    private abstract static sealed class ValueCheckers {
        ...
        abstract void appendInvocations(BlockBuilder bb, GeneratedClass javaType);
    }
    ...
    private static final class NoopValueCheckers extends ValueCheckers {
        ...
        @Override
        void appendInvocations(final BlockBuilder bb, final GeneratedClass javaType) {
            // no-op
        }
    ...
    private static sealed class BaseValueCheckers extends ValueCheckers {
        ...
        void appendInvocations(final BlockBuilder bb, final GeneratedClass javaType) {
            ...
            if (!patterns.isEmpty()) {
                bb
                    .str(javaType.getReferenceString(CODEHELPERS))
                        .eol(".checkPattern(_value, " + MEMBER_PATTERN_LIST + ", " + MEMBER_REGEX_LIST + ");");
            }
        }
    ...
    private static final class RangeValueCheckers extends BaseValueCheckers {
        ...
        @Override
        void appendInvocations(final BlockBuilder bb, final GeneratedClass javaType) {
            super.appendInvocations(bb, javaType);
            rangeGenerator.appendCheckerCall(bb, "_value", "_value");
        }
    }
    ...
    private BlockBuilder body(final boolean topLevel) {
        ...
        valueCheckers.appendInvocations(bb, javaType);
        bb.cB();
        ...
        if (!INSTANCE_IDENTIFIER.name().equals(valueType.name())) {
            ...
            if (VALUEOF_TYPES.contains(valueType)) {
                bb.str(importedType).eol(".valueOf(defaultValue)");
            } else if (valueType instanceof Decimal64Type decimal64) {
                bb.str(importedType).str(".valueOf(defaultValue).scaleTo(").jInt(decimal64.fractionDigits()).eol(")");
            } else if (valueType.equals(STRING_TYPE)) {
                bb.eol("defaultValue");
            } else if (valueType.equals(BINARY_TYPE)) {
                bb.str(importedName(JU_BASE64)).eol(".getDecoder().decode(defaultValue)");
            } else if (valueType.equals(EMPTY_TYPE)) {
                bb.str(importedName(CODEHELPERS)).eol(".emptyFor(defaultValue)");
            } else {
                bb.str("new ").str(importedType).eol("(defaultValue)");
            }
            bb
                .eol(");")
                .cB();
