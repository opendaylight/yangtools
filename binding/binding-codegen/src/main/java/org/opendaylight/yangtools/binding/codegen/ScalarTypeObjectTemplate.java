...
abstract sealed class ScalarTypeObjectTemplate extends ArchetypeTemplate<ScalarTypeObjectArchetype> {
        ...
        void appendMethods(final BlockBuilder bb, final ConcreteType valueType) {
            ...
            bb
                ...
                .str("public final boolean equals(").str(importedName(OBJECT)).str(" obj)").oB()
                    .str("return this == obj || obj instanceof ").str(type().simpleName()).eol(" other")
                    .str("    && ");
            if (valueType.isArray()) {
     ...
     */
    private sealed interface ValueCheckers {
        /**
         ...
         */
        static ValueCheckers of(final ScalarTypeObjectArchetype archetype) {
            final var restrictions = archetype.getRestrictions();
            return restrictions.isEmpty() ? NoopValueCheckers.INSTANCE
                : new RestrictionsValueCheckers(archetype.valueType(), restrictions);
        }

        void appendDeclarations(BlockBuilder bb, GeneratedClass javaType);

        void appendInvocations(BlockBuilder bb);
    }

    private static final class NoopValueCheckers implements ValueCheckers {
        ...
        @Override
        public void appendDeclarations(final BlockBuilder bb, final GeneratedClass javaType) {
            // no-op
        }

        @Override
        public void appendInvocations(final BlockBuilder bb) {
            // no-op
        }
    ...
    private static final class RestrictionsValueCheckers implements ValueCheckers {
        private final @Nullable AbstractRangeGenerator<?> rangeGenerator;
        private final Restrictions restrictions;
        private final ConcreteType valueType;

        RestrictionsValueCheckers(final ConcreteType valueType, final Restrictions restrictions) {
            this.valueType = requireNonNull(valueType);
            rangeGenerator = restrictions.getRangeConstraint().isEmpty() ? null
                : requireNonNull(AbstractRangeGenerator.forType(valueType));
        ...
        public void appendDeclarations(final BlockBuilder bb, final GeneratedClass javaType) {
            ...
            if (length.isPresent()) {
                bb.blk(LengthGenerator.generateLengthChecker("_value", valueType, length.orElseThrow(), javaType)).nl();
            }

            final var rangeGen = rangeGenerator;
            if (rangeGen != null) {
                bb.blk(rangeGen.generateRangeChecker("_value", restrictions.getRangeConstraint().orElseThrow(),
                    javaType)).nl();
            }
...
//          .blk(generateRestrictions(type(), fieldName, value.getReturnType()))
        ...
        public void appendInvocations(final BlockBuilder bb) {
            final var length = restrictions.getLengthConstraint();
            if (length.isPresent()) {
                LengthGenerator.appendCheckerCall(bb, "_value", "_value");
            }

            final var rangeGen = rangeGenerator;
            if (rangeGen != null) {
                rangeGen.appendCheckerCall(bb, "_value", "_value");
            }
...
//          .blk(genPatternEnforcer(fieldName));
    ...
    private BlockBuilder body(final boolean topLevel) {
        ...
        valueCheckers.appendInvocations(bb);
        bb.cB();
