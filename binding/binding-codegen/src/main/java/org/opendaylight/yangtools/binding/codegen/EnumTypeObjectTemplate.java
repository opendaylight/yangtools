    ...
    private void appendBody(final BlockBuilder bb) {
        ...
        if (it.hasNext()) {
            while (true) {
                ...
                bb.str("    ").str(value.constantName()).str("(").iStr(value.value()).str(", \"").str(value.name())
                    .append("\")");
        ...
        bb
            ...
            .eol("        return switch (name)").oS();
        for (var value : archetype.values()) {
            bb.str("            case \"").str(value.name()).str("\" -> ").str(value.constantName()).eS();
        }
        bb
            .eol("            default -> null;")
            .eol("        ").cS()
            ...
            .eol("        return switch (intValue)").oS();
        for (var value : archetype.values()) {
            bb.str("            case ").iStr(value.value()).str(" -> ").str(value.constantName()).eS();
        }
        bb
            .eol("            default -> null;")
            .eol("        ").cS()
