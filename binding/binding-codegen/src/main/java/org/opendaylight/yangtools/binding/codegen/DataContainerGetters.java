 ...
 */
@NonNullByDefault
final class DataContainerGetters implements BlockFragment {
    record Spec(MethodSignature method, boolean hasOverride) implements Comparable<Spec> {
        ...
        String propertyName() {
            return toFirstLower(method.name().substring(GETTER_PREFIX_LENGTH));
        }
    ...
    private final List<Spec> methods;
    private final List<Spec> partialMethods;

    DataContainerGetters(final InterfaceTemplate<?> template) {
        this.template = requireNonNull(template);

        // traverse the DataContainerArchetype's partials and collect their methods
        final var archetype = template.archetype;
        ...
        methods = List.copyOf(nameToSpec.values());
        partialMethods = partials.values().stream()
    ...
    public void appendTo(final BlockBuilder bb) {
        ...
        while (true) {
            ...
            final var method = spec.method;
            final var override = spec.hasOverride ? (BlockFragment) bbb -> {
            ...
            switch (method.statement()) {
                ...
                case AnydataEffectiveStatement stmt -> generateRequireMethod(bb, method, override);
                case AnyxmlEffectiveStatement stmt -> generateRequireMethod(bb, method, override);
                case LeafEffectiveStatement stmt -> generateRequireMethod(bb, method, override);
                case LeafListEffectiveStatement stmt -> generateRequireMethod(bb, method, override);
                default -> {
                    // nothing else
                }
            }
