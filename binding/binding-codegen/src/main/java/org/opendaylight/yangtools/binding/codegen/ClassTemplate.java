...
class ClassTemplate extends BaseTemplate {
    ...
    private BlockBuilder validNamesAndValues(final BitsTypeDefinition typedef) {
        ...
        final var bb = new BlockBuilder()
            ...
            .str("}").nl()
            .nl()
            .at().eol(override).str("public boolean[] values() {").nl()
    ...
    private @Nullable BlockBuilder generateToString(final List<GeneratedProperty> props) {
        ...
        for (var property : props) {
            bb.str("    ").str(importedName(CODEHELPERS)).str(".").append(valueAppender(property));
            bb.str("(helper, \"").str(property.getName()).str("\", ").str(fieldName(property)).str(");").newLine();
        }
        return bb
