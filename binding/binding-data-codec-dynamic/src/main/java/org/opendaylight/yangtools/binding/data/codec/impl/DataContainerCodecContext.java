...
abstract sealed class DataContainerCodecContext<D extends DataContainer, R extends CompositeRuntimeType,
        ...
        permits ChoiceCodecContext, CommonDataObjectCodecContext, NotificationCodecContext {
    ...
    public CodecContext yangPathArgumentChild(final PathArgument arg) {
        final var supplier = switch (arg) {
            case NodeIdentifier nodeId -> yangChildSupplier(nodeId);
            case NodeIdentifierWithPredicates nip -> yangChildSupplier(new NodeIdentifier(nip.getNodeType()));
            case NodeWithValue<?> nv -> null;
        };
    ...
    static @NonNull IllegalArgumentException childNullException(final BindingRuntimeContext runtimeContext,
        ...
        try {
            runtimeContext.loadClass(TypeName.ofClass(childClass));
    ...
    private static @NonNull ChildAddressabilitySummary computeChildAddressabilitySummary(final Object nodeSchema) {
        ...
        if (nodeSchema instanceof ChoiceSchemaNode choice) {
