...
public abstract class AugmentableCodecEntryObject<T extends EntryObject<T, K>, K extends Key<T>>
        extends AugmentableCodecDataObject<T> implements EntryObject<T, K> {
    private static final VarHandle KEY;

    static {
        try {
            KEY = MethodHandles.lookup().findVarHandle(AugmentableCodecEntryObject.class, "key", Key.class);
    ...
    protected AugmentableCodecEntryObject(CommonDataObjectCodecContext<T, ?> context, DataContainerNode data) {
        super(context, data);
    }
