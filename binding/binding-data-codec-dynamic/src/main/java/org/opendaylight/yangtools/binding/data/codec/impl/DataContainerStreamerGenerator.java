...
final class DataContainerStreamerGenerator<T extends DataContainerStreamer<?>> implements ClassGenerator<T> {
        ...
        for (var schemaChild : statement.toDataNodeContainer().getChildNodes()) {
            if (!schemaChild.isAugmenting()) {
                ...
                if (schemaChild instanceof EffectiveStatementEquivalent<?> equiv) {
                    final var childStream = createStream(loader, props, equiv.asEffectiveStatement(), getter);
                    if (childStream != null) {
    ...
    private ChildStream createStream(final BindingClassLoader loader, final ImmutableMap<String, Type> props,
            final EffectiveStatement<?, ?> childSchema, final Method getter) {
        return switch (childSchema) {
