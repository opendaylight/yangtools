    ...
    GeneratedTransferObject<?> createDerivedType(final TypeBuilderFactory builderFactory,
        ...
        return switch (baseType) {
            ...
            case UnionTypeObjectArchetype union -> {
                final var restrictions = Restrictions.compute(statement, statement.typeStatement());
                yield new UnionTypeObjectArchetype(typeName, statement, union.typePropertyNames(),
                    union.typePropertyTypes(), List.of(), restrictions != null ? restrictions : Restrictions.empty(),
                    union);
            }
        };
    }
