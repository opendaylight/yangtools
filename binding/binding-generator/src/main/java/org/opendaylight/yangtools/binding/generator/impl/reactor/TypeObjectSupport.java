...
abstract sealed class TypeObjectSupport permits TypeObjectSupport.Base, TypeObjectSupport.Derived {
    ...
    static final class Union extends Base {
        ...
        /**
         * Returns string which contains the same value as <code>name</code> but integer suffix is incremented by one.
         * If <code>name</code> contains no number suffix, a new suffix initialized at 1 is added. A suffix is actually
         * composed of a '$' marker, which is safe, as no YANG identifier can contain '$', and a unsigned decimal
         * integer.
         *
