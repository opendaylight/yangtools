...
abstract sealed class TypeObjectSupport permits TypeObjectSupport.Base, TypeObjectSupport.Derived {
    ...
    static TypeObjectSupport of(final TypeEffectiveStatement type) {
        ...
        return switch (simpleName) {
            case "binary" -> new Scalar(type, ScalarTypes.BINARY);
            case "bits" -> new Bits(type);
            case "boolean" -> new Scalar(type, ScalarTypes.BOOLEAN);
            case "decimal64" -> new Scalar(type, (DecimalTypeDefinition) type.typeDefinition());
            case "empty" -> new Scalar(type, ScalarTypes.EMPTY);
            case "enumeration" -> new Enumeration(type);
            case "identityref" -> new Identityref(type);
            case "int8" -> new Scalar(type, ScalarTypes.INT8);
            case "int16" -> new Scalar(type, ScalarTypes.INT16);
            case "int32" -> new Scalar(type, ScalarTypes.INT32);
            case "int64" -> new Scalar(type, ScalarTypes.INT64);
            case "string" -> new Scalar(type, ScalarTypes.STRING);
            case "union" -> new Union(type);
            case "leafref" -> new Leafref(type);
            case "instance-identifier" -> new Scalar(type, ScalarTypes.INSTANCE_IDENTIFIER);
            case "uint8" -> new Scalar(type, ScalarTypes.UINT8);
            case "uint16" -> new Scalar(type, ScalarTypes.UINT16);
            case "uint32" -> new Scalar(type, ScalarTypes.UINT32);
            case "uint64" -> new Scalar(type, ScalarTypes.UINT64);
