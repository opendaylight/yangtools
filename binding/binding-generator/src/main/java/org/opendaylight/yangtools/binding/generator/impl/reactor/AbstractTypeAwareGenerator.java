...
abstract class AbstractTypeAwareGenerator<
    ...
    @Override
    final GetterMethod constructGetter(final Type returnType, final Iterator<GetterAnnotation> annotations) {
        final var local = contextType;
        if (local == null) {
            return constructGetter(statement(), returnType, annotations);
        }
        return constructGetter(statement(), returnType, Iterators.concat(annotations,
