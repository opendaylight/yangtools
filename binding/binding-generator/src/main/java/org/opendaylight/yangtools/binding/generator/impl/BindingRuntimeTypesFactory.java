...
final class BindingRuntimeTypesFactory implements Mutable {
    ...
    private void indexModules(final Map<QNameModule, ModuleGenerator> moduleGens) {
        ...
        final var caseToChildren = new HashMap<CaseRuntimeType, List<EffectiveStatement<?, ?>>>();
        final var augmentToChildren = new HashMap<AugmentRuntimeType, List<EffectiveStatement<?, ?>>>();
    ...
    private void collectSubstsForCase(final Map<CaseRuntimeType, List<EffectiveStatement<?, ?>>> caseToChildrenStmts) {
        final var localToSubstitutions = HashMultimap.<CaseObjectArchetype, CaseRuntimeType>create();
     ...
     * <ul>
     *  <li>key - {@link CaseObjectArchetype} - the {@link CaseRuntimeType#javaType} we want to substitute</li>
     ...
     */
    private static void addSubstitutionalCases(
            final Multimap<CaseObjectArchetype, CaseRuntimeType> localToSubstitutions, final CaseRuntimeType local,
        ...
        for (var candidate : candidates) {
            ...
            if (substitutional) {
                localToSubstitutions.put(localType, candidate);
            }
        }
    }
    ...
    private void collectSubstsForAugment(
        ...
        for (var type : allTypes.values()) {
            ...
            final var runtimeArchetype = runtime.javaType();
            for (final var entry : augToChildrenStmts.entrySet()) {
                final var substitution = entry.getKey();
                // do not compare the same instance to itself
                if (runtime.equals(substitution)) {
                    continue;
                }

                final var substitChildren = entry.getValue();
                final var substitArchetype = substitution.javaType();
                final var substitIfaces = substitArchetype.getImplements();

                final var runtimeTypeChildren = augToChildrenStmts.get(runtime);
                final var runtimeTypeIfaces = runtimeArchetype.getImplements();
                ...
                substitutional &= runtimeArchetype.target().equals(substitArchetype.target());

                // if the 'augmentType' can be substituted for 'substitution', add it to the map
                if (substitutional) {
                    augmentToSubstitutionAugments.put(runtimeArchetype, substitution);
                }
            }
        }
    }
