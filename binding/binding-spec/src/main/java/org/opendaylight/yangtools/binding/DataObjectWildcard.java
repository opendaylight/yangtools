 ...
 */
public sealed class DataObjectWildcard<T extends DataObject> extends DataObjectReference<DataObjectStep<?>, T>
    ...
    public static final class WithKey<T extends KeyAware<K> & DataObject, K extends Key<T>>
        ...
        WithKey(final Iterable<DataObjectStep<?>> steps, final KeyStep<K, T> lastStep) {
     ...
     */
    final @NonNull Iterable<DataObjectStep<?>> steps;

    private final @NonNull DataObjectStep<T> lastStep;

    DataObjectWildcard(final Iterable<DataObjectStep<?>> steps, final DataObjectStep<T> lastStep) {
    ...
    private <N extends DataObject> @NonNull DataObjectWildcard<N> childIdentifier(final DataObjectStep<N> arg) {
    ...
    public final <N extends KeyAware<K> & ChildOf<? super T>, K extends Key<N>>
        ...
        return (WithKey<N, K>) childIdentifier(new KeyStep<>(listItem, listKey));
    }
    ...
    public final <C extends ChoiceIn<? super T> & DataObject, K extends Key<N>,
        ...
        return (WithKey<N, K>) childIdentifier(new KeyStep<>(listItem, requireNonNull(caze), listKey));
    }
    ...
    public DataObjectIdentifier<T> tryToIdentifier() {
        final var steps = tryExactSteps();
        return steps == null ? null : (DataObjectIdentifier<T>) DataObjectIdentifier.ofSteps(steps);
    }

    final @Nullable ImmutableList<ExactDataObjectStep<?>> tryExactSteps() {
        final var ret = unsafeExactSteps();
        return ret.stream().allMatch(ExactDataObjectStep.class::isInstance) ? ret : null;
    }
    ...
    final @NonNull ImmutableList<ExactDataObjectStep<?>> getExactSteps() {
        ...
        return unsafeExactSteps();
    }

    @SuppressWarnings({ "rawtypes", "unchecked" })
    private @NonNull ImmutableList<ExactDataObjectStep<?>> unsafeExactSteps() {
        return ImmutableList.copyOf((Iterable) steps);
    }
    ...
    static <N extends DataObject> @NonNull DataObjectWildcard<N> trustedCreate(final DataObjectStep<?> lastStep,
        ...
        return switch (lastStep) {
            case NodeStep<?> node -> new DataObjectWildcard(pathArguments, lastStep);
            case KeyStep<?, ?> key -> new WithKey(pathArguments, key);
            case KeylessStep keyless -> new DataObjectWildcard(pathArguments, lastStep);
        };
    }
    ...
    public abstract static sealed class Builder<T extends DataObject>
        ...
        Builder(final DataObjectWildcard<T> identifier) {
        ...
        @Override
        final <X extends DataObject> RegularBuilder<X> append(final ExactDataObjectStep<?> step) {
            return append((DataObjectStep<?>) step);
        }

        @Override
        final <X extends DataObject> RegularBuilder<X> append(final InexactDataObjectStep<?> step) {
            return append((DataObjectStep<?>) step);
        }
        ...
        abstract <X extends DataObject> @NonNull RegularBuilder<X> append(@NonNull DataObjectStep<?> step);
    }
    ...
    public static final class KeyedBuilder<T extends DataObject & KeyAware<K>, K extends Key<T>>
        ...
        @SuppressWarnings("unchecked")
        KeyedBuilder(final RegularBuilder<?> prev, final KeyStep<?, ?> lastStep) {
            super(prev, lastStep);
        ...
        @Override
        <X extends DataObject> RegularBuilder<X> append(final DataObjectStep<?> step) {
    ...
    private static final class RegularBuilder<T extends DataObject> extends Builder<T> {
        ...
        @SuppressWarnings("unchecked")
        RegularBuilder(final DataObjectStep<?> step) {
            super(step);
            lastStep = (DataObjectStep<T>) requireNonNull(step);
        }

        RegularBuilder(final DataObjectWildcard<T> identifier) {
            super(identifier);
            lastStep = identifier.lastStep();
        }

        @SuppressWarnings("unchecked")
        RegularBuilder(final KeyedBuilder<?, ?> prev, final DataObjectStep<?> step) {
            super(prev, step);
            lastStep = (DataObjectStep<T>) requireNonNull(step);
        }
        ...
        public DataObjectWildcard<T> build() {
            return new DataObjectWildcard<>(buildSteps(), lastStep);
        }
