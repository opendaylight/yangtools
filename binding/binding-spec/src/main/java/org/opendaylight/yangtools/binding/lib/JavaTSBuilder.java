...
public final class JavaTSBuilder implements Mutable {
    ...
    private final Collection<? extends Augmentation<?>> augment;
    ...
    <T extends Augmentable<T> & DataContainer> JavaTSBuilder(final T augmentable) {
        sb.append(augmentable.implementedInterface().getSimpleName()).append('{');
        augment = augmentable.augmentations().values();
    }
    ...
    public String build() {
        if (!augment.isEmpty()) {
            ...
            startProperty("augment").append(augment.toString());
        }
