...
public abstract sealed class AbstractDataObjectReferenceBuilder<T extends DataObject> implements Builder<T>
        permits DataObjectReferenceBuilder, InstanceIdentifier.Builder {
    ...
    @Override
    public <N extends DataObject & Augmentation<? super T>> AbstractDataObjectReferenceBuilder<N> augmentation(
            final Class<N> augmentation) {
        return append(new NodeStep<>(augmentation));
    }

    @Override
    public <N extends ChildOf<? super T>> AbstractDataObjectReferenceBuilder<N> child(final Class<N> container) {
...
//    }
    ...
    protected abstract <X extends DataObject> @NonNull AbstractDataObjectReferenceBuilder<X> append(
        DataObjectStep<X> step);

    protected abstract <X extends DataObject & KeyAware<Y>, Y extends Key<X>>
        @NonNull AbstractDataObjectReferenceBuilder<X> append(@NonNull KeyStep<Y, X> step);
