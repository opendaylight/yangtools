 ...
 *
 * <p>This is quite similar to a {@link DataObject}, but it cannot be directly stored in a data store. Its data subtree
 * may actually have a relationship to a data store subtree -- but that part is dependent on the context in which this
 * object is used.
 *
 * <p>As per the extension definition, the structure of children is such that there is a single {@code container} which
 ...
 *
 * <p>Furthermore {@code rc:yang-data} can only appear as a top-level module contract and therefore the code generation
 * is limited to a single interface, which only provides the default implementation of {@link #implementedInterface()}
 * bound to itself.
 *
